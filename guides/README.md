# Guides

这里是 `RedCube AI` 面向 GitHub 与外部读者的公开文档入口。

如果你是专家、教授、内容团队负责人或需要接入本项目的技术同事，优先从这里开始。

## 面向人类用户

- [人类用户快速上手](human_quickstart.md)
- [典型交付示例](deliverable_examples.md)

## 面向技术同事

- [私有作者信息与 prompts 配置](private-profile-setup.md)
- [公开发布到 GitHub](public-github-publish.md)

## 文档边界

- `guides/`：公开文档、GitHub 首页可引用的说明
- `docs/`：内部 AI / Superpowers 文档、开发计划、设计草案与过程痕迹

如果你要新增一个会在 GitHub 首页、README、外部分享中被引用的文档，请优先放在 `guides/`。
