...
function parseResolvedCommand(stdout) {
...
function parseVersionProbe(stdout) {
...
function isStablePythonVersion(probe) {
  return Number(probe?.major) === 3 && Number(probe?.minor) > 0 && Number(probe?.minor) < 14;
}

function probePythonWithPlaywright(command, spawnSyncImpl) {
  try {
...
function probePythonVersion(command, spawnSyncImpl) {
  try {
...
function resolveRuntimeStateRoot(env) {
...
function managedPythonPaths(env) {
...
function managedPythonReady(paths, spawnSyncImpl, signature) {
...
function runOrThrow(command, args, spawnSyncImpl, label) {
  let result;
  try {
...
function resolveManagedBasePython(spawnSyncImpl) {
...
function ensureManagedPythonCommand(env, spawnSyncImpl) {
