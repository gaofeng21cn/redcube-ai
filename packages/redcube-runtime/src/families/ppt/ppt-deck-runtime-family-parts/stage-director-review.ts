  ...
  async function generateDirectorReviewDraft(contract, deliverablePaths, preflight, adapter = CODEX_DEFAULT_ADAPTER) {
    ...
    const renderSummary = isImagePagesArtifact(renderArtifact)
      ...
      ? summarizeNativeSlides(renderArtifact).map((slide) => ({
          ...
          structural_visual_count: slide.structural_visual_count,
          structural_visual_roles: slide.structural_visual_roles,
          mechanical_card_template_absent: slide.mechanical_card_template_absent,
          non_text_visual_specific_ok: slide.non_text_visual_specific_ok,
          peak_page: false,
    ...
    const visualQaContext = buildPptInAttemptVisualQaContext({
      ...
      contract,
      renderArtifact,
      qaRoute: 'visual_director_review',
