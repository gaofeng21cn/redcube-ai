...
interface SourceAugmentationAdapterRunInput {
  requestFile: string;
  request: JsonRecord;
  sourceAugmentationResultFile?: string;
}

interface SourceAugmentationAdapterResult {
  ok: boolean;
  blockingReason?: string;
  executionSurface?: string;
  executorOutput?: JsonRecord;
}
...
function validateExecutorOutput(executorOutput: JsonRecord, request: JsonRecord): SourceAugmentationAdapterResult {
  ...
  const validation = validateSourceAugmentationResultContract(executorOutput, {
    ...
    requiredEvidenceGaps: Array.isArray(trigger.evidence_gaps)
...
function resolveResultFilePath(
  configuredResultFile: unknown,
  sourceAugmentationResultFile: unknown,
  requestFile: string,
): string {
  const explicit = safeText(configuredResultFile);
  if (explicit) return path.resolve(explicit);
  const sourceResultFile = safeText(sourceAugmentationResultFile);
  if (sourceResultFile) return sourceResultFile;
  return path.join(path.dirname(requestFile), 'source-augmentation-result.json');
}

function buildExternalCommandAdapter(command: unknown) {
  ...
  return {
    ...
    run({ requestFile, request }: SourceAugmentationAdapterRunInput): SourceAugmentationAdapterResult {
      if (!executorCmd) {
        return {
          ok: false,
...
function buildResultFileAdapter(resultFile: unknown) {
  ...
  return {
    ...
    }: SourceAugmentationAdapterRunInput): SourceAugmentationAdapterResult {
