...
type LifecycleResidueStage = 'story_architecture' | 'visual_authorship' | 'delivery_packaging';
type ViolationStatus = 'present' | 'cleared';

interface ResidueDefinition {
  violation_id: string;
  stage: LifecycleResidueStage;
  protected_surface: string;
  file: string;
  evidence_patterns: string[];
  why_blocked: string;
  inverted_match?: boolean;
}

interface ResidueViolation extends ResidueDefinition {
...
function collectViolations(definitions: readonly ResidueDefinition[]): ResidueViolation[] {
...
function summarizeLifecycleResidue(violations: ResidueViolation[]) {
  const grouped: Record<LifecycleResidueStage, ResidueViolation[]> = {
    story_architecture: [],
    visual_authorship: [],
    delivery_packaging: [],
  };
...
export const P19_UNIFIED_LIFECYCLE_CONTRACT = Object.freeze({
  ...
  family_mapping: {
    xiaohongshu: {
      ...
      visual_authorship: ['visual_direction', 'author_image_pages', 'repair_image_pages', 'render_html', 'fix_html'],
...
export const P19_TEAM_GATE_CONTRACT = Object.freeze({
  ...
  frozen_contracts: {
    ...
    ppt_visual_director_review_contract: [
      ...
      'packages/redcube-runtime/src/families/ppt/overlay/profiles.ts',
...
export const P19_CREATIVE_OWNERSHIP_EXECUTION_CONTRACT = Object.freeze({
  ...
  primary_creative_routes: {
    xiaohongshu: ['storyline', 'single_note_plan', 'visual_direction', 'author_image_pages', 'repair_image_pages', 'render_html', 'fix_html', 'publish_copy'],
    ppt_deck: ['storyline', 'detailed_outline', 'slide_blueprint', 'visual_direction', 'author_image_pages', 'repair_image_pages', 'render_html', 'fix_html'],
  },
...
const XIAOHONGSHU_RESIDUE_DEFINITIONS: readonly ResidueDefinition[] = Object.freeze([
  {
...
const PPT_RESIDUE_DEFINITIONS: readonly ResidueDefinition[] = Object.freeze([
  {
...
function buildReviewOverlayStatus() {
  const pptReviewActive = matchesAllEvidence('packages/redcube-runtime/src/families/ppt/overlay/profiles.ts', [
