...
export function buildArtifactLocatorContract({ workspaceRoot, productSessionSurfaceRef }) {
  return {
    ...
    domain_id: DOMAIN_ID,
    owner: DOMAIN_OWNER,
    locator_model: 'opl_stage_folder_contract_refs_only',
    primary_artifact_truth: {
      surface_kind: 'opl_stage_folder_contract_consumer',
      contract_id: 'rca.opl_stage_folder_artifact_runtime.v1',
      physical_root_template: '$OPL_STATE_DIR/runtime-state/domains/redcube_ai/deliverables/<program_id>/<topic_id>/<deliverable_id>/stages/<nn-stage>/attempts/<attempt_id>/',
      required_attempt_members: [
        'attempt.json',
        'manifest.json',
        'inputs/',
        'outputs/',
      ...
      current_pointer_refs: [
        'current.json',
        'latest.json',
        'stages/<nn-stage>/current.json',
        'stages/<nn-stage>/latest',
      ],
      completion_rule: 'success requires required output roles, valid role manifest, owner receipt refs, and receipt file',
      blocked_rule: 'blocked requires typed blocker refs and evidence file',
      orphan_rule: 'outputs without valid role manifest/receipt/evidence are orphan artifacts and never complete a stage',
    ...
    authority_boundary: {
      ...
      owner_receipt_owner: DOMAIN_OWNER,
      visual_review_export_verdict_owner: DOMAIN_OWNER,
      opl_role: 'stage_folder_locator_index_rebuild_status_explain_gallery_handoff_shell',
      ...
      derived_read_model_is_first_truth: false,
    },
  };
}
...
export function buildControlledVisualStageAttemptFixture() {
  const sharedRefs = {
    descriptor_refs: [
      'opl-generated:family_stage_control_plane#/stages',
