...
async function exportDomainActionAdapterJs(request: any) {
  const module = await import('./actions/domain-action-adapter.js');
  return module.exportDomainActionAdapter(request);
}

async function dispatchDomainActionAdapterJs(request: any) {
  const module = await import('./actions/domain-action-adapter.js');
...
export function exportDomainActionAdapter(request: Record<string, unknown>): Promise<Record<string, unknown>> {
  return exportDomainActionAdapterJs(request) as Promise<Record<string, unknown>>;
}
