...
function parseProfileLibrary(libraryText = '') {
  const profiles = {};
...
function inferRouting(routeText = '', profileIds = []) {
...
function normalizeBundlePayloadDir(basePath) {
...
function ensureInstallTarget(configHome, force) {
...
export function resolveConfigHome(options = {}) {
...
export function bootstrapPrivateProfile(options = {}) {
...
export function exportPrivateProfile(options = {}) {
  const configHome = resolveConfigHome(options);
...
export function installPrivateProfile(options = {}) {
  const bundleFile = path.resolve(String(options.bundleFile || '').trim());
  if (!bundleFile) {
  ...
  try {
    let payloadDir = '';
    if (statSync(bundleFile).isDirectory()) {
      payloadDir = normalizeBundlePayloadDir(bundleFile);
    } else {
      execFileSync('tar', ['-xzf', bundleFile, '-C', tempDir]);
      payloadDir = normalizeBundlePayloadDir(tempDir);
    }
