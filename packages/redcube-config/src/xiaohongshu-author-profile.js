...
function parseProfileBlock(profileId, blockText = '') {
  const block = safeText(blockText);
  if (!block) return null;

  const readField = (label, fallback = '') => safeText(
...
function libraryFileFromRuntime(runtimeConfig) {
  return path.join(runtimeConfig.promptsDir, 'aligned', '自动小红书', '作者档案库.md');
}

function inferConfigScope(workspaceRoot, runtimeConfig) {
...
export function ensureWorkspaceXiaohongshuAuthorTemplate({ workspaceRoot }) {
