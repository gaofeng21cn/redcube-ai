...
function normalizeExecutorRoutingConfig(raw: unknown, sourceFile: string): RedcubeExecutorRoutingConfig {
  ...
  for (const [routeKey, routePolicy] of Object.entries(routes)) {
    ...
    normalizedRoutes[routeKey] = {
      ...
      fallback: routePolicy.fallback === 'fail_closed' ? 'fail_closed' : 'inherit_effective_default_executor',
      failure_policy: routePolicy.failure_policy === 'fail_closed' ? 'fail_closed' : 'fallback_with_proof',
    };
  }
...
export function resolveExecutorRouting(
  ...
  return {
    ...
    structured_call_routing: {
      ...
      fallback: routePolicy?.fallback || 'inherit_effective_default_executor',
      failure_policy: routePolicy?.failure_policy || 'fallback_with_proof',
    },
