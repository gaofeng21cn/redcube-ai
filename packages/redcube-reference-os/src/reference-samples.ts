...
interface ReviewStateRecord {
  baseline?: {
    ...
    promoted_reference_id?: unknown;
    promoted_at?: unknown;
    promoted_by?: unknown;
    source_deliverable_id?: unknown;
  } | null;
}

const ALLOWED_REFERENCE_MODES = new Set<string>([
  'seed_backed_with_source_provenance',
  'source_backed',
]);

const ALLOWED_SAMPLE_STATUSES = new Set<string>([
  'approved',
]);

const ALLOWED_SUPPORTED_MODES = new Set<string>([
  'draft_new',
  'optimize_existing',
]);
...
export function validateReferenceSampleMeta(meta: unknown): ValidationResult {
  ...
  if (!isPlainObject(meta.scope)) {
    ...
    } else if (isNonEmptyString(meta.profileId) && !meta.scope.profileIds.includes(meta.profileId)) {
    ...
    } else if (!meta.scope.supportedModes.every((item: unknown) => ALLOWED_SUPPORTED_MODES.has(String(item || '').trim()))) {
...
export function loadReferenceSampleFixture({
  rootDir,
  familyId,
  sampleId,
}: {
  ...
  const meta = readJson<Partial<ReferenceSampleMeta> & JsonRecord>(metaFile);
  ...
  return {
    ...
    metaFile,
    sourceFile,
    meta: meta as ReferenceSampleMeta,
    sourceText,
    validation,
  };
}
...
export function summarizeReferenceCoverage({
  ...
  const approvedSamples: ApprovedSampleRecord[] = [];
  ...
  const missingProfiles: Array<{ overlay: string; profileId: string }> = [];
  for (const overlay of overlays) {
  ...
  return {
    ...
    expectedProfileCount: overlays.reduce((sum: number, overlay: string) => sum + overlayRegistry.listProfiles(overlay).length, 0),
...
export function listReferenceSamples({ rootDir }: { rootDir: string }): ReferenceSampleCatalog {
  if (!isNonEmptyString(rootDir)) {
    throw new Error('rootDir 不能为空');
  }

  const approvedSamples: ReferenceCatalogRecord[] = [];
  const invalidSamples: Array<ReferenceCatalogRecord & { errors: string[] }> = [];
...
export function listPromotedReferences({ workspaceRoot }: { workspaceRoot: string }): PromotedReferenceRecord[] {
  ...
  const promotedReferences: PromotedReferenceRecord[] = [];
  for (const topicEntry of readdirSync(topicsDir, { withFileTypes: true })) {
    ...
    for (const deliverableEntry of readdirSync(deliverablesDir, { withFileTypes: true })) {
      ...
      const deliverable = readJson<DeliverableRecord>(deliverableFile);
      const reviewState = readJson<ReviewStateRecord>(reviewStateFile);
      const baseline = reviewState?.baseline || null;
      if (baseline?.promotion_state !== 'promoted') continue;

      promotedReferences.push({
        promoted_reference_id: stringOrNull(baseline.promoted_reference_id) ?? '',
        promoted_at: stringOrNull(baseline.promoted_at),
        promoted_by: stringOrNull(baseline.promoted_by),
        source_deliverable_id: stringOrNull(baseline.source_deliverable_id) ?? deliverableId,
        deliverable_id: deliverableId,
        topic_id: topicId,
        overlay: stringOrNull(deliverable.overlay) ?? '',
        profile_id: stringOrNull(deliverable.profile_id) ?? '',
      });
    }
  }
