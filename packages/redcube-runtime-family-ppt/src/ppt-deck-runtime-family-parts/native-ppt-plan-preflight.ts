...
export function createNativePptPlanPreflightParts({
      ...
      ...(exactFixes.length > 0
        ? {
            native_shape_plan_validation_feedback_contract: {
              ...
              instruction: 'For each exact_shape_fix, find the matching native_shapes[] item by shape_id and satisfy every non-null requirement on that same shape. Set bounds.height_in to required_height_in or larger when present; set font_size to required_font_size or larger when present; rewrite editable_text so normalized audience text reaches required_text_char_count when present. If other_shape_id and required_gap_in are present, move or resize shape_id and/or other_shape_id so the two visible text boxes no longer overlap and keep at least required_gap_in clearance. If panel_shape_id and required_inset_in are present, keep the whole text box inside that panel with at least required_inset_in on left, top, right, and bottom. If required_connector_thickness_in is present, set both width_in and height_in positive and at least that thickness for line/connector shapes. If the existing composition has no room, redesign the page geometry, reduce slots, or shorten text while preserving meaning. Do not return the same failing text, the same failing font size, the same failing height, the same overlap, the same panel padding bleed, or a zero/negative connector dimension.',
        ...
        ? {
            native_shape_plan_structural_retry_contract: {
              ...
              instruction: [
                ...
                'Every non-decorative audience-facing shape must bind to a declared zone on the same slide and stay inside that zone.',
              ].join(' '),
        ...
        ...(exactFixes.length > 0
          ? {
              validation_retry_contract: {
                ...
                exact_shape_fixes: exactFixes,
                global_shape_class_fixes: safeArray(validationFeedback?.global_shape_class_fixes),
                per_shape_acceptance_rule: 'for every matching shape_id, native_shapes[] must satisfy all provided exact fixes: height >= required_height_in, font_size >= required_font_size, normalized editable_text length >= required_text_char_count, no visible text overlap with other_shape_id when required_gap_in is present, and full containment inside panel_shape_id with required_inset_in when present',
                ...
                structural_preservation_acceptance_rule: 'Python validation repair must still return a full structural plan: preserve template_layout_grammar, each slide template_layout_binding, all declared zones, safe_inset_in >= 0.15, zone_gap_in_min >= 0.32, zone_inset_in_min >= 0.15, and layout_zone_id bindings for all non-decorative audience-facing shapes.',
        ...
        ...(structuralFixes.length > 0
          ? {
              structural_retry_contract: {
                ...
                required: true,
                required_structural_fixes: structuralFixes,
                deck_acceptance_rule: 'top-level design_spec_lock, deck_layout_rhythm_plan, and template_layout_grammar must all be present, complete, and owned by llm_agent when required',
                slide_acceptance_rule: 'each slide must include template_layout_binding with selected archetype, archetype instance, rhythm role, zones, safe insets, and matching layout_zone_id on every non-decorative audience-facing shape',
  ...
  function nativePreflightRequiredInset(failure: JsonRecord): number | null {
    const reason = safeText(failure?.reason);
    if (reason !== 'ai_first_text_panel_safe_area_violation') return null;
  ...
  function nativePreflightGeometryRepairInstruction(failure: JsonRecord): string {
    ...
    if (reason === 'ai_first_text_panel_safe_area_violation') {
      ...
      return `Move or resize this text box so it sits fully inside ${panelShapeId} with at least 0.15in inset on all sides. A text box centered in a panel still fails if any edge bleeds into the panel padding.`;
    }
    if (reason === 'ai_first_connector_thickness_too_small' || reason === 'ai_first_connector_bounds_not_numeric') {
  ...
  function buildNativeValidationFeedback({
    ...
    const failures = safeArray(validation?.payload?.failures)
      .flatMap((slide) => safeArray(slide?.failures).map((failure) => ({
        ...
        archetype_id: safeText(failure?.archetype_id),
        ...
        layout_zone_id: safeText(failure?.layout_zone_id),
    ...
    return {
      repair_request: [
        ...
        'For ai_first_text_panel_safe_area_violation, use panel_shape_id/other_shape_id as the containing panel and place the failed text box completely inside it with at least 0.15in inset on all sides; if needed, enlarge the panel or shorten the text.',
        'For ai_first_shape_outside_template_layout_zone, use layout_zone_id, zone_bounds, and shape_bounds to move or resize the exact shape so the whole shape rectangle stays inside the declared zone. If the zone itself is too small, enlarge that zone and keep the same layout_zone_id binding instead of dropping the binding.',
