...
export function nativePptSampleLayoutProfile(contract: JsonRecord): JsonRecord | null {
  ...
  return {
    ...
    required_design_decision: 'Pick archetype before coords. Status board: title+claim+dominant input_hub+3 cards+vertical arrows+one proof band. Title 40-44pt; body 18pt; title/body>=2.0x. Cards>=4.0x1.35in content; point_text>=0.96in, 12-22 chars. Hub text 同一材料同步进入三条路线验证, >=10.4x0.82in, >=22pt, centered, spans card centers. Exactly 3 vertical kind=connector arrows: hub bottom to card top centers, w<=0.04in, h>=0.66in, gap>=0.12in, tailEnd=triangle/line.end_arrow=true. No bus/ticks/boundary/takeaway/inventory.',
    archetype_contracts: [
      {
        ...
        exact_content_slots: {
          status_zone: {
            ...
            horizontal_connector_bus_allowed: false,
            connector_text_clearance_in_min: 0.12,
            connector_zone_rule: 'connectors stay inside declared status/proof zone safe bounds',
            connector_card_center_tolerance_in: 0.22,
            connector_alignment_rule: 'hub>=10.4in centered; spans card centers; exactly one vertical arrow per card; no horizontal bus',
            connector_direction_rule: 'card arrows use kind=connector plus tailEnd=triangle or line.end_arrow=true',
          },
          proof_zone: {
...
export function createNativePptSampleAuthoringParts({
  ...
  function compactFix(fix: JsonRecord): JsonRecord {
    return Object.fromEntries(Object.entries({
      ...
      text_char_count: compactNumber(fix?.text_char_count || fix?.current_text_char_count),
      required_text_char_count: compactNumber(fix?.required_text_char_count || fix?.threshold),
