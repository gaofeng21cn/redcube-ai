...
function buildOplFamilyRouteSurfaces() {
  return [
    {
...
function buildReviewPublicationRefs({ reviewState, publicationProjection }) {
  return {
    review_state_ref: {
...
function buildOplFamilyAdoption({
  ...
  return {
    ...
    adoption_command: PRODUCT_ENTRY_SESSION_COMMAND_TEMPLATE,
...
export function buildOplFamilyLifecycleAdapterSurface({
  ...
  return {
    ...
    discovery: {
      ...
      owner_split: buildOplFamilyOwnerSplit({ runtimeOwner }),
      route_surfaces: buildOplFamilyRouteSurfaces(),
      delivery_identity: {
...
export function buildOplRuntimeManagerRegistration({
  ...
  return {
    ...
    consumable_projection_refs: ['/skill_catalog/skills/0/domain_projection/runtime_continuity', '/product_entry_shell/opl_bridge', '/session_continuity', '/artifact_inventory', '/runtime_inventory', '/review_state', '/publication_projection', '/opl_family_lifecycle_adapter'],
    state_index_inputs: {
