...
- `agent_lab_handoff.json` 与 `production_acceptance/rca-goal-workflow-agent-lab-suite.json`：最小 `/goal` AgentLab / OMA handoff surface。前者让 OMA `agent:evidence` / `improve:external-suite` 读取 RCA 标准 target-agent handoff；后者是 OPL `agent-lab run --suite ... --json` 可直接消费的顶层 external suite。两者都只投递 refs、receipt shape、typed blocker 和 no-forbidden-write proof，不携带 visual truth、artifact body、memory body、review/export verdict 或 owner receipt body。
