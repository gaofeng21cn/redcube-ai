...
对 RCA visual deliverable、evidence packet、slide/report asset 或 product delivery 任务，真实进度按可审阅视觉产物、artifact delta、owner decision、human gate 或用户可用结果计算；runtime、queue、stage、read model、测试、docs、provider liveness 和平台修复只是支持证据。RCA/OPL 执行基座是首选路径，不是视觉交付的前置条件：基座顺畅时走基座，基座卡住时，前端执行者继续推进合法的视觉交付增量，并把暴露出的基座问题作为 side repair lane 记录或修复。只有继续动作会越权写 deliverable/owner authority surface、缺少必要 source/data、或同一写集 ownership 冲突且无法隔离时，才暂停具体目标动作。repair lane 不能吞掉交付主线，除非用户目标本身就是平台修复。
