...
- The current repo-verified public entry surfaces are the single `redcube-ai` app skill, `CLI`, and `MCP`; `controller` remains an internal control-plane label. Under strict OPL Agent purity, `invokeDomainEntry`, `invokeProductEntry`, local scripts, product/session/status wrappers, and repo-tracked contracts must shrink to visual handler targets, authority refs, or machine-readable contracts as OPL generated/default callers replace repo-local shells. OPL/Temporal hosted scheduling is the default runtime posture after task start, `Codex CLI` remains the default concrete stage executor, and non-default executor / proof adapters remain explicit opt-in lanes.
