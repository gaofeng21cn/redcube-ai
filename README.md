...
- The current active mainline is `redcube-runtime-program`: `P0 review-closeout` is passed with a credible clean-clone baseline; phase-2 source-truth, governance, operator-surface, and runtime-watch hardening remain absorbed provenance; and the current active tranche is now `Hermes / stable family closure truth`, which keeps Hermes frozen as the runtime substrate owner, preserves the canonical `ppt_deck` closure, tightens stable-family runtime execution truth plus routed artifact envelopes into repo truth, and lands `xiaohongshu` as a second Hermes-backed family closure without rewriting its human-publication semantics into direct delivery.
