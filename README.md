...
- When an external agent or OPL wants the repo-tracked skill surface directly, use the single `redcube-ai` app skill; `frontdesk` / `invoke` / `session` remain machine-readable command contracts underneath that one skill.
