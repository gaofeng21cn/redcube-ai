...
- The hosted quality lane runs `npm run typecheck` once to build compiled package exports before typechecking, then runs `node --experimental-strip-types scripts/run-test-group.ts fast`, `family`, and `meta`; family shared pin checks must stay clean-clone safe through `scripts/run-test-group-lib.ts`.
