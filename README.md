...
- The first public surface is the single `redcube-ai` app skill; `frontdesk` / `invoke` / `session` stay as machine-readable command contracts underneath that skill. In that contract, `frontdesk` means the agent-facing product-entry overview / intake / entry shell, not a landed GUI, WebUI, or end-user front office.
...
- When an external agent or OPL wants the repo-tracked skill surface directly, use the single `redcube-ai` app skill and launch CLI-backed commands through `npm run --prefix <redcube-ai-repo> redcube -- ...`; `frontdesk` / `invoke` / `session` remain machine-readable command contracts underneath that one skill. The legacy `redcube product frontdesk` command key is retained for compatibility as the product-entry overview / intake shell; it does not imply a mature human-facing GUI or WebUI. The OPL federated bridge stays an internal integration surface.
- The hosted quality lane runs `npm run typecheck` once to build compiled package exports before typechecking, then runs `node --experimental-strip-types scripts/run-test-group.ts fast`, `family`, and `meta:ci`; `meta:ci` is the meta remainder after fast coverage, so hosted CI does not repeat the same root test files. Browser / renderer proof setup stays in the explicit `native-ppt-proof` and `image-ppt-proof` jobs, not the default quality lane. Family shared pin checks must stay clean-clone safe through `scripts/run-test-group-lib.ts`.
- Local `npm run test:integration`, `npm run test:e2e`, `npm run test:full`, and `npm run test:full:remaining` still keep the Codex/Python preflight, but only explicit route-heavy files stay serialized; the remaining files use the Node test runner's default concurrency. After a local fast pass, `npm run test:integration:remaining` runs only the integration files not already covered by fast. After local fast + family + meta:ci + integration:remaining passes, `npm run test:full:remaining` runs the computed full-suite remainder without repeating already covered meta/integration files.
