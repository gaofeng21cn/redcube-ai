...
- Treat the implementation surface as TypeScript orchestration plus Python native helpers. JavaScript files visible in the tree are legacy allowlisted residue under migration policy, not the default pattern for new agent work.
...
- The hosted quality lane runs `npm run typecheck` once to build compiled package exports before typechecking, then runs `node scripts/run-test-group.mjs fast`, `family`, and `meta`; family shared pin checks must stay clean-clone safe through `scripts/run-test-group-lib.mjs`.
