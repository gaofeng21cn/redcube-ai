...
- 对外第一入口是单一 `redcube-ai` app skill；`frontdesk` / `invoke` / `session` 继续作为这个 skill 底下的 machine-readable command contracts。
...
- Agent 应把实现面理解为 TypeScript orchestration 加 Python native helpers。仓内可见的 JavaScript 文件只是迁移策略下的 legacy allowlisted residue，不是新增 agent 工作的默认范式。
- 如果外部 agent 或 OPL 需要直接读取 repo-tracked skill surface，使用单一 `redcube-ai` app skill 即可；`frontdesk` / `invoke` / `session` 继续作为这个 skill 底下的 machine-readable command contracts，而 OPL federated bridge 仍然只是内部集成面。
- hosted quality lane 固定先跑一次 `npm run typecheck`，生成 compiled package exports 后再做类型检查，然后直接跑 `node scripts/run-test-group.mjs fast`、`family` 和 `meta`；family shared pin 检查必须继续通过 `scripts/run-test-group-lib.mjs` 保持 clean-clone 可运行。
