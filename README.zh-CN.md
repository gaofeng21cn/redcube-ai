...
<p align="center"><strong>面向正式视觉交付的技能优先领域智能体，用来把资料、审阅和导出文件组织到同一条交付线上</strong></p>
...
- `RedCube AI` 是独立的视觉交付领域智能体。它对外第一身份是视觉交付：接收材料、分阶段完成视觉创作、审阅、回修、导出和文件交付。
...
- `OPL` 是 Codex-first、stage-led 的完整智能体运行框架，可以把 RedCube 作为外部领域智能体托管；这条路径是内部集成 / 托管运行路径，不是 RedCube 的对外第一身份。
- 当 OPL 托管 RedCube 时，除非显式选择 hosted/proof 后端，`Codex CLI` 仍是最小具体执行单元；直达路径和 OPL 托管路径都必须收敛到同一个下游 RedCube 领域智能体入口（`invokeDomainEntry` service-safe surface）。
...
- 如果外部智能体或 OPL 需要直接读取仓库跟踪的技能面，使用单一 `redcube-ai` 应用技能，并通过 `npm run --prefix <redcube-ai-repo> redcube -- ...` 启动 CLI 命令；`status` / `invoke` / `session` 继续作为这个技能下面的机器可读命令合同。`redcube product status` 命令键继续保留为兼容入口，语义是产品入口概览 / 材料接收壳，不代表成熟的人用 GUI 或 WebUI；OPL 托管路径仍然只是内部集成面。
