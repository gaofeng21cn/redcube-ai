...
- hosted quality lane 固定先跑一次 `npm run typecheck`，生成 compiled package exports 后再做类型检查，然后直接跑 `node --experimental-strip-types scripts/run-test-group.ts fast`、`family` 和 `meta:ci`；`meta:ci` 是扣除 fast 已覆盖文件后的 meta remainder，避免 hosted CI 重复跑同一批根级测试文件。family shared pin 检查必须继续通过 `scripts/run-test-group-lib.ts` 保持 clean-clone 可运行。
