...
- For `ppt_deck`, generic PPT/slides wording and `.pptx` input or output describe the deliverable/container only. Native editable authoring requires decisive-Attempt semantic judgment over the complete current user request and explicit evidence for editable PowerPoint objects or the native authoring route; examples are not a trigger vocabulary, and keyword/regex/extension matching is forbidden. Reference decks, templates, notes, PDF export, quality wording, Agent preference, or validator behavior cannot promote the default image-first route.
- Mentioning or `@`-ing OMA beside RCA in a visual-delivery request does not authorize Agent engineering. Artifact QA can mutate only the current deliverable lineage; it cannot patch RCA source, skills, contracts, validation mappings, or invoke an OMA improve run without a separate explicit Agent-engineering request.
...
- A native helper, OfficeCLI/PPT Master workflow, validator pass/fail result, or template file extension cannot become route authority or reopen a quality cycle after the three-round repair budget is exhausted.
