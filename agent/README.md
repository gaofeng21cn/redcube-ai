...
`contracts/stage_control_plane.json` 是这个 pack 的 machine-readable Cognitive Kernel binding，也是 RCA direct product-entry / Codex executor 的 canonical visual route surface。每个 visual stage 声明 prompt、stage skill policy、tool affordance、knowledge、strategy、candidate-pool、independent-gate、quality-gate 和 handoff refs。`agent/stages/manifest.json` 只给 OPL standard compiler 列出同一 stage 的最小 repo/action/transition refs，不能复制这些 route/professional-skill body 或成为第二份 stage control plane。这些 refs 只指导 Codex executor 在 stage 内工作；RCA-owned review/export gates、owner receipts、typed blockers 和 artifact authority 仍决定是否推进。
