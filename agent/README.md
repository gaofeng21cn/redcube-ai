...
`agent/stages/manifest.json` 是这个 pack 唯一的物理 stage 输入：它声明 stage、repo refs、action refs、transition refs、trust lane 与 authority boundary。OPL 由该输入生成 `opl-generated:family_stage_control_plane`；RCA 不跟踪、加载或复制第二份 stage plane。阶段的专业策略继续由 `agent/prompts/`、`agent/professional_skills/`、`agent/quality_gates/` 和 direct domain handler 持有；这些 refs 只指导 Codex executor 在 stage 内工作，RCA-owned review/export gates、owner receipts、typed blockers 和 artifact authority 仍决定是否推进。
