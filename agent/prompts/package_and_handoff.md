...
- Bind package transport identity and separate release-integrity evidence to the exact Meta Review successor bytes and hashes. Review final exported-file render equivalence, publication-copy fidelity to accepted source, presenter-notes consistency with final visible content, manifest/file/hash closure, and operator handoff usability against the declared export and package dependency scopes; exact hashes are not content authority.
- Keep package Review delta-scoped. A package wrapper, export conversion, render, or layout-only delta does not stale accepted content or reference Review. Reopen an upstream judgment only when semantic evidence reaches that scope through its declared dependency closure.
...
- Producer and repairer return artifact identity and, for repairer, the complete `repair_map`; neither returns `route_impact.stage_quality_cycle.outcome` or a Stage route decision. A reviewer or re-reviewer returns `route_impact.stage_route_decision` for `pass`, `quality_debt`, final-budget `repair_required` with consumable bytes, or a pre-exhaustion `repair_required` whose narrowest owning Stage is a different declared Stage. Under `same_stage_repair_required`, a package-local defect with repair budget remaining returns only a recommendation so the controller creates this Stage's repairer. Under `cross_stage_route_back_before_budget_exhaustion`, an upstream-owned defect may instead terminate this StageRun with `decision_kind=route_back`, a `target_stage_id` different from `package_and_handoff`, and non-empty evidence refs; this is the only terminal route allowed for `repair_required` before budget exhaustion. A materialized no-output diagnostic remains consumable and may support that owner route. Hard gates and literal zero-consumable-artifact attempts, where neither candidate nor diagnostic exists, return no route decision or recommendation.
- A terminal reviewer or re-reviewer supplies the decisive dependency-scoped quality outcome over the exact located bytes and the Stage route decision; it does not create a receipt-level verdict, sign an owner receipt, or set a ready flag. At final budget, `repair_required` remains the Review outcome and the controller closes the consumable candidate as `completed_with_quality_debt`. The controller verifies fresh reviewer/producer session identities, maps the outcome to the formal Review receipt verdict, and binds transport identity to the exact candidate receipt, refs, bytes, sizes, and hashes. RCA's owner-receipt authority function then validates the formal receipt plus the separate release-integrity evidence and signs the domain owner receipt used by export-ready, publication-ready, handoff-ready, or owner-accepted projections. Re-review must additionally close all prior required findings against the repair map; any repair regression or critical new finding that reopens repair becomes a required finding with a collision-free `finding_id`. Optional observations do not reopen repair and are compatible with `pass`.
