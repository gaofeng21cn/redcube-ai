...
- For `author_image_pages` and `repair_image_pages`, execute `contracts/runtime-program/ppt-image-first-production-route.json#/api_contract`: discover the `imagegen` skill and callable `image_gen.imagegen` / `image_gen__imagegen` from the active tool inventory, and never treat skill presence alone as tool availability.
- The built-in image tool does not require `OPENAI_API_KEY`. If it is absent or unavailable before any image output, automatically parse the active Codex provider from `$CODEX_HOME/config.toml` or `~/.codex/config.toml` and call `gpt-image-2` through that provider's `base_url` and resolved credential. Reuse the installed `imagegen` CLI and use ephemeral `uv run --with openai` when its SDK dependency is absent. The selected image-first route is sufficient authorization; do not ask for fallback confirmation, dependency installation, or a separately configured OpenAI key, and never persist or disclose the resolved token.
