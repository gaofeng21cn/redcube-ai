...
- `scripts/install-codex-plugin.ts` 生成的 `.agents/plugins/marketplace.json` 继续是本地安装/刷新副产物，不进入 tracked repo source；生成的 marketplace `source.path` 指向 `.`，以表达当前 active source root 是整个 developer checkout。
