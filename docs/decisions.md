...
- 后续不得把旧 repo-local deliverable runner、run store、DAG scheduler、supervision diagnostic 或 public lookup action 恢复成兼容别名、internal fixture、standard sidecar template 或 active docs truth。需要追溯时只读 `docs/history/**`、旧 commit 或 tombstone/provenance。
...
- RCA 继续保留 direct `runtimeWatch` 作为 review / progress refs-only read model，不把它写成 RCA sidecar 默认 action、generic supervisor、session runtime 或兼容别名。
