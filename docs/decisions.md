...
`Codex CLI` 是 RCA 仓内唯一物化的 concrete stage executor。其他 executor 的选择、运行、重试和 receipt body 归 OPL hosted surface；RCA 只消费 opaque receipt / requirement refs，不保留 local adapter、routing config、profile、fallback 或兼容 fail boundary。
