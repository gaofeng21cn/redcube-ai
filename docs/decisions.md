...
- 当前真实主线回到核心五件套、active gap plan、runtime architecture 和机器合同：Temporal-backed OPL provider 是生产在线 runtime substrate，`Codex CLI` 是默认 stage executor，`hermes_agent` 只作为显式非默认 executor adapter / proof backend 保留。后续不得把历史 upstream Hermes pilot、旧 cutover board 或 repo-local Hermes 迁移材料恢复为默认 runtime owner、current implementation checklist、readiness proof 或 compatibility surface。
