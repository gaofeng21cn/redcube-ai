...
`contracts/action_catalog.json` 是 action 的唯一正文，`agent/stages/manifest.json` 是 RCA 唯一的物理 declarative stage input。它只列出六个 stage、已有 repo refs、action refs、next-stage refs、trust lane 与 authority boundary；OPL 从固定路径 fail-closed 地生成 `opl-generated:family_stage_control_plane`。RCA 不复制或加载本地 stage plane；visual route、professional skill、quality gate 和 artifact authority 的领域正文分别留在 `agent/` 的策略 refs 与 direct domain handler。RCA product-entry manifest、domain-handler export 和 stage plan 只返回 manifest ref 与 generated locator。
...
OPL Stage Folder writer 是物理合同 writer，不是 RCA owner receipt signer。`success` 必须有 required output roles、valid role manifest、RCA owner receipt refs 和 receipt file；`blocked` 必须有 RCA typed blocker refs 和 evidence file。Output 文件、stage folder 存在、provider completion、generated surface ready、queue empty、attempt ledger written、stage-run terminal state、controlled canary、mock-safe canary、conformance pass 或 read-model current 都不能替代 RCA owner answer。
