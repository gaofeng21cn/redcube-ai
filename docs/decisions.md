...
### Prompt 分层保留专业依赖，不固定模型认知配方

Stage prompt 只说明阶段目标、好结果、authority/证据边界与交付形状；真实 route prompt 承载当前任务方法；professional skill 通过已校验 locator 和 skill-owned concise `Runtime Summary` 进入 effective prompt，代码不复制第二份 remit；attached output contract、validator、sample fixture 与 fresh feedback 分别持有机器形状、样例几何和精确修复要求。Markdown `runtime_seed/runtime_artifact` 及其无 caller parser 已退役。Source/story basis 必须先于 materialization，最终 artifact 必须经过 fresh rendered review；blueprint 与 visual direction、director review 与 screenshot mechanics 可在这些依赖内迭代或并行，不冻结 executor 的认知顺序。
...
质量预算统一为初稿之后最多三轮 `repairer + fresh re_reviewer`。Provider retry、协议 closeout resume 和同 thread 的 `in_thread_refinement` 均不消耗该预算，也不能产生 Review receipt。
...
`ppt_deck` 的专业 route 顺序仍是 `storyline -> detailed_outline -> slide_blueprint -> visual_direction -> author_image_pages -> visual_director_review -> screenshot_review -> repair_image_pages -> export_pptx`，但 route 不是 OPL Stage，也不能单凭 route 名推断整个 Attempt 的职责。Review-only route 只供 reviewer/re-reviewer，专用 repair route 只供 repairer，其他生产与打包 route 可由 producer 或 repairer 执行；这允许 repairer 在同一 Stage 目标内重做 storyline、visual direction、artifact 或 package bytes，而不会取得终局路由权。`storyline / detailed_outline / slide_blueprint` 属于 `communication_strategy`；页面生成、formal reviewer Attempt、repairer Attempt 与 fresh re-reviewer Attempt 共同构成 `artifact_creation` 的质量循环。visual-director 与 screenshot helper 是 reviewer/re-reviewer Attempt 内的专业 QA 操作，只产出 `in_attempt_visual_qa` 证据，不增加 Attempt、不签 Review receipt；`review_and_revision` 单独承担 deck-wide Meta Review。

Attempt 的终局 Stage route judgment 与 domain owner receipt 是两条不同权限轴。Terminal reviewer/re-reviewer 可以返回 `route_impact.stage_route_decision`，但 route helper 不得据此生成 owner receipt 或 `export_ready`。最终 package 先由 OPL StageRunController 从独立 Attempt 记录物化 formal Review receipt，再由 RCA `emit_domain_owner_receipt` authority surface 校验 domain refs 并签 owner receipt；ready projection 只能消费这两个 receipt 与 exact artifact identity binding 的组合结果。
...
RCA 的 6 个 top-level Stage 是唯一 OPL 调度真相：`source_intake` 冻结 source truth 与缺口，`communication_strategy` 决定叙事 / 大纲 / 页面角色，`visual_direction` 决定视觉语言 / 节奏 / 密度，`artifact_creation` 在已选 route 内完成候选、独立 Review、repair 和 fresh re-review，`review_and_revision` 对整个交付物做独立 Meta Review 并决定 defect owner/route-back，`package_and_handoff` 在 Meta Review 后导出、打包并对新生成或转换的最终 bytes 做独立 package-scoped quality loop。Package producer/repairer 只返回候选与 route recommendation；terminal reviewer/re-reviewer 在 exact hash 与 fresh-session 绑定成立后返回 `route_impact.stage_quality_cycle.outcome` 和 Stage route decision，由 OPL StageRunController 把 outcome 映射为 formal Review receipt verdict 并推进 Stage。Ready claim 仍需 RCA owner-receipt authority function 单独签收。
