...
- `emit_external_work_order_owner_closeout` 是 RCA-owned product domain_action_adapter guarded action，供 OPL 在外部 work order patch 吸收后调用。
...
- `runtime_watch` 已从 RCA product domain_action_adapter guarded action / default dispatch 面退役；需要 watch / progress 时由 OPL status / workbench / runtime read model 消费 RCA direct `runtimeWatch` read surface。
...
- 旧 managed supervision action 与 `product_entry_continuation` 已从 RCA product domain_action_adapter 默认 dispatch / guarded action 面物理删除或收薄；generic supervision 和 product-entry continuation 归 OPL runner / generated session shell。
...
- RCA 在 runtime-program contracts、manifest、status 和 product domain_action_adapter projection 中维护 `privatized_functional_module_audit`，统一盘点 product-entry session snapshot refs adapter、workspace/source intake、memory/writeback receipt transport、artifact export lifecycle、review/repair transport、native helper envelope、operator projection shell、generic CLI/MCP wrappers、Codex executor adapter、observability/stability read model、visual pack compiler handoff 和 minimal visual authority functions。
...
### 决策：product domain_action_adapter / action / status parity 由 canonical metadata 驱动

- Product domain_action_adapter guarded actions、forbidden writes、manifest domain_action_adapter、family action catalog、CLI help 和 MCP product-entry routes 统一从 RCA-owned canonical action/status metadata 派生。
...
- RCA 在 manifest、product domain_action_adapter 和 runtime-program contracts 中新增 `opl_substrate_adapter_export`，作为 RCA domain-owned OPL substrate adapter/export surface。
...
- RCA 在 runtime-program contracts、manifest 和 product domain_action_adapter projection 中新增 `opl_stability_read_model_consumption`，只引用 OPL `family-conflict-envelope`、`control_loop_summary`、`usage_projection`、`resource_pressure`、`runtime observability-export` 和 external stability policy 的 refs-only read model。
...
- RCA 在 manifest、status、session 和 product domain_action_adapter 中新增 `workspace_receipt_inventory_projection`，从 workspace runtime receipt root 读取 domain owner、visual pattern memory accepted/rejected 与 lifecycle cleanup/restore/retention receipt refs。
...
- RCA 在 manifest、product domain_action_adapter、runtime-program contracts 和 focused tests 中新增 `visual_transition_evaluator` 与 `evaluate_visual_transition` guarded action。
...
- `emit_domain_owner_receipt`、`apply_visual_memory_writeback`、`apply_visual_workspace_lifecycle` 已作为 product domain_action_adapter guarded actions 落地。
...
- `RedCube AI` 在 product-entry manifest 和 product domain_action_adapter projection 中新增 `domain_owner_receipt_contract`、`lifecycle_guarded_apply_proof`、`physical_skeleton_follow_through` 与 `review_helper_baseline_follow_through`，并把 `controlled_memory_apply_proof` 扩展到 accepted/rejected runtime receipt refs。
...
- `RedCube AI` 在 product-entry manifest 和 product domain_action_adapter projection 中新增 `controlled_soak_no_regression_attempt`。
...
- 公开发布包装固定为 `RedCube AI Foundry Agent / OPL-compatible package built on OPL Framework`：single `redcube-ai` app skill、service-safe domain entry、product domain_action_adapter/projection、stage control projection 和 standard domain-agent skeleton mapping 是同一 package 的不同 surface。
...
- `RedCube AI` 的 OPL 长期托管口径更新为 `OPL Product Entry -> OPL stage-led family runtime provider -> RCA product domain_action_adapter export/dispatch -> RedCube service-safe domain entry`。
...
- `redcube product domain_action_adapter export|dispatch` 继续是 OPL provider 到 RCA owner surface 的受控桥接。OPL/Temporal/Hermes/local provider 只能 enqueue、dispatch、signal、query、投影 attempt/receipt，不得写 visual truth、review verdict、publication projection truth、canonical artifacts 或 export authority。
...
- RCA 的 runtime owner 不再是 concrete executor；默认 runtime owner 是 OPL provider。`Codex CLI` 仍是第一公民 concrete executor 选项；显式非默认 executor 通过 OPL generic Agent Executor Adapter 进入，RCA 只消费 OPL executor receipt / product domain_action_adapter receipt refs。
...
- 当前状态：除真实 production-hosted controlled visual stage soak 外，本边界已落地到 status/runtime architecture/product domain_action_adapter/receipt proof 口径；旧 Hermes/Gateway/local-manager active path 已降为 explicit proof/provenance/history。
...
- 若未来启用 SQLite，它只能作为可删除、可重建的 domain_action_adapter index，存储 session/deliverable/route/artifact/review/export 索引与 hash/provenance；不得存放 PNG/PPTX/PDF blob，不得成为 visual-domain truth、canonical artifact truth 或 review/export judgment owner。
...
- RCA product domain_action_adapter adapter 只作为 OPL typed family queue / OPL family runtime provider wakeup 的受控投影与 dispatch 面启用；它不成为 OPL 自有 visual truth domain_action_adapter，也不持有 review verdict、publication gate 或 canonical artifact authority。
