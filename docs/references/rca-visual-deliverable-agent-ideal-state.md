...
执行顺序上，RCA 侧的功能面归位已经完成到 consumer projection 和 thin evaluator：source/workspace intake、artifact gallery/handoff、review/repair transport、native-helper generic envelope、generic transition runner、route/decision graph、observability/SLO、App drilldown、stability read-model 和 scaffold/template checklist 的 owner 都是 OPL / App；RCA adapter 保持薄，只持有 visual transition spec/table、guard evaluator、typed blocker、owner receipt refs 和 visual authority。真实 OPL-hosted visual stage、Temporal long soak、跨 family 重复 proof 和 production closure 是后置运行验收，不应再被写成 RCA 侧 generic primitive 或 generic observability 功能缺口。
