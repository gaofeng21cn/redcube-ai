...
因此，RCA 的理想形态是视觉交付 `Domain Knowledge / Authority Pack + thin program surface`。RCA 不维护独立 agent runtime platform，也不长期维护 generic scheduler、generic queue、generic attempt ledger、generic state-machine runner、generic workspace/source intake shell、generic artifact gallery、generic memory transport、generic review/repair transport、native-helper generic envelope、generic observability 或通用 App/workbench runtime。RCA 的 descriptor、contract/schema、product sidecar、service-safe domain entry、projection builder、domain transition spec/table、review/export gate、artifact locator、receipt schema、tests、lifecycle adapter 和 native helper implementation 只把 visual stage pack、route truth、review/export verdict、artifact refs、typed blocker 和 owner receipt 暴露给 OPL。这些薄程序面服务 OPL 发现、托管、审计和投影，不构成第二套通用平台。
...
执行顺序上，RCA 侧的功能面已经先归位到 consumer projection、thin evaluator 和 private functional audit surface，但代码里仍保留若干活跃私有实现：managed DAG scheduler、attempt/state-machine runner、managed-run JSON store、product-entry session store、workspace/source intake、memory/writeback receipt transport、artifact export lifecycle、review/repair transport、native helper envelope、operator projection shell、generic CLI/MCP wrappers、Codex executor adapter 和 observability/stability read-model consumer projection。理想 owner 是 OPL / App 上收 generic scheduler、runner、attempt ledger、session shell、workspace/source shell、artifact lifecycle/handoff shell、review-repair transport、native-helper envelope、executor adapter、observability/SLO 和 workbench；RCA adapter 保留 visual transition spec/table、source readiness verdict、visual memory body、review/export verdict、artifact authority、native helper implementation、route-level executor policy、typed blocker、owner receipt refs 和 visual authority。真实 OPL-hosted visual stage、Temporal long soak、跨 family 重复 proof 和 production closure 是后置运行验收，不应被写成 RCA 侧 generic primitive 或 generic observability 完成。
