...
RCA 可以为了清洁目标态重构 packages、runtime families、prompts、native helper callsite 和 product shell。保留项必须回答它是否直接作出 source readiness、visual direction、review/export verdict、artifact mutation、visual memory accept/reject 或 owner receipt signing；否则默认不能作为 RCA 长期私有功能。2026-05-17 当前已知非知识功能面已经由 `privatized_functional_module_audit` 分类为 OPL hosted surface、OPL generated surface、refs-only adapter、declarative pack 或 minimal authority function；分类不是完成态。凡仍需 generated/hosted production consumption、active caller migration、wrapper retirement、operator drilldown、workspace/source/lifecycle shell 或 physical cleanup 的功能面，都继续构成功能/结构差距。
...
2026-05-16 follow-through：RCA 已扩展 `opl_generic_primitive_consumption`，并在 `contracts/runtime-program/current-program.json`、`contracts/runtime-program/opl-family-contract-adoption.json`、product-entry manifest、product sidecar export、managed DAG guard 和 retired-surface guard 中同步声明：standard domain-agent scaffold、functional harness、generic runtime、generic scheduler/daemon/queue/stage attempt/attempt ledger/typed closeout/runner/workbench、memory transport、memory refs-only writeback chain、artifact lifecycle、review/repair transport、generic transition runner、restart/dead-letter/repair/human gate 状态链和 native-helper generic envelope 都由 OPL 持有。RCA 是这些 generic primitives 的消费方，不是 owner；OPL harness pass 也不等价于 RCA visual ready、exportable、handoffable 或 artifact-producing owner receipt。
...
2026-05-16 operator evidence readiness follow-through：RCA 已新增 `operator_evidence_readiness_projection`，从既有 no-regression / owner receipt proof、domain owner receipt contract、controlled memory runtime receipt refs、lifecycle guarded apply proof、controlled soak blocker、workspace receipt inventory、OPL generic primitive consumer coverage 与 stability read-model consumer refs 派生 OPL/App/operator 可读 next-gap surface。它只说明真实 artifact-producing owner receipt、OPL-hosted controlled visual-stage long soak、真实 reusable visual memory body 与跨 family repeated no-regression evidence 仍是后续证据缺口；不写 visual truth、artifact blob 或 memory body，不声明 production soak complete，也不实现 OPL generic runtime、workbench 或 observability。

2026-05-16 workspace receipt inventory follow-through：RCA 已新增 `workspace_receipt_inventory_projection`，把 workspace runtime receipt root 下的 domain owner、accepted/rejected memory 和 cleanup/restore/retention lifecycle receipt refs 投到 manifest/status/session/sidecar。它是 RCA-owned refs-only read model，用来给 OPL/App/operator 看当前 workspace receipt coverage；它不写 receipt instance、不读取 memory body、不生成 OPL artifact gallery/handoff shell/workbench，也不构成 production visual-stage soak。

2026-05-16 visual transition evaluator follow-through：RCA 已新增 `visual_transition_evaluator` 和 `evaluate_visual_transition` sidecar guarded action。它只把 RCA-owned `visual_transition_spec` 的 guard refs 评估为 next stage、owner action、repair action、receipt/no-regression refs 或 typed blocker，并输出 transition bridge evidence refs；OPL 继续持有 generic transition runner、matrix runner、retry/dead-letter、route graph/workbench、provider attempt ledger 和 App projection。该 evaluator 不写 visual truth、artifact blob、memory body 或 review/export verdict，也不声明 visual-ready、exportable、handoffable 或 production soak。
...
执行顺序上，RCA 侧的功能面已经先归位到 consumer projection、thin evaluator 和 private functional audit surface，但代码里仍保留若干活跃私有实现：managed DAG scheduler、attempt/state-machine runner、managed-run JSON store、product-entry session store、workspace/source intake、memory/writeback receipt transport、artifact export lifecycle、review/repair transport、native helper envelope、operator projection shell、generic CLI/MCP wrappers、Codex executor adapter 和 observability/stability read-model consumer projection。理想 owner 是 OPL / App 上收 generic scheduler、runner、attempt ledger、session shell、workspace/source shell、artifact lifecycle/handoff shell、review-repair transport、native-helper generic envelope、executor adapter、observability/SLO、workbench 和 generated wrapper owner；RCA adapter 保留 visual transition spec/table、source readiness verdict、visual memory body、review/export verdict、artifact authority、native helper implementation、route-level executor policy、typed blocker、owner receipt refs 和 visual authority。真实 OPL-hosted visual stage 既包含 hosted path 接通与 caller migration 的功能/结构面，也包含 artifact-producing receipt 的证据面；Temporal long soak、cross-family repeated proof 和 production closure 是后置运行验收，不应被写成 RCA 侧 generic primitive 或 generic observability 完成。
