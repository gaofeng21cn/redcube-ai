...
- `当前功能面`：2026-05-16 已把 RCA 侧 OPL consumer follow-through 落到合同、manifest、sidecar projection 和 guard；OPL stability read-model 也已作为 refs-only consumer projection 挂入 RCA surface。后续不再把 generic primitive handoff、functional harness coverage 或 observability/control read-model 当成 RCA 功能缺口。
