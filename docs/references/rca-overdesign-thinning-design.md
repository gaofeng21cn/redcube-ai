# RCA 过度设计收薄设计（2026-07-10 历史结构子集）

Owner: `RedCube AI`
Purpose: `historical_overdesign_thinning_reference`
State: `historical_reference`
Machine boundary: 本文只保留 2026-07-10 八项结构子集的范围、写集和验收语境。2026-07-11 全仓 17 项审计的当前完成度、main 吸收和远端读回以 active plan、源码、contracts、runtime/readback 和对应 owner surface 为准。

## 目标

在不削弱 RCA visual-domain authority、route behavior、review/export gate、native helper implementation 和 OPL hosted boundary 的前提下，本文记录 2026-07-10 ponytail audit 的八项结构候选。它们只是 2026-07-11 全仓 17 项审计的历史子集，不能单独证明本轮审计完成：
...
| `retirement-guard` | 压缩 functional/morphology/readback/test 自证体系 | `contracts/{functional_privatization_audit,physical_source_morphology_policy}.json`、`scripts/check-private-platform-retirement.ts`、对应 projection builders/tests/docs | private-platform no-resurrection readback、contract/source morphology tests、typecheck |
...
- 当时的八项候选需逐项为 `done`，不得用 narrower lane summary 替代完整清单；这不是当前 17 项审计的完成门槛。
...
| 规划项 | 状态 | 完成度 | 当前证据 |
| --- | --- | ---: | --- |
| Node 22 stdlib walker | `done` | 100% | performance artifact walker 已改用 Node 原生递归能力，相关 fast/full tests 继续覆盖隐藏文件与 artifact 聚合。 |
| 单一 family registry | `done` | 100% | default overlay 与 runtime-family registry 由同一 canonical family table 派生。 |
| protocol validator utility 复用 | `done` | 100% | source augmentation validator 的 JSON 基础操作已归 protocol utility，未新增跨包 facade。 |
| `getRun` / `redcube runs get` 退役 | `done` | 100% | public export、CLI、测试和文档入口已删除；无 run 的 projection 继续走当前 OPL refs-only surface。 |
| 单消费者 Parts factory 收薄 | `done` | 100% | PPT、XHS、poster screenshot-review 及相邻单消费者 plumbing 已扁平化，保留确有注入/测试价值的边界。 |
| overlay value-first | `done` | 100% | overlay canonical value、派生公开类型与 trust-boundary validation 已收口，公开类型回归仍在。 |
| RCA-owned Hermes execution 退役 | `done` | 100% | routing schema/config/runtime、Hermes adapter/proof、`fix_html` 第二 executor pass 已删除；非 Codex 输入 fail closed，OPL hosted executor 只保留 refs/owner boundary。 |
| private-platform retirement guard 收薄 | `done` | 100% | functional/morphology 合同、AST import closure 与 compact readback 已收口，未把 source guard clean 写成 physical delete authority。 |

本记录只说明八项历史结构候选曾完成其当时的验收，不能用来推导 2026-07-11 全仓 17 项审计、当前 `main` 吸收、远端同步或任何 readiness 结论。Native PPT 的 live 同源盲评、第三 viewer、owner receipt、visual ready、Temporal long soak 与 production ready 继续按 active plan 和 machine completion audit 的 typed blocker 读取。
