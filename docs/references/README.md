...
This layer is not the active baton. Active contract-linked program briefs stay in `../program/`, stable rules stay in `../policies/`, and no-longer-current provenance stays in `../history/`.
This layer is also not the public identity layer: reference documents may discuss OPL, gateway, harness, bridge, federation, or hosted runtime context, but those terms must be read as internal integration, target-state, or provenance language. The default public identity remains `RedCube AI` as a visual-deliverable domain agent.
