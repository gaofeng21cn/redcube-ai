...
RCA 当前已经通过 product-entry manifest 顶层 `domain_memory_descriptor` 暴露标准 `family-domain-memory-ref.v1` projection，并继续保留 RCA-owned `domain_memory_descriptor_locator`、migration plan、seed fixture locator、writeback proposal generator、accept/reject command、writeback receipt locator、operator receipt projection，以及 `controlled_visual_stage_attempt` proof descriptor。它们证明 direct RedCube skill 与 OPL-hosted path 可以回到同一 RCA-owned descriptor、sidecar 和 quality refs。2026-05-12 OPL fresh `domain-memory list --json` 已把 RCA 解析为 resolved descriptor。当前状态仍是 repo-source contract / descriptor landed，不是 provider-hosted controlled visual stage soak completed；真实 proof instance、accepted/rejected receipt instance、visual memory body 和 artifact output 仍属于 workspace/runtime roots。本轮 OPL+MAS 优先，RCA controlled visual stage soak 延后；延后期间不得退化 descriptor、sidecar、quality refs 或 direct route parity。
