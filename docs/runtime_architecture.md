...
其中 `frontdesk` 只表示 agent-facing product-entry overview / intake / entry-shell contract；legacy `redcube product frontdesk` 命令键继续保留为兼容命令，不表示 GUI、WebUI 或最终用户前台壳已经落地。
