...
- OPL 持有 stage-led 托管集成、provider-backed family runtime、generic executor selection、attempt ledger 与 receipt residency；Temporal 是 OPL production online runtime 的必需 substrate，local provider 只用于 dev/CI/offline diagnostics。OPL 可以索引 product-entry registration、session continuity、runtimeWatch、artifact、review/publication projection，但不持有 RedCube visual truth。
- RCA 只物化 `Codex CLI` executor；`Hermes-Agent` 等其他 runtime carrier 只按上游 external reference 或历史 provenance 读取，不构成 RCA active adapter/proof backend。
...
- 仓库目录治理现在通过 `scripts/repo-hygiene.sh` 在 `scripts/verify.sh` 各 lane 和 grouped test 执行前运行。Line budget 的唯一 authoritative gate 是 `scripts/line-budget.ts`；Sentrux structural quality 继续由 `scripts/run-structural-quality-gate.sh` 承担。两者默认是 advisory：日常 `smoke`、`fast`、`ci`、`structure`、`npm test`、`npm run test:smoke`、`npm run test:fast` 和 `npm run test:ci` 入口只报告 oversized / growth / stale baseline / structural regression，不因这些报告阻断普通开发；每日结构治理或显式维护使用 `scripts/verify.sh line-budget-strict`、`scripts/verify.sh structure-strict`、`npm run line-budget:strict` 或 `OPL_LINE_BUDGET_STRICT=1`。本地 build 使用 TypeScript 增量图，CI 通过 `typecheck:ci` 强制重建；`scripts/run-test-group.ts` 同时给 Python native helper 子进程注入仓外 cache 环境。Framework link 只由 OPL canonical package-link/bootstrap surface 管理，验证 wrapper 不再维护第二套 link gate。tracked 主线不得包含 `dist/`、`build/`、`out/`、`__pycache__`、`*.egg-info`、`.DS_Store`、项目级 `.codex/`、`.omx/`、`.runtime-program/`、`runtime-state/`、`.agent-contract-baseline.json` 或 `.agents/` 这类生成物 / 本地状态；RCA Codex plugin scaffold 的 canonical source 现保留在 `plugins/redcube-ai/`，legacy `plugins/rca/` 已退役且不得作为兼容 alias path、repo-local installer 或 tracked `.agents/plugins/marketplace.json` 注册恢复。
...
| `docs/references/` | Target-state、integration、product-entry、memory locator、OPL executor policy refs、governance 等 support references。 | 不承担 current truth 或 active baton。 |
