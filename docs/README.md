...
- OPL 持有 stage-led 托管集成、provider-backed family runtime 与 hosted executor selection；Temporal 是 OPL production online runtime 的必需 substrate，local provider 只用于 dev/CI/offline diagnostics。OPL 可以索引 product-entry registration、session continuity、runtimeWatch、artifact、review/publication projection，但不持有 RedCube visual truth。
- RCA 仓内 runtime 只物化 `Codex CLI`；其他 hosted executor 只通过 OPL-owned receipt / requirement refs 进入 handoff，不在 RCA 保留 config、descriptor、fallback 或执行分支。
