...
- 仓库目录治理现在通过 `scripts/repo-hygiene.sh` 在 `scripts/verify.sh` 各 lane 和 grouped test 执行前运行。Line budget 与 Sentrux structural quality 默认是 advisory：日常 `smoke`、`fast`、`ci`、`structure` 和 `npm run test:*` 入口只报告 oversized / growth / stale baseline / structural regression，不因这些报告阻断普通开发；每日结构治理或显式维护使用 `scripts/verify.sh line-budget-strict`、`scripts/verify.sh structure-strict`、`npm run line-budget:strict` 或 `OPL_LINE_BUDGET_STRICT=1`。`scripts/run-test-group.ts` 同时给 Python native helper 子进程注入仓外 cache 环境。tracked 主线不得包含 `dist/`、`build/`、`out/`、`__pycache__`、`*.egg-info`、`.DS_Store`、项目级 `.codex/`、`.omx/`、`.runtime-program/`、`runtime-state/`、`.agent-contract-baseline.json` 或 `.agents/` 这类生成物 / 本地状态；RCA Codex plugin scaffold 保留在 `plugins/rca/`，不再通过 repo-local installer 或 tracked `.agents/plugins/marketplace.json` 注册。
