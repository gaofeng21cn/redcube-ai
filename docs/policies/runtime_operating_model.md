...
- 当前产品 runtime owner 是 `Hermes-backed runtime substrate`
- `Codex-default host-agent runtime` 只保留为 transition deployment host / regression bridge / development shell
...
- 在当前 Codex-native 语境里，Hermes-backed runtime substrate 是当前正式默认产品形态
...
- `Hermes runtime substrate canonical closure` 现已在当前主线上落地：routed run session、event log、executor identity 与 shared `runtime_topology` 现在由 Hermes substrate 收口，同时 `Codex-default host-agent runtime` 仅作为 transition host 保留
