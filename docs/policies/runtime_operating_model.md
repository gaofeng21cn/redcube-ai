...
- 当前产品 runtime owner 是 `repo-local managed runtime pilot`
- `Codex` 本地 operator host 是当前 deployment host / regression bridge / development shell
- 上游 `Hermes-Agent` 继续只作为目标 substrate，不是当前 owner
...
- 在当前 Codex-native 语境里，repo-local managed runtime pilot 才是当前正式默认产品形态
...
- 历史 `Hermes stable family closure truth` 冻结件只保留为本地迁移 provenance；当前 repo-verified 基线仍是 stable family runtime output 暴露同一份 repo-local execution truth，并由 `Codex` 本地 operator host 承担当前部署宿主角色
