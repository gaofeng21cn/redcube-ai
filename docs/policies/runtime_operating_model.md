...
- `Hermes stable family closure truth` 现已在当前主线上落地：stable family runtime output 会直接暴露同一份 Hermes execution truth，routed artifact 也会统一保留 quartet envelope，同时 `Codex-default host-agent runtime` 仅作为 transition host 保留
