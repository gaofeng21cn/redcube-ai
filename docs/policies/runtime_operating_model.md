...
- `runtime watch locator integrity hardening` 之后的 prefrozen follow-on board 也已冻结：当前允许的 same-mainline continuation 固定为 `family parity / autopilot continuity`，其第一条 tranche 是 `phase_2_family_parity_governance_surface_convergence`，不得再把 `next_tranche_candidate` 诚实地写回 `null`
