...
- RCA live stage-run progress evidence 已落地为 OPL 标准消费入口 `contracts/live_stage_run_progress_evidence.json`，并由 `contracts/stage_run_kernel_profile.json` 指向；旧 `contracts/owner_chain_live_progress_evidence.json` 继续作为输入/引用。该 surface 使用 `surface_kind=domain_live_stage_run_progress_evidence` 与 `status=owner_typed_blocker_recorded_not_ready_claim`，并记录 RCA-owned `owner_receipt_refs`、`typed_blocker_refs`、`human_gate_refs`、`quality_or_export_receipt_refs`、`memory_lifecycle_refs`、`no_regression_refs`、`long_soak_refs`、`doc_refs` 与 `next_verification_command_refs`；RCA 私有来源标记保留在 `rca_surface_kind`。已有 mock-safe `ppt_deck` image-first owner-chain canary 只证明 workspace-only artifact / review-export receipt plumbing；human ready/export handoff 已记录 owner follow-through ref `rca-operator-ready:export-handoff:deck-owner-chain-human-ready-20260614` 并关闭同一子项的 pending typed blocker，但这只表示 handoff 子项可被 OPL 消费，不授权 visual ready、exportable、handoffable、domain ready、production ready 或 production visual-stage long soak complete。Temporal controlled visual-stage long soak、real memory lifecycle receipt instances 与 cross-family repeated no-regression tail 仍以 RCA-owned progress entry 暴露为 domain-owned typed blocker，分别是 `rca-typed-blocker:controlled-soak:temporal-long-soak-pending`、`rca-typed-blocker:memory-lifecycle:real-receipt-instances-pending` 与 `rca-typed-blocker:no-regression:cross-family-production-scaleout-pending`。该 lane 不调用真实图片 API，不把 workspace artifact 写入 repo，不携带 visual truth/artifact body/review-export verdict body/memory body；真实 live provider evidence、Temporal long soak、real memory lifecycle receipt instances 与 production-like no-regression 仍是 open evidence gates。
