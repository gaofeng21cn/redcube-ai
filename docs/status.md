...
- RCA live stage-run progress evidence 已落地为 OPL 标准消费入口 `contracts/live_stage_run_progress_evidence.json`，并由 `contracts/stage_run_kernel_profile.json` 指向；旧 `contracts/owner_chain_live_progress_evidence.json` 继续作为 owner-chain 输入/引用。字段级 refs、progress entries、source contract、verification command 与具体 ref id 归这些 machine contracts、production acceptance surface 和 focused tests；status 只保留当前读法：已有 mock-safe `ppt_deck` image-first owner-chain canary 和 human ready/export handoff refs 可被 OPL 消费，但它们不授权 visual ready、exportable、handoffable、domain ready、production ready 或 production visual-stage long soak complete。Temporal controlled visual-stage long soak、real memory lifecycle receipt instances 与 production-like repeated no-regression 仍是 RCA-owned typed-blocker evidence tail。该 lane 不调用真实图片 API，不把 workspace artifact 写入 repo，不携带 visual truth/artifact body/review-export verdict body/memory body；真实 live provider evidence、Temporal long soak、real memory lifecycle receipt instances 与 production-like no-regression 仍是 open evidence gates，当前执行顺序和 open gate 读法由 [RCA 理想目标态差距与完善计划](./active/rca-ideal-state-gap-plan.md) 维护。
...
2026-06-21 current source-morphology read：`contracts/physical_source_morphology_policy.json` 已单独分类 `deliverable_route_attempt_shell` 与 `executor_runtime_route_run_records`。前者覆盖 `runDeliverableRoute` / `run-deliverable-route-parts/`，只承认 visual route domain handler、route dependency / continuation / repair refs、stage artifact contract refs、domain-entry response refs 和 typed blocker；后者覆盖 `executor-runtime.ts` / `executor-runtime-parts/route-run-records.ts`，只承认 executor policy refs、neutral route-run record/event refs、stale attempt audit refs、cross-provider attempt refs 和 typed blocker。两者都带 no-resurrection / default-caller cutover gate，防止恢复 generic route attempt shell、retry/dead-letter owner、attempt ledger、runtime record store、event log、session runtime 或 generated wrapper owner。该读法只关闭 source morphology 未分类缺口，不表示 physical delete、OPL default caller parity、visual ready、exportable、handoffable、domain ready 或 production ready。

2026-06-21 tranche follow-through：同一 `contracts/physical_source_morphology_policy.json` 现在新增 `default_caller_tail_thinning_gate`。该 gate 把 product-entry/session refs adapter、`runtime_watch_projection`、`domain_action_adapter_guarded_actions`、operator evidence projection、manifest projection、route attempt shell、executor route-run records 和 repo shell verification wrappers 统一列为 non-live functional/structure tail；每项只能保持 refs-only read model、domain handler target、service-safe domain entry、minimal authority function、native helper implementation、repo-native verification wrapper 或 tombstone/provenance。进一步删除或收薄前必须同时具备 OPL generated default-caller parity、no-active repo-local default caller、RCA owner receipt / typed blocker roundtrip、no-forbidden-write proof、retired alias no-resurrection proof 和 tombstone/provenance pointer。该 gate 明确禁止 `runtimeWatch` 回到 `domain_action_adapter` 默认派发、禁止 `domain_action_adapter` 成为 generic dispatch / generated wrapper owner、禁止 route-run records 成为 attempt ledger owner、禁止 shell wrappers 成为 runtime owner；它不授权 physical delete、visual ready、exportable、handoffable、domain ready 或 production ready。

2026-06-22 source-purity follow-through：同一 tail gate 现在新增 `active_source_resurrection_scan_policy`。该 policy 以 `tests/helpers/rca-retired-surface-guard.ts#ACTIVE_PRIVATE_PLATFORM_RESURRECTION_CLAIM_PATTERNS` 和 `tests/rca-legacy-name-allowance.test.ts` 扫描 active source，fail closed 于 `runtimeWatch_can_return_to_domain_action_adapter_default_dispatch=true`、`domain_action_adapter_can_become_generic_dispatch_owner=true`、generated wrapper owner、generic runtime/session/workbench owner 等复活声明。它只证明 active source 没有把 retained tail 写回私有平台 owner 或 default dispatch；不授权 physical delete、default caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。

2026-06-22 private-platform strict readback：`npm run private-platform:readback` / `scripts/check-private-platform-retirement.ts --format json` 现在把 `buildPrivatizedFunctionalModuleAuditProjection()`、`buildPhysicalSourceMorphologyPolicy()`、`runtime_watch_projection` refs-only boundary、`domain_action_adapter` blocked actions / forbidden writes、`active_source_resurrection_scan_policy` 的 active source 扫描和 current-program audit readback 合并成 direct JSON `rca_private_platform_retirement_strict_readback`；`npm run test:private-platform:strict` 与 `scripts/verify.sh private-platform:strict` 会先 build 再执行同一 readback 并写入 `/tmp/redcube-ai-private-platform-retirement.json` 作为 strict gate artifact。该 readback 只证明 repo source / contract guard 未复活私有平台、default dispatch 或 wrapper owner；不写 visual truth、artifact blob、memory body、review/export verdict、owner receipt，不授权 physical delete、default-caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。

2026-06-22 tail readback compact follow-through：同一 strict readback 顶层的 `default_caller_tail_compact_retirement_summary` 直接从 `contracts/physical_source_morphology_policy.json#/default_caller_tail_readback/compact_retirement_summary` 派生。当前 compact 读面只保留 current worklist count 和 role guard：`tail_surface_count=0`、`cleanup_candidate_count=0`、`owner_delta_required=false`、`can_apply_cleanup=false`，供 OPL tranche foldback / operator drilldown 读取；它不保存旧 surface id 清单，不替代 current role classification，不授权 physical delete、default-caller cutover、visual/export/handoff/domain/production ready，也不创建 owner receipt 或 typed blocker 实例。

2026-06-22 default-caller tail owner-delta readback：`npm run default-caller-tail:readback` / `scripts/check-private-platform-retirement.ts --scope default-caller-tail --format json` 现在把同一 `default_caller_tail_readback` 提炼成 direct JSON `rca_default_caller_tail_owner_delta_readback`，输出 compact summary、8 条 `owner_delta_route`、`typed_blocker_ref_shape` 与 `owner_delta_work_order_pack`。`npm run test:default-caller-tail:strict` 与 `scripts/verify.sh default-caller-tail:strict` 会先 build 再执行同一读面并写入 `/tmp/redcube-ai-default-caller-tail-readback.json`。该 readback 只证明 retained tail 的 owner-delta / typed-blocker 路由、work-order pack shape 和 no-resurrection guard 仍 fail-closed；不创建 typed blocker 实例，不签 owner receipt，不授权 physical delete、default-caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。

2026-06-22 generated-surface false-ready follow-through：`contracts/generated_surface_handoff.json`、`contracts/functional_privatization_audit.json`、`contracts/runtime-program/current-program.json`、`contracts/runtime-program/opl-family-contract-adoption.json` 和对应 runtime-program leaf 现在把 generated descriptor / declarative pack 消费显式读成 `declares_generated_surface_descriptor_consumed=true`，但 `declares_generated_surface_consumption_complete=false`、`declares_production_consumption_complete=false`，并通过 `false_ready_guard` 禁止把 descriptor/contract consumption 写成 default caller cutover、App/operator consumption、visual-stage long soak、domain ready 或 artifact ready。该 guard 是非 Live 的结构读面；真实 production visual-stage long soak、App/operator sustained consumption、artifact-producing owner receipt 和 domain readiness 仍属于后置证据门。

2026-06-21 tranche follow-through：`domain_action_adapter` dispatch implementation 已从单个 orchestration 文件拆薄为 `dispatch-orchestration.ts` 路由入口、`dispatch-evidence-actions.ts` evidence / owner-closeout actions、`dispatch-receipt-actions.ts` receipt / memory / lifecycle actions 和 `dispatch-shared.ts` envelope / blocker / runtime JSON helpers。`contracts/physical_source_morphology_policy.json#active_surface_classifications/domain_action_adapter_guarded_actions` 继续以 `packages/redcube-domain-entry/src/actions/domain-action-adapter-parts/` 目录级 source ref 覆盖这些 parts，classification 仍是 `domain_handler_target`，不新增 generic dispatch owner、generated wrapper owner、compatibility alias、visual/export/handoff/domain/production ready claim 或 physical delete authority。

2026-06-21 source-ref follow-through：`contracts/physical_source_morphology_policy.json#/source_ref_integrity_gate` 现在同时输出 checked source refs、checked machine-boundary refs、retired legacy surface id pointer policy 和 retired compatibility payload field policy 的 guard scope。所有 active surface `source_refs` 必须是 repo-local path / directory / anchored path，所有 `machine_boundary_refs` 必须带 anchor 并解析到当前 checkout；绝对路径、父目录穿越、URI / URL、`human_doc:*` machine ref、悬空 ref 或无 anchor boundary 都会重新打开 source hygiene gap。该 gate 只防 stale / external / retired compatibility refs 成为第二真相源；它不创建 missing ref、不生成 alias、不授权 physical delete、不声明 default caller cutover、App/live readiness、visual/export/handoffable、domain ready 或 production ready。
...
2026-06-21 tranche follow-through：同一 policy 现在新增 `default_caller_tail_readback`。该 readback 对 8 个 retained tail 逐项输出 classification、source refs、missing evidence worklist、owner-delta route、RCA-owned typed blocker ref shape 和 no-resurrection policy；当前所有 tail 都仍缺 default-caller parity、no-active repo-local caller、owner receipt / typed blocker roundtrip、no-forbidden-write proof、retired-alias no-resurrection proof 和 tombstone/provenance pointer。该 readback 只能作为 cleanup/retirement 的当前缺口读面，不授权 physical delete、default-caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。

2026-06-22 default-caller tail work-order pack follow-through：同一 readback 现在还暴露 `owner_delta_work_order_pack`，从 compact retirement summary 派生 8 个 retained tail 的 required evidence ids、owner-delta route 和 typed blocker ref shape，并把 work-order authority boundary 固定为全部 false。该 pack 只给 OPL/App/operator/OMA 提供 body-free 下一步缺口包；不执行 work order、不创建 typed blocker 实例、不签 owner receipt、不写 visual truth、不授权 physical delete、default-caller cutover、visual/export/handoff/domain/production ready。
