...
2026-06-22 private-platform strict readback：`npm run private-platform:readback` / `scripts/check-private-platform-retirement.ts --format json` 现在把 `buildPrivatizedFunctionalModuleAuditProjection()`、`buildPhysicalSourceMorphologyPolicy()`、`runtime_watch_projection` refs-only boundary、`domain_action_adapter` blocked actions / forbidden writes 和 current-program audit readback 合并成 direct JSON `rca_private_platform_retirement_strict_readback`；`npm run test:private-platform:strict` 与 `scripts/verify.sh private-platform:strict` 会先 build 再执行同一 readback 并写入 `/tmp/redcube-ai-private-platform-retirement.json` 作为 strict gate artifact。该 readback 只证明 repo source / contract guard 未复活私有平台或 wrapper owner；不写 visual truth、artifact blob、memory body、review/export verdict、owner receipt，不授权 physical delete、default-caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。
