...
2026-06-22 private-platform strict readback：`scripts/check-private-platform-retirement.ts --format json` 现在把 `buildPrivatizedFunctionalModuleAuditProjection()`、`buildPhysicalSourceMorphologyPolicy()`、`runtime_watch_projection` refs-only boundary、`domain_action_adapter` blocked actions / forbidden writes 和 current-program audit readback 合并成 `rca_private_platform_retirement_strict_readback`；`npm run test:private-platform:strict` 与 `scripts/verify.sh private-platform:strict` 会先 build 再执行该 strict gate。该 readback 只证明 repo source / contract guard 未复活私有平台或 wrapper owner；不写 visual truth、artifact blob、memory body、review/export verdict、owner receipt，不授权 physical delete、default-caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。
