...
- 2026-05-11 OPL family index 校准：OPL `agents list` 与 `stages list` 当前能把 RCA 识别为 aligned standard domain-agent skeleton 和 6-stage family stage plane；RCA 本仓也已落地 visual pattern memory migration/proposal/receipt/operator projection surface、标准 `family-domain-memory-ref.v1` adapter 和 `controlled_visual_stage_attempt` proof。该 adapter 只让 OPL family index 解析 RCA memory refs；RCA visual pattern memory body、accept/reject、visual route、review/export verdict 与 artifact authority 不迁出 RCA。
