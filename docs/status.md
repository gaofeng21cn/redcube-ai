...
2026-06-22 default-caller tail owner-delta readback：`npm run default-caller-tail:readback` / `scripts/check-private-platform-retirement.ts --scope default-caller-tail --format json` 现在把同一 `default_caller_tail_readback` 提炼成 direct JSON `rca_default_caller_tail_owner_delta_readback`，输出 compact summary、8 条 `owner_delta_route` 与 `typed_blocker_ref_shape`。`npm run test:default-caller-tail:strict` 与 `scripts/verify.sh default-caller-tail:strict` 会先 build 再执行同一读面并写入 `/tmp/redcube-ai-default-caller-tail-readback.json`。该 readback 只证明 retained tail 的 owner-delta / typed-blocker 路由和 no-resurrection guard 仍 fail-closed；不创建 typed blocker 实例，不签 owner receipt，不授权 physical delete、default-caller cutover、visual ready、exportable、handoffable、domain ready 或 production ready。
