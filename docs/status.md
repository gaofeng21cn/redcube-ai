...
2026-05-22 OPL 进一步消费一条 RCA `controlled_visual_stage_attempt` domain-dispatch workorder：`opl runtime action execute --action domain_dispatch:redcube:sat_8623e55a341caf9672ed8ab2:record|verify` 已把 RCA-owned typed blocker `rca-typed-blocker:controlled-soak:temporal-long-soak-pending`、production acceptance owner-chain refs、repeated no-regression refs 和 next verification command refs 记录并验证为 OPL refs-only external evidence receipt `opl://external-evidence/redcube/domain_dispatch:redcube:sat_8623e55a341caf9672ed8ab2`。Fresh OPL worklist 中 RCA remaining dispatch workorder 从 7 条降到 6 条；这只关闭 controlled-soak blocker 的 OPL workorder accounting，不改变 `production_visual_stage_long_soak_complete=false`，也不声明 visual ready、exportable、handoffable、artifact-producing owner receipt、真实 memory/lifecycle receipt scaleout 或 cross-family repeated no-regression production cadence 完成。
...
2026-05-21 Lane C 新增 RCA-owned `rca_efficiency_handoff_projection`，并接入 production acceptance contract、product-entry manifest 和 product sidecar projection。该 surface 汇总既有 `cache_status`、`elapsed_ms`、`render_execution`、`reused_slide_ids`、`cost_summary`、`screenshot_review` gate 和 export result refs，输出 refs-only Agent Lab compatible suite input；suite kind 继续使用 OPL Agent Lab `standard`，不要求新增 RCA-specific kind。2026-05-22 OMA efficiency patch-loop 后，该投影进一步显式暴露 source pack reuse、prompt/static-prefix cache、page-local batch/parallel telemetry、blocked-page-only repair 和 export preview cache 的 refs-only 证据，并把 target verification refs 放入 handoff；这些 refs 只提升效率可观察性和编排评估，不降低 `screenshot_review`、review/export gate、artifact authority、visual memory authority 或 owner receipt 门槛，不声明 visual ready、exportable、handoffable 或 production soak complete。
