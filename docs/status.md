...
当前 active source 中仍可见的 `product-entry-session-snapshot-ref-adapter`、direct `runtimeWatch` read model、product sidecar guarded actions、operator evidence/stability projection、native helper envelope、MCP/CLI/product-entry wrapper 和 `@redcube/gateway` package boundary 都不是旧接口兼容承诺。`@redcube/gateway` 已在 morphology policy 中独立分类为 `package_protocol_boundary`，public identity 仍是 `redcube-ai`，不代表 gateway public identity 或 generic gateway runtime owner。Operator evidence 源码已从单一 manifest-return 聚合面拆成 `operator-evidence-refs.ts`、`operator-evidence-readiness.ts` 与 final assembly 三层。它们只允许作为 refs-only adapter、domain handler target、native helper implementation、fixture、diagnostic 或 package/protocol boundary；generated/default caller parity、domain authority refs preserved、no-regression proof 与 provenance consumer migration 成立后，继续 rename/delete/tombstone，不新增 compatibility alias、facade 或旧 public path。
