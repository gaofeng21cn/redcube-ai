...
- upstream Hermes 历史合同保留 `REDCUBE_HERMES_GATEWAY_COMMAND` 和 `hermes gateway run` 作为历史命令值 provenance；读者可见字段名已收口为 upstream launch semantics，guard tests 阻断新的 `*_gateway_command*` object key 或 `gateway -> ...` active route chain。
