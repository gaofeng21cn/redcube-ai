...
- 2026-07-06 source hygiene pass 继续关闭高信号过度设计：overlay/runtime-family registry 的默认 module list 只保留 source-local loader owner，不再由 `package.json` manifest metadata 双维护；`redcube-pack-*` 三个 type-only package 边界已并回对应 runtime-family 类型层，只保留 pack id 字符串；Codex plugin repo-local legacy alias symlink 已退役；CLI option parser 已改用 `node:util.parseArgs` tokenization，同时保留当前 long-option map 行为；product-entry retained tail 只做静态 export thinning，不声明 default-caller cutover 或 live evidence closure。
