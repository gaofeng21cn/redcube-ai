...
Codex Developer Mode source pointer: 仓库根层 `.codex-plugin/plugin.json` 是 `rca-local` 指向 live developer checkout 时的 Codex plugin manifest 入口；它只把 Codex source root 固定到当前 `redcube-ai` repo，并直接通过 `./plugins/redcube-ai/skills/` 和 `./plugins/redcube-ai/assets/` 引用 canonical scaffold。canonical plugin scaffold 统一在 `plugins/redcube-ai/`：`plugins/redcube-ai/.codex-plugin/plugin.json` 的 plugin name 与 `plugins/redcube-ai/skills/redcube-ai/SKILL.md` 的 skill name 都使用 repo slug `redcube-ai`；legacy `plugins/rca` 与 `plugins/redcube-ai/skills/rca` symlink alias 已退役，不再作为安装、测试或 skill 发现路径。repo-local `.agents/plugins/marketplace.json` 是已退役副产物，`scripts/install-codex-plugin.ts` 只校验 canonical source 并清理旧副产物。Codex marketplace registration 由 OPL-owned wrapper 写入 `OPL_STATE_DIR/codex-plugin-marketplaces/rca-local`。该入口只解决 Codex plugin metadata/source channel 路径，不声明 visual ready、exportable、handoffable、domain ready 或 production ready。
...
本轮 executor adapter owner 读法已在机器字段上收窄：RCA `opl_executor_adapter_receipt` 保留 `owner=opl_runtime_manager`，并把显式 backend 写成 `selected_executor_backend=hermes_agent`。`hermes_agent` 只表示一次显式非默认 executor/backend 选择，`backend_lifecycle=historical_opt_in_deferred_external_adapter` 且 `rca_default_backend=false`；它不是 RCA default backend、runtime owner 或 production visual evidence。当前 active caller scan 显示 runtime-family executor routing、runtime deliverable descriptor、executor routing schema、`pyproject.toml` console script 和 Python native helper catalog 仍引用 Hermes symbols / `redcube_ai.hermes.agent_loop_bridge_impl`，其中多个 caller 不在本 lane 写集内，因此不能物理删除导出符号或 package module。本仓内 RCA-owned Hermes API / loop implementation 已收缩为 fail-closed retired boundary：不再发起 API 请求、不再启动 Python subprocess、不再 import/run upstream Hermes-Agent；旧 mock proof 覆盖改为 retired/fail-closed 覆盖。已不存在的旧 wrapper `python/redcube_ai/hermes/agent_loop_bridge.py` 继续保持 retired；当前保留的是 active caller 约束下的最小 import/entrypoint 边界。删除门归 OPL Agent Executor Adapter：OPL adapter default-caller parity、attempt ledger / runtime record parity、RCA route policy / receipt refs preserved、focused proof tests 迁到 OPL-owned surface、no-active RCA caller scan、RCA owner receipt 或 typed blocker。
...
Cognitive Kernel 已落为 RCA stage 内部结构，而不是新的用户流程或额外 runtime route。`contracts/cognitive_kernel_adoption.json` 和 `contracts/golden_path_profile.json` 声明 advisory/current adoption 与 golden path；`agent/tools/domain_affordances.md` 是 domain tool affordance catalog；`contracts/stage_control_plane.json` 的 6 个 visual stage 只保留 `strategy_refs`、`tool_refs` 与 policy/body generated readback refs；完整 `tool_affordance_boundary`、`candidate_pool_policy`、`independent_gate_policy` 和 `handoff_policy` body 由 product-entry manifest 的 `family_stage_control_plane` generated descriptor 提供；`contracts/pack_compiler_input.json#/cognitive_stage_pack_contract` 固定 OPL pack compiler 的 refs-only input。工具目录只表达 Codex executor 可用 affordance，不规定顺序；候选池只作为 stage-internal refs 与 lineage；独立 gate 仍回 RCA review/export / owner receipt / typed blocker。该结构不改变 `ppt_deck` image-first 默认路线，不让 OPL 写 visual truth、artifact body、review/export verdict 或 owner receipt。
...
| Runtime-program and acceptance surfaces | current-program index / parts、单一 aggregate manifest metadata、canonical projection refs、production acceptance 和 live progress evidence 是机器 truth；大型机器 surface 只在 current-program 和 OPL adoption aggregate 中保留 refs-only projection，不复制完整 body。status 只说明这些 surfaces 记录 body-free owner receipt refs、artifact/review/export refs、memory/lifecycle refs、typed blocker refs 和 next verification refs。 | `contracts/runtime-program/current-program.index.json`、`contracts/runtime-program/current-program-parts/**`、`contracts/runtime-program/current-program.bundle-manifest.json`、`contracts/runtime-program/opl-family-contract-adoption.json`、`contracts/production_acceptance/rca-production-acceptance.json`、runtime evidence |
...
- 2026-07-07 source hygiene pass 继续关闭高信号过度设计：overlay/runtime-family registry 的默认 module list 只保留 source-local loader owner，不再由 `package.json` manifest metadata 双维护；`redcube-pack-*` 三个 type-only package 边界已并回对应 runtime-family 类型层，只保留 pack id 字符串；Codex plugin repo-local legacy alias symlink 已退役；CLI option parser 已改用 `node:util.parseArgs` tokenization，同时保留当前 long-option map 行为；domain-entry product/session/fanout/action cluster 的 `safeText` / `requireField` / JSON read helper 已收口为 package-local `actions/action-utils.ts`；runtime source-augmentation / source-readiness / route helper cluster 复用 package-local `runtime-utils.ts`；runtime-protocol Stage Folder / screenshot / source-truth / Python helper cluster 复用 package-local `protocol-utils.ts`；PPT family 复用既有 `core-helpers.ts`。剩余 `safeText` / `readJson` / `ensureDir` raw 命中只按 package-local owner、nullable/readback 语义和净删减收益判断，不上升成全仓 util，也不以 grep 归零为目标；product-entry retained tail 只做静态 export thinning，不声明 default-caller cutover 或 live evidence closure。
