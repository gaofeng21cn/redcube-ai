...
当前物理源码仍有命名与包边界 tail：MCP server、product-entry session store、workspace/run envelope helpers、runtimeWatch、product sidecar export/dispatch、operator evidence/stability projections 和 generated-surface handoff mapping 仍是 active source。2026-05-21 已把 OPL generated wrapper、generic primitive、stability read-model 与 substrate export projection body 从 `product-sidecar-guarded-actions.ts` 拆到 `product-sidecar-parts/opl-generic-boundaries.ts`，使 guarded actions 文件收薄为 action metadata / audit projection consumer；同日又把 `product-sidecar.ts` 的 refs-only sidecar export projection、runtime framework projection 和 mapped surfaces 拆到 `product-sidecar-parts/sidecar-export-projection.ts`，使 `product-sidecar.ts` 收薄为 stable API entry、dispatch envelope 和 RCA guarded authority handlers。这些只是源码边界清理，不表示 OPL generated shell、session/workbench shell 或 production visual-stage long soak 已完成。后续又把 `product-entry-continuity-surfaces.ts` 的 OPL family lifecycle adapter body 拆到 `product-entry-continuity-surfaces-parts/opl-family-lifecycle-adapter.ts`，入口文件从 696 行收薄到 385 行；该 parts 模块对应 OPL shared `product-entry-companions` lifecycle adapter primitive，是 migration input，不是 RCA-owned generic session shell。RCA 仍需等 pinned shared package 和 generated/default product-entry session caller 更新后，才能把本地 parts module 替换为 OPL shared import 并删除本地实现。随后 `get-product-entry-manifest.ts` 的 skill catalog、product-entry shell、linked status/direct surface 与 operator-loop action catalog 装配已拆到 `get-product-entry-manifest-parts/shell-catalog.ts`，主 manifest 文件从 943 行收薄到 716 行；该 parts 模块只是 OPL generated product-entry/skill/operator shell 的迁移输入，仍不能写成 OPL generated manifest default caller 已完成。上述 active source 不能被写成 RCA-owned generic runtime，也不能被写成已经完全像新 Agent 模板；它们的当前角色是 protocol/domain handler target、refs-only adapter、native helper envelope、operator read model 或 provenance。`WORKSPACE_LOCATOR_ENVELOPE_BOUNDARY`、`RUN_LOCATOR_ENVELOPE_BOUNDARY` 和 `RUNTIME_WATCH_BOUNDARY` 是当前 machine-readable refs-only 边界：它们显式声明不拥有 generic workspace shell、runner、attempt ledger、scheduler、session runtime、supervisor、workbench、artifact lifecycle 或 visual ready/export verdict。后续 cleanup 需要 OPL generated wrapper / session shell / App workbench evidence、direct/hosted parity、owner receipt roundtrip、no-regression proof 和 no-compatibility-alias migration。
