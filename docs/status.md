...
- `domain_memory_descriptor_locator` 已从 descriptor/locator 推进到可迁移/可验收的 repo-source contract surface：`rca.visual_pattern_memory.migration_plan.v1` 描述从 workspace/runtime root、product-entry session、`visual_director_review`、`screenshot_review`、export closeout 或 human reference 发现候选经验，到 seed fixture locator、RCA accept/reject、memory locator ref 与 writeback receipt ref 的流程；`rca.visual_pattern_memory.seed_fixture_locator.v1` 和 `rca.visual_pattern_memory.writeback_receipt_locator.v1` 只承载 locator / receipt ref 字段。Landed：合同、manifest surface、current-program 指针、policy/reference 文档与 focused tests。Pending：真实 reusable visual lesson 的迁移执行、domain-owned memory 写入、stage descriptor `visual_pattern_memory_refs` 接入与 product-entry/operator projection 消费显示。该变更不写 repo-tracked memory entry、receipt instance、PNG/PPTX/PDF、review verdict 或 visual truth。
