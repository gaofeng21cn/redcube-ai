...
2026-06-22 tail readback compact follow-through：同一 strict readback 顶层新增 `default_caller_tail_compact_retirement_summary`，直接从 `contracts/physical_source_morphology_policy.json#/default_caller_tail_readback/compact_retirement_summary` 派生。该 summary 只把 8 个 retained tail 的当前状态压缩为 `cleanup_candidate_count=0`、`owner_delta_required=true`、`can_apply_cleanup=false` 和缺失 evidence id 列表，供 OPL tranche foldback / operator drilldown 读取；它不替代逐项 tail classification，不授权 physical delete、default-caller cutover、visual/export/handoff/domain/production ready，也不创建 owner receipt 或 typed blocker 实例。
