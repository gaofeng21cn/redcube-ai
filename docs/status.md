...
| Runtime-program and acceptance surfaces | current-program index / parts、单一 aggregate manifest metadata、production acceptance 和 live progress evidence 是机器 truth；status 只说明这些 surfaces 记录 body-free owner receipt refs、artifact/review/export refs、memory/lifecycle refs、typed blocker refs 和 next verification refs。 | `contracts/runtime-program/current-program.index.json`、`contracts/runtime-program/current-program-parts/**`、`contracts/production_acceptance/rca-production-acceptance.json`、runtime evidence |
