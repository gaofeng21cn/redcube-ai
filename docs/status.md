...
Cognitive Kernel 已落为 RCA stage 内部结构，而不是新的用户流程或额外 runtime route。`contracts/cognitive_kernel_adoption.json` 和 `contracts/golden_path_profile.json` 声明 advisory/current adoption 与 golden path；`agent/tools/domain_affordances.md` 是 domain tool affordance catalog；`contracts/stage_control_plane.json` 现在只保留 6 个 visual stage 的 refs、stage ids 与 authority boundary，不再复制 `tool_affordance_boundary`、`candidate_pool_policy`、`independent_gate_policy` 或 `handoff_policy` body；`contracts/pack_compiler_input.json` 固定 OPL pack compiler 可消费的 `agent/` refs 与 minimal authority refs。工具目录只表达 Codex executor 可用 affordance，不规定顺序；候选池只作为 stage-internal refs 与 lineage；独立 gate 仍回 RCA review/export / owner receipt / typed blocker。该结构不改变 `ppt_deck` image-first 默认路线，不让 OPL 写 visual truth、artifact body、review/export verdict 或 owner receipt。
...
| --- | --- | --- |
| Declarative Visual Pack / stage control | RCA root stage / pack contracts 已收为 `agent/` refs、stage ids、artifact authority refs 和 OPL generated-stage-control owner boundary；完整 generic stage-control substrate 归 OPL generated/hosted surface，不由 RCA root contract 维护。 | `agent/`、`contracts/stage_control_plane.json`、`contracts/pack_compiler_input.json`、focused stage/control tests |
| Product-entry / generated surface boundary | product-entry manifest、domain handler target、minimal action target refs 和 generated descriptor refs 只暴露 descriptor、domain handler target、refs-only projection、owner receipt shape 与 typed blocker；repo-local product/session/status/foundry generic shell 不再是 RCA active surface。 | product-entry manifest、`contracts/action_catalog.json`、OPL generated descriptor refs、source/tests |
