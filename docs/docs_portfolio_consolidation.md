...
OPL 系列项目的全局主参考是 `/Users/gaofeng/workspace/one-person-lab/docs/active/opl-family-development-reference.zh-CN.md`。它持有全局 framework 目标、跨仓差距顺序、shared primitive 上收、App/workbench 目标与同名 docs taxonomy。RCA 文档只维护 RCA target state、RCA gap、visual authority、direct/hosted surface 与 RCA-to-OPL 上收候选。
...
upstream Hermes proof/provenance 进入 `docs/history/hermes/`。runtime-program
合同中的 `human_doc:program_*` 继续作为语义化读者上下文 ID，不再暗示物理目录名。

本轮 RCA runtime/program/history cleanup 的新增约束是：先审计
`contracts/runtime-program/*.json` 的 `human_doc:*` 引用；迁移物理文档时保持语义
ID 稳定，并同步 README / lifecycle / tombstone 读法。

本轮审阅按正文内容判断生命周期。已逐篇覆盖：

- `docs/product/`、`docs/runtime/`、`docs/delivery/`、`docs/source/` 的 README 与正文文档。
- `docs/active/` 根层 baton brief 与 `docs/history/phase-2/` 全部 tranche brief。
...
- `docs/active/`：当前执行、当前计划、当前差距、active baton、absorbed tranche 与 closeout evidence。
- `docs/specs/`：当前仍有效的技术规格索引；旧 spec 必须标清 active/history。
...
新增文档先判断它是 current truth、active baton、stable policy、product/delivery/source support、reference 还是 history/tombstone，再落入 canonical 目录。旧 `docs/program/` 不再作为 recurring material 落点。
...
2. active baton 和仍被 `human_doc:*` 引用的 current brief 留在 `docs/active/`，并写清 lifecycle state。
...
6. 如果某项 RCA 计划实际是通用 framework primitive，RCA 只记录 visual-domain 证据和 handoff 边界，implementation owner 回到 OPL；不要在 RCA 维护 provider runtime、generic queue、generic artifact gallery、generic route graph、generic review queue、generic memory locator 或 generic observability 的平行计划。
...
| `docs/active/redcube_product_entry_mvp.md` | current baton | 留在 `docs/active/`，补 lifecycle note | `current-program.json` 指向 product-entry brief |
| `docs/active/managed_product_entry_hardening.md` | current baton / session continuity | 留在 `docs/active/`，补 lifecycle note | `current-program.json` 指向 managed product-entry brief |
| `docs/active/opl_framework_hosted_product_entry.md` | contract-linked internal integration | 留在 `docs/active/` | OPL 托管路径仍需读者上下文；RCA 不让出 visual truth |
| `docs/history/hermes/upstream_hermes_agent_*.md` | contract-linked proof / provenance | 留在 `docs/history/hermes/`，按 proof/provenance 文档阅读 | current contracts 仍有 `human_doc:*` 指针；当前 owner 是 RCA direct path + OPL provider-backed hosting |
| `docs/history/phase-2/*.md` | absorbed tranche / contract-linked baton history | 留在 `docs/history/phase-2/`，统一 lifecycle note | 多个 tranche contract 仍指向这些 human docs；其内容已吸收进 current runtime/delivery/source/governance |
...
- `docs/history/hermes/**` 属于 repo-local Hermes migration provenance。
- `docs/superpowers/**` 仍按本仓约定保持未跟踪；本轮不把本地 AI / Superpowers 过程草稿导入 repo-tracked history。
- `docs/history/phase-2/**`、`docs/active/opl_framework_hosted_product_entry.md` 与 `docs/history/hermes/upstream_hermes_agent_*.md` 仍存在 runtime-program 合同引用；本轮已物理迁移，并通过 lifecycle note 标明 absorbed / internal integration / provenance 性质。
...
- `docs/references/positioning/domain-harness-os-positioning.md` 仍在 reference 层，因为它由 `human_doc:domain_harness_os_positioning` 语义 ID 保护；它只作为 internal boundary vocabulary 读取。
...
- 合同引用不再阻止物理迁移：仍被 `contracts/runtime-program/*.json` 通过 `human_doc:*` 指向的 brief 保持语义 ID 稳定，物理位置按 canonical lifecycle 归入 active、references 或 history。
...
- `docs/runtime/runtime_architecture.md` 瘦身为 current runtime topology / executor-backend / watch-projection / OPL-hosted boundary 说明；Phase 2 closeout、old gateway/harness narrative 与 Hermes proof 细节下沉到 active/reference/history 索引读取。
- `docs/history/phase-2/README.md` 是 absorbed tranche 子目录索引；各 tranche brief 已迁入 history。
- `docs/history/hermes/upstream_hermes_agent_*.md` 已迁入 Hermes history，继续按 historical proof / blocker / closeout provenance 读取。
...
本轮移动了 contract-linked reader-context 文件，但未修改 `human_doc:*` 语义 ID 或 runtime-program contracts；验证需覆盖 docs path hygiene 与 runtime-program provenance。
