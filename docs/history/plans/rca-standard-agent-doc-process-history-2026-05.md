...
- RCA 将 generated descriptor scope 收成 `cli`、`mcp`、`skill`、`product_entry`、`product_status`、`product_session`、`domain_action_adapter`、`workbench`，并将 `functional_harness_cases` 作为 OPL generated test surface；repo-local wrappers 只作为 domain handler target、direct domain entry 或 refs-only adapter。
- RCA 从 default domain_action_adapter dispatch/action 面删除或收薄 `supervise_managed_run` 与 `product_entry_continuation`，把 generic supervision / continuation 归 OPL runner/session shell。
