# Retired Managed Product Entry Contract

Owner: `RedCube AI`
Purpose: `managed_product_entry_contract_tombstone`
State: `history`
Machine boundary: 人读 tombstone。机器真相继续归 `contracts/runtime-program/product-entry-session-continuity.json`、product-entry manifest、CLI/MCP/API behavior、runtime artifacts 和 owner receipts。

已退役词汇：

- `managed_product_entry_hardening`
- `managed-product-entry-hardening.json`
- `human_doc:program_managed_product_entry_hardening`

当前 owner surface：

- active contract：`contracts/runtime-program/product-entry-session-continuity.json`
- active support doc：`docs/references/product-entry/product_entry_session_continuity.md`
- callable surface：`getProductEntrySession` / `redcube product session` / `get_product_entry_session`

退役读法：

旧 `managed` 命名只允许作为 provenance、semantic-id 或 tombstone 出现。它不得恢复为 active caller、public action key、default runtime owner、sidecar template、compatibility alias、facade 或 wrapper。RCA 继续持有 visual truth、review/export verdict、artifact authority、visual memory accept/reject authority 和 owner receipt；OPL 持有 generic stage runtime / attempt ledger / queue / wakeup shell。
