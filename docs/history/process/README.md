...
| --- | --- | --- |
| RCA docs portfolio scope | Root `README*`, `docs/*.md`, `docs/active/*.md`, `docs/public/*.md`, `docs/product/*.md`, `docs/runtime/*.md`, `docs/delivery/*.md`, `docs/source/*.md`, `docs/policies/*.md`, `docs/specs/*.md`, `docs/references/**/*.md` and `docs/history/process/*.md` have lifecycle roles routed through `docs/README.md`, `docs/docs_portfolio_consolidation.md`, this process index and retained provenance records. | Reopen a specific document body only when a fresh scan finds active-looking checklist text, reusable prompt material, current-owner conflict, stale SSOT duplication or machine surface dependence on historical prose. |
| Active truth owners | `docs/active/rca-ideal-state-gap-plan.md` remains the only active completion / gap / baton owner. `docs/active/opl-private-implementation-migration-inventory.md` remains the path-level private implementation owner. Lifecycle policy remains in `docs/docs_portfolio_consolidation.md`; this file only keeps process provenance. | Private-inventory detail refresh and generated/default caller thinning evidence tails remain active work under the active plan, not process-history work. |
| Content-level consolidation | Adapter-thinning, Hermes command-list, Phase 2 high-risk/residual, tombstone and plans-history details are represented by the provenance groups above and by the corresponding history directory indexes. Detailed rewrite facts stay recoverable from git history and retained provenance files; current rules stay in owner docs, contracts and tests. | Do not add file-by-file closeout logs here. If a historical rule is still current, fold it back into the owner doc, contract, source or test before compressing the process trace. |
...
| Parent OPL series goal | RCA history/provenance bodies are covered at directory-index level for docs lifecycle purposes. This is a tranche-level foldback, not six-repo completion. | Remaining RCA scope includes production-like no-regression, Temporal visual-stage long soak, generated/default caller thinning and concrete source/contract/test retirement lanes. Do not expand this process index back into dated proof logs. |
