...
- Theme / SSOT: `docs/history/hermes/**` verification command lists, launcher
  details, pass-count transcripts and live proof snippets. Current executor /
  backend boundary remains core docs, `docs/runtime/`,
  `docs/active/rca-ideal-state-gap-plan.md`, runtime-program contracts,
  source/tests, owner receipts and typed blockers. Current verification truth
  remains repo-native scripts/tests and live evidence; history briefs only keep
  provenance.
...
- Edited: the six target Hermes history bodies, `docs/history/hermes/README.md`,
  this process index and
  `docs/history/process/retired-surface-provenance.md`.
- Coverage result: target Hermes history bodies no longer preserve exact test
  command lists, launcher invocations, assertion text or pass-count transcript
  as current verification guidance. They keep historical verification
  categories, current owner refs and no-readiness boundaries.
- Retired / guarded: no source module, contract, test, workflow or history file
  was physically deleted. The retired surface is active-looking command/proof
  prose inside Hermes history bodies; it may not be reused as current CI gate,
  verification runbook, default runtime owner proof, readiness evidence or
  Agent prompt.
- Remaining RCA unreviewed scope under the parent OPL series goal: other Hermes
  history bodies not touched in this command-list tranche, active private
  inventory details, generated/default caller thinning evidence tails, and
  future source/contract/test retirement lanes that need fresh no-active-caller
  plus owner receipt / typed blocker proof.
- Next write scope: continue with another precise history/provenance body theme
  after fresh scan, or move to a clean sibling repo lane. Do not use this
  tranche as proof that every RCA `docs/**/*.md` paragraph has been
  line-reviewed.
