...
| Hermes / Phase 2 / plans / tombstone bodies | Historical bodies keep only provenance, current owner maps and no-resurrection rules. Active-looking command lists, proof transcripts, quickstart/runbook boards, source/review/delivery contract tables, native proof handbooks and continuation boards must not be reused as current guidance. |
...
- Scope reviewed: root `README*`, `docs/*.md`, `docs/active/*.md`, `docs/public/*.md`, `docs/product/*.md`, `docs/runtime/*.md`, `docs/delivery/*.md`, `docs/source/*.md`, `docs/policies/*.md`, `docs/specs/*.md`, `docs/references/**/*.md`, `docs/history/process/*.md`, and current machine surfaces relevant to docs truth and retired-surface guard.
- SSOT decision: `docs/active/rca-ideal-state-gap-plan.md` remains the only active completion/gap/baton owner; `docs/active/opl-private-implementation-migration-inventory.md` remains path-level private implementation owner; `docs/docs_portfolio_consolidation.md` owns lifecycle policy; this file only keeps compressed process provenance.
- Content-level consolidation: prior adapter-thinning, Hermes command-list, Phase 2 high-risk/residual, tombstone and plans-history entries were compressed into the provenance groups above and the corresponding history directory indexes. Detailed file-by-file rewrite facts remain recoverable from git history and retained provenance files; current rules remain in owner docs, contracts and tests.
- Retired / guarded: no source, contract, test, workflow or package surface changed in this tranche. Guarded retired surfaces remain active-looking Hermes/Phase 2/history prose, repo-local product/session/domain_action_adapter/runtimeWatch/operator projection compatibility wording, old public path tests, alias/facade/fallback surfaces, `legacy_payload_field_aliases`, and physical-delete-by-read-model claims.
- Remaining RCA scope under the parent OPL series goal: active private-inventory detail refresh, generated/default caller thinning evidence tails, production-like no-regression, Temporal visual-stage long soak, and concrete source/contract/test retirement lanes needing no-active-caller plus owner receipt / typed blocker proof. History/provenance bodies are covered by directory-level indexes unless a fresh scan finds an active-looking checklist, reusable prompt, current owner conflict, or machine surface that still depends on an uncompressed historical rule.
- Next write scope: after fresh intake, choose one precise SSOT lane such as private-inventory detail governance, generated/default caller thinning, production evidence tail, a triggered history-body reopening, or a concrete old surface retirement candidate. Do not expand this process index back into dated proof logs.
