...
- `Codex CLI` 是当前第一公民 concrete executor；`Hermes-Agent` 只作为 historical / opt-in / deferred external adapter、显式 proof backend、executor adapter 评估、diagnostic 或历史参考读取。
...
| Upstream Hermes cutover / target-shape planning | `upstream_hermes_agent_fast_cutover_board.md`、`upstream_hermes_agent_final_target_shape.md` | 只保存 2026-04 历史 cutover 假设和 target-shape provenance；当前目标已更新为 OPL stage-led runtime、Temporal production substrate、Codex-first concrete executor、Hermes historical / opt-in / deferred external adapter 和 RCA domain authority split。 |
