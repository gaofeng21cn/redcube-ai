...
`contracts/opl_agent_package_manifest.json` 已移除 installed-lock authority、
lifecycle-receipt ownership、`package_core`、lifecycle locator 与 managed-dependency
metadata；OPL consumer 仍有 lock/materializer/runtime-source 兼容读面。这些兼容面只能
作为迁移输入，不能覆盖 fresh carrier readback。
