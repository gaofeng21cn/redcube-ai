...
在这条 OPL 路线上，OPL stage-led family runtime provider 负责 provider profile/provisioning、task registration hydration、runtime status projection、doctor/repair/resume、hosted executor selection、native helper catalog 与高频状态索引；Temporal 是 production online runtime 的必需 substrate。OPL 不持有 RedCube visual-domain truth、canonical artifacts、review-state truth 或 publication projection truth；RCA 也不复制 OPL hosted executor 的配置或执行实现。
