...
当前入口真相是：`redcube-ai` app skill、`CLI / MCP` 已经构成可验证的 `agent entry`，同时 repo-verified 的轻量 `product entry` service surface 也已落地；`status` 在这里仅指面向 agent 的 product-entry overview / intake / entry-shell contract，legacy `redcube product status` 命令键继续保留为兼容入口，但真正面向最终用户的成熟 GUI / WebUI / managed web shell 仍未落地。
