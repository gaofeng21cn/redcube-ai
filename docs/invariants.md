...
- 外部 executor 名称只允许出现在 OPL hosted refs、history/provenance、stable legacy machine contract 或 no-resurrection guard；RCA active source 不保留对应 config、descriptor、fallback、compatibility boundary 或 execution branch。
...
- RCA 的 executor-first policy 固定为仓内只物化 `codex_cli`。其他 executor 由 OPL hosted surface 选择和运行，RCA 只接收 opaque refs；不得恢复 local routing、profile、fallback 或把外部 executor 写成与 Codex 质量/行为等价。
