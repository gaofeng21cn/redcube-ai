...
当前 deliverable facade 只覆盖已存在的 `ppt_deck` 与 `xiaohongshu` surface；默认 truth surface 是 `buildOplStageExecutionPlan` / `opl_stage_execution_plan`，显式 stage rerun 继续使用 `runDeliverableRoute`，审计与投影继续使用 `createDeliverable`、`auditDeliverable`、`runtimeWatch`、`getReviewState`、`getPublicationProjection`。repo-local `runManagedDeliverable` 只保留为显式 diagnostic / historical regression path，不是默认 product-entry runtime。
