...
`stage_control_projection` 是给 OPL family Stage Control Plane 的 descriptor 和 stage-plan adapter。它把 RCA visual routes 映射到 family stage kinds，并允许 OPL provider 基于 `opl_stage_execution_plan` 调度 stage attempts；RCA root `contracts/stage_control_plane.json` 只保留 stage refs、stage ids、artifact authority refs 和 no-visual-authority-transfer boundary，完整 generic stage plane 由 OPL generated/hosted surface 持有。该 projection 不改变 hydrated `stage_sequence`，不持有 RedCube visual truth、review owner 或 artifact authority。Codex App direct skill 调用与 OPL 托管调用必须在 `invokeDomainEntry` / product-entry command contract 后收敛。
...
- RCA 仓内保留的证据只说明显式 opt-in backend fail closed，并暴露删除门
...
- 备选 proof backend 是 `hermes_agent`
  ...
  - 当前 RCA-owned backend implementation 已退役，调用时 fail closed
  - 默认 model / reasoning 不再由 RCA 仓内 Hermes bridge 读取或证明
...
`service-safe domain entry -> runtime-local family runner contract -> executor adapter -> concrete agent runtime`

其中：

- `runtime-local family runner contract` 继续定义 route、artifact、review surface 与 visual-domain truth；通用 stage runtime、attempt ledger、session/workbench shell 归 OPL
...
`ppt_deck` runtime-local family runner 的 core 现在也按这个边界组织：`core.ts` 保留 route / lifecycle / visual-domain assembly，execution adapter、creative owner/source stamp、primary surface 和 structured artifact batch/executor helper 进入 `ppt-deck-runtime-family-parts/execution-adapters.ts`。这让 core 不再直接承载 executor/backend 分支，同时保持 public route、payload shape 和 visual route contract 不变。

当前还要额外冻结一个边界：

- `ppt_deck`、`xiaohongshu`、`poster_onepager` 的受保护创作 stage 现在统一走 `@redcube/runtime/src/families/* + Codex CLI structured generation`；三个 `runtime-family-*` workspace package 已退役
...
OPL generated MCP product-entry descriptor 只承载 RCA action target refs / active manifest action metadata 中的 action。`invokeOplHostedProductEntry` 仍是 internal OPL-hosted integration callable surface，供 OPL handoff contract 使用，不作为 `redcube_product_entry` MCP action key 暴露。
