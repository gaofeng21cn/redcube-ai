...
`contracts/foundry_agent_series.json` 和 `contracts/domain_descriptor.json` 现在只作为 RCA domain-specific profile / refs anchor：它们不再复制 OPL Foundry generic `series_design_profile`、workspace topology profile 或 public series spine。RCA 的视觉资料输入、visual stage pack、PPT/PDF/PNG/export bundle 输出和视觉 authority 函数写在 `domain_specific_profile`、`agent/`、stage/action refs 与 authority refs 中。OPL generic lifecycle、public series profile、workspace topology、provider runtime 和 App/workbench shell 由 OPL owned/generated/hosted surface 持有；这些 refs 不创建 RCA 私有 lifecycle，也不把 OPL provider completion 升级为 visual ready、exportable、handoffable 或 domain ready。
...
`stage_control_projection` 是给 OPL family Stage Control Plane 的 descriptor 和 stage-plan adapter。它把 RCA visual routes 映射到 family stage kinds，并允许 OPL provider 基于 `opl_stage_execution_plan` 调度 stage attempts；RCA root `contracts/stage_control_plane.json` 是标准 OPL `family-stage-control-plane.v1` 输入，但只保留 stage refs、stage ids、stage contract pointers、artifact authority refs 和 no-visual-authority-transfer boundary，完整 generic stage runtime 仍由 OPL generated/hosted surface 持有。该 projection 不改变 hydrated `stage_sequence`，不持有 RedCube visual truth、review owner 或 artifact authority。Codex App direct skill 调用与 OPL 托管调用必须在 `invokeDomainEntry` / product-entry command contract 后收敛。
Stage artifact runtime 的第一真相现在是 OPL Stage Folder Contract，而不是旧 status/read-model 对文件的解释。RCA route handler 写入 OPL runtime-state stage attempt folder，并物化 manifest、outputs、evidence、receipts 与 current/latest pointer；具体物理路径、role manifest、receipt 文件、orphan/broken/stale rules 和 conformance fields 回到 Stage Folder contracts、artifact locator contract、source/tests 和 OPL index/read-model。OPL 提供 index、rebuild、status/explain、orphan/broken/stale projection、gallery/handoff shell；RCA 继续签发 owner receipt、typed blocker、visual/review/export verdict 和 artifact authority。裸 output 文件没有 valid manifest/receipt/evidence 时只能读为 orphan，不能完成 stage。
...
RCA-owned workspace/runtime apply surfaces 只写入或返回 workspace runtime refs、domain receipt refs、typed blockers、no-regression evidence、memory receipt refs、lifecycle mutation receipts、proof refs 或 external work-order closeout evidence；它们不签发 artifact-producing visual owner receipt，不能声明 visual ready、exportable、handoffable 或 production soak complete。`workspace_receipt_inventory_projection`、operator evidence readiness、efficiency handoff 和 substrate export 都是 body-free refs-only read/projection surfaces，供 OPL/App/operator/Agent Lab 索引 evidence gaps、efficiency refs 和 locator refs；它们不是 artifact gallery、workbench、production evidence producer 或 visual verdict owner。OPL-hosted path 只能消费 locator、projection、receipt refs、operator projection 和 repair hints，不能把 provider completion 或 stage metadata 升级成 RCA visual ready / exportable / handoffable verdict。真实 OPL Temporal controlled visual-stage long soak 当前仍未完成。
...
RCA 暴露给 OPL/App/operator/Agent Lab 的 operator evidence readiness、efficiency handoff 与 substrate adapter/export 都按同一规则读取：它们只聚合或导出 workspace/source/artifact/memory 的 locator、index、lifecycle、operator evidence、efficiency suite 和 quality-floor refs。具体 projection source list、payload path、suite ids 和 accepted refs 回到 production acceptance contracts、product-entry manifest、runtime-program parts、source/tests 和 Agent Lab contracts。它们不导出 visual truth、layout/review/export verdict、deliverable artifact body、visual memory body 或 owner receipt authority；OPL 只能索引这些 refs 并路由回 RCA owner surface，不能把 refs 提升成 visual-ready、exportable、handoffable、memory accept/reject verdict 或 production soak completion。
...
- 非默认 executor 只在 OPL hosted refs、技术参考或 history/provenance 中出现；Temporal 是 OPL production online runtime 的必需 provider
...
## RCA-local executor 与 OPL hosted refs

RCA 仓内 runtime 只物化 `Codex CLI` concrete executor。`packages/redcube-runtime-protocol/src/executor-runtime.ts` 只构造 Codex execution model / descriptor；family runtime 直接调用 Codex caller，不再维护第二 backend 的 config、profile、descriptor、fallback、compatibility fail boundary 或 execution branch。

其他 executor 的选择、运行、重试、attempt ledger 与 receipt body 归 OPL hosted surface。RCA 只通过 `contracts/runtime-program/opl-hosted-executor-refs.schema.json` 接受 `opl_executor_adapter_receipt_ref` 与 `opl_hosted_executor_requirement_ref` 两个 opaque refs；该 schema 不复制 backend、provider、profile、fallback 或 receipt body。

当前执行链固定为：

`service-safe domain entry -> OPL stage attempt refs -> RCA visual route handler -> Codex CLI -> RCA artifact/review/export authority`

其中：

- OPL 持有 hosted executor selection、stage attempt runtime、attempt ledger、retry/dead-letter 与长期托管；
- RCA 持有 route family、profile / pack descriptor、visual-domain truth、review / publication projection、artifact authority 与 owner receipt；
- `fix_html` 继续复用 dependency recovery 与 `stop_after_stage` continuation，但只执行一次 RCA-local Codex route，不再探测环境变量或发起第二 executor pass；
- `ppt_deck`、`xiaohongshu`、`poster_onepager` 的受保护创作 stage 统一走 Codex structured generation；native helper 只负责 materialization / validation / export refs。

旧 external executor、proof lane 与 repo-local bridge 只按 `docs/history/`、stable legacy machine contract 或 no-resurrection guard 读取，不是 active runtime surface。
