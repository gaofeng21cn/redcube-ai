...
RCA manifest / lock 不声明 OPL Framework implementation dependency；OPL module workflow 在 checkout 内维护 `node_modules/opl-framework` 指向当前 resolved OPL root，因此 Temporal、E2B、OPL CLI 和通用 Framework 依赖只由 OPL 安装一次。
