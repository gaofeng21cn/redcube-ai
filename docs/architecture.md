...
Generic executor selection、hosted adapter、attempt ledger 与 receipt residency 归 OPL owner surface。RCA 通过 `contracts/pack_compiler_input.json#/source_refs/executor_policy_source_ref` 指向 `contracts/stage_control_plane.json#/stages/*/selected_executor`，只消费 opaque refs，不维护本地 executor-routing schema/config、backend registry、Hermes adapter/proof 或第二执行器编排。
