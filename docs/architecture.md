...
RCA PPT professional skill 包现在把可复用方法压到每个 `SKILL.md` 的最小模板资源：serial pipeline、spec lock、progressive disclosure、style boundary、visual QA 与 editable PPTX grammar 都是 skill-local 方法模板，不是 runtime route、artifact body 或 review/export verdict。`contracts/capability_map.json#/dry_run_token_mapping_check` 与 `contracts/agent_lab_handoff.json#/dry_run_token_mapping_check` 提供非 live token 映射检查，证明 `ppt_visual_density`、`native_pptx_editability` 等反馈 token 只能路由到 professional skill refs 和 patch candidates，不能写 visual truth、runtime state、owner receipt 或 quality/export verdict。
