...
- artifact-producing 主链路证据必须通过 RCA product-entry route handler 进入 `run_deliverable_route`，即 `npm run --prefix /Users/gaofeng/workspace/redcube-ai redcube -- product invoke ... --task-intent run_deliverable_route`；AgentLab / OPL Meta Agent 只证明 refs-only、control-plane、takeover 或 handoff，不替代 route handler 产物证据。
...
- `screenshot_review` 对 image-first PNG 执行 16:9、非空、重复 hash、低信息密度、边缘裁切、标题区域裁切、碎片化、字段泄漏与可选 OCR domain_action_adapter 检查；缺 PNG、缺关键 manifest 或 hard-block 视觉 QA 时 fail-closed。
...
live 模式需要可用的本机 Codex executor imagegen 能力，不要求 RCA 显式配置 `OPENAI_API_KEY`、Base URL、Codex provider token 或 `REDCUBE_IMAGE_GENERATION_TOKEN`。CI `image-ppt-proof` job 只在 `workflow_dispatch`、nightly schedule 或 PR label `image-ppt-proof` 触发；默认质量 lane 不跑真实 image generation。
