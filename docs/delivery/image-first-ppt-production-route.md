...
live 模式需要可用的本机 Codex executor imagegen 能力，不要求 RCA 显式配置 `OPENAI_API_KEY`、Base URL 或 provider token。CI `image-ppt-proof` job 只在 `workflow_dispatch`、nightly schedule 或 PR label `image-ppt-proof` 触发；默认质量 lane 不跑真实 image generation。
