...
- 构图、层级、节奏、视觉峰值和盲评美观度；
- 进度优先：新增能力不能把无人工 gate 的 product-entry 自动推进改成外层逐 stage 手工操作。
...
- 无显式人工审阅时，product entry 仍一次 `invoke` 自动推进到 terminal review/export gate。
...
8. 文档、contracts、tests、current-program projection 与实际能力一致。
