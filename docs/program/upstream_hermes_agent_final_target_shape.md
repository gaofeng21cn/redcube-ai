...
- 让 future direct `RedCube Product Entry` 与 future `OPL Gateway` handoff 收敛到同一个 service-safe domain entry contract
...
- 成熟的最终用户产品入口仍未落地
- `OPL Product Entry -> OPL Gateway -> RedCube` 仍不是 repo-verified 的真实用户前台路线
- future `RedCube Product Entry` 目前只有 contract shell / adapter precursor，还没有独立 UI 或 direct-entry runtime shell
...
因此当前真正还没落地的，不是 runtime substrate proof，而是：

- mature 的最终用户 `RedCube Product Entry`
