...
- `export_pptx` 在 native 路线通过审查后复制 editable source PPTX，并在 export bundle 记录 `source_pptx`、`native_ppt_shape_manifest`、`native_ppt_repair_log`、source/final hashes、renderer proof、shape manifest summary 与 `native_export_bundle_operator_proof_summary_v1`
- `repair_pptx_native` 消费 `screenshot_review.slide_reviews` 的阻断页反馈，并把 target / preserved slide ids、blocked-slide source 与 repair scope 写入 repair log
...
- visual benchmark 已覆盖至少 6 页、多 layout、多内容类型、shape richness、字段不泄漏、PNG 非空 proof metrics，以及“不把 PNG 打进 PPTX”的回归风险
...
- `planned`：真实 product-entry native delivery smoke；可复现 proof runner 与手动 CI lane；native layout 视觉丰富度 benchmark；export/operator proof evidence；docs/contracts closeout。
- `done`：`tests/product-entry-native-ppt-live-proof.test.ts` 真实走 source readiness、create deliverable、`author_pptx_native`、`visual_director_review`、`screenshot_review`、`export_pptx`；`tools/native-ppt-proof/run.sh` 生成 editable PPTX/PDF/PNG/manifest/proof summary；`.github/workflows/ci.yml` 增加 `workflow_dispatch` 的 `native-ppt-proof` artifact lane；layout writer 增加可编辑 line/oval/rect 装饰结构与 richness metrics；export bundle 增加 operator proof summary 与 hashes。
...
- `verification`：targeted native runtime/layout/live proof、Python renderer unittest、CI/meta catalog tests、proof runner、`npm run typecheck`、`./scripts/verify.sh fast`、`npm run test:meta`、`npm run test:integration`、`git diff --check`。
- `commit-push state`：本轮所有 lane 应吸收回 `main`，最终 push 后删除临时 worktree/branch。
