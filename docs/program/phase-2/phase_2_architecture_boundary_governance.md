...
验收：`overlay-core` 不依赖 `@redcube/hermes-substrate`；现有 governance surface 输出由 gateway/runtime 注入 runtime topology projection。
...
验收：pack package 不表达 executor owner；runtime artifact 明确记录 provenance。
