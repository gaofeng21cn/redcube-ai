...
- 抽出 Codex runtime topology authority：`runtime-protocol` 持有默认 topology builder，`overlay-core` 不再依赖 `@redcube/hermes-substrate`，`hermes-substrate` 只保留兼容转发。
...
验收：`overlay-core` 不依赖 `@redcube/hermes-substrate`；Codex default runtime topology 由 `@redcube/runtime-protocol` 单一持有，substrate 只转发兼容 API。
...
把 `host_agent`、`hermes`、`codex_cli_json_output` 这类 executor/provenance 命名从 pack type 层迁到 runtime artifact provenance。pack 只保留 schema、pack-id、artifact type 和非创作边界。
