...
如果 workspace 里后续要做小红书图文，bootstrap 还会补 `.redcube/identity.json`。它是作者署名与品牌副标的唯一 workspace 配置；也可用 `REDCUBE_AUTHOR_PROFILE_ID`、`REDCUBE_SIGNATURE_DISPLAY`、`REDCUBE_SIGNATURE_SUBTITLE` 做显式进程级覆盖。
