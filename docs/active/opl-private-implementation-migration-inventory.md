...
Product-entry manifest 入口已继续收薄：OPL runtime inventory、task lifecycle、automation catalog、skill/operator shell 与 workbench/sidecar projection 组装已拆入 `packages/redcube-gateway/src/actions/get-product-entry-manifest-parts/shell-projections.ts`。`get-product-entry-manifest.ts` 仍保留 RCA domain refs、manifest 组装、visual route truth refs 和 authority refs；这只是源码边界收薄，不表示 OPL 已完全接管 generic session/workbench/sidecar primitive。
...
| Product-entry manifest and shell catalog | product-entry manifest/status/session projection | manifest orchestration、visual route truth refs、authority surface refs、artifact locator refs、domain handler refs；OPL shell projection 组装已拆入 manifest parts module | generated product-entry/status/session/workbench shell、skill/operator action catalog、generic session/workbench/sidecar primitive | OPL generated manifest shell 成为 active/default caller；direct route parity 和 no-compatibility-alias proof 成立；RCA 仅保留 domain refs |
