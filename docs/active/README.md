...
| `foundry-agent-os-target-delta.md` | 维护 RCA 对 OPL family `foundry_agent_os_standard` 的 target delta：上收到 OPL 的 generic surface、保留在 Visual Authority Kernel 的边界、`current_owner_delta` 默认读根和实施门 | active support |
...
- [RCA Foundry Agent OS 目标差异页](./foundry-agent-os-target-delta.md)
- [RCA 私有实现与 OPL 迁移台账](./opl-private-implementation-migration-inventory.md)
