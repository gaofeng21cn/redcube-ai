...
4. `naming_contract_hygiene`
   将历史 `managed`、generic session store、gateway/runtime/domain_action_adapter 读者可见语义继续降到 provenance / semantic-id / tombstone。任何改名必须先确认 active caller、runtime-program pointer、test contract 和 retired-alias no-resurrection policy。
