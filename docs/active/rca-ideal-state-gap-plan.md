...
- `当前实态`：RCA 已声明消费 OPL `family_scheduler_replacement`，没有发现类似 MAS local LaunchAgent scheduler 的默认运行 owner 残留；sidecar/action/status parity cleanup 与 native helper wrapper retirement 的当前 repo tranche 已落地。剩余差距集中在 OPL generic visual primitive handoff、thin adapter cleanup、真实 memory/lifecycle receipts、scaffold 抽取和 hosted visual-stage evidence。
- `最短路径`：继续推进 OPL generic visual primitives handoff、scaffold/template extraction、memory/lifecycle functional receipts 和 OPL App projection；sidecar/action/status parity 与 native helper package-module-only guard 作为后续新增 surface 的持续门禁。
- `验收顺序`：focused OPL-hosted controlled attempt 在功能上收和 cleanup 后执行；Temporal long soak 与 legacy physical cleanup 是最后生产验收和收尾。
...
2026-05-16 校准：当前 RCA 已把 scheduler 关系写成 thin consumer projection：OPL 持有 `family_scheduler_replacement`、family scheduler / daemon / generic lifecycle owner；RCA 不新增 generic scheduler。fresh sidecar evidence 显示 `npm run --silent build && node apps/redcube-cli/dist/cli.js product sidecar export --workspace-root /tmp/redcube-opl-doc-check --format json` 返回 `family_scheduler_replacement.owner=opl`、`consumer=redcube_ai`、`projection_mode=consumer_projection_only`、`rca_generic_scheduler_owner=false`、`rca_generic_daemon_owner=false`、`rca_generic_lifecycle_owner=false`，且 `managed_dag_scheduler_scope=visual_deliverable_internal_dag_only`。旧 gateway/frontdoor/federation/Hermes-first surface 在文档和 AGENTS 中已降为历史、内部边界或受控 runtime package 语境。RCA 仍保留大量功能性代码，但这些主要是 visual-deliverable domain package 的薄程序面和 artifact-heavy 实现：CLI/MCP、service-safe domain entry、runtime families、product sidecar、review/export gate、native helper envelope、artifact locator、receipt schema、tests 和 projection builder。它们不应被全部上收到 OPL；OPL 应上收的是 provider runtime、queue、generic transition runner、workspace/source intake shell、artifact gallery/handoff shell、review/repair transport、observability/SLO 和 App/workbench shell。
...
后续完善顺序必须先处理已明确的功能面上收、adapter thinning、目录范本抽取和旧面清理，再做真实 hosted visual stage / Temporal long soak。OPL-hosted attempt、no-regression evidence 和 long soak 是验收手段，不应成为 artifact gallery / review repair / source intake / native helper envelope / observability 这些已知功能上收的前置条件。
...
| OPL-hosted controlled stage | RCA 已提供 OPL discovery、stage control projection、product sidecar dispatch、owner receipt / typed blocker / no-regression evidence ref 合同，并通过 repo-local focused proof 证明 OPL 可消费 `emit_no_regression_evidence` / `emit_domain_owner_receipt` 的 return shape 和 refs。 | OPL generic visual primitive handoff、thin adapter cleanup、真实 OPL-hosted controlled visual stage long soak 和真实 artifact-producing domain owner receipt 尚未作为完成证据跑出。 | 先完成 source/artifact/review/helper/observability 等通用外围上收和 adapter thinning，再由 OPL provider 发起 focused hosted attempt，RCA 产出 domain receipt 或 typed blocker / no-regression evidence ref；最后扩展到 live soak，不把 OPL completion 提升为 RCA visual ready。 |
...
| Domain transition spec | RCA 已通过 `visual_transition_spec` 在 product-entry manifest、sidecar projection、runtime-program contract 和 focused tests 中声明 source/readiness 到 communication strategy、visual direction、artifact creation、review/repair、package/export 的 RCA-owned transition table、guard contract、oracle fixture 和 owner action；OPL 已新增 RCA visual transition ingestion adapter，可把该 surface 适配成通用 `family_transition_spec` / matrix cases，并在 `domain manifests` / `agents descriptor` 中只读投影 matrix ingestion；OPL family runtime 现在还能把 compatible `family_transition_matrix_result` hydrate 成 provider-hosted `family_transition/domain_tick` task，并把 transition bridge owner/no-regression/blocker refs 投到 runtime workbench / operator item；RCA repo-local fixture 也已覆盖 hosted attempt bridge 消费 `visual_transition_spec` 后只对账 `domain_owner_receipt_ref`、`typed_blocker` 或 `no_regression_evidence_ref` 的 return shape；本轮补强 provider-attempt ref 绑定和递归 forbidden payload guard，拒绝嵌套 completion claim、artifact blob、memory body 或 review/export verdict。 | OPL generic runner handoff、真实 provider-hosted transition attempt 的 retry/dead-letter、长时运行和 Temporal long soak 仍未作为运行证据产出；当前新增证据是 receipt 对账形状 fixture，不是 visual-stage production soak。 | 先把 `visual_transition_spec` 与 matrix result 稳定接到 OPL generic runner / workbench shell，并保持 RCA 只返回 next stage、repair action、typed blocker、domain owner receipt ref 或 no-regression evidence ref；随后再由真实 OPL provider attempt bridge 消费 transition result，验证 live receipt 对账，不生成 generic runner state，不携带 artifact blob、memory body 或 review/export verdict。 |
| Native helper / Python surface | Python helper catalog、runtime callsite 和 proof lane 已收敛到 package-module invocation；`ppt_deck_review.py`、`ppt_deck_export.py`、`ppt_deck_native.py` 与 `agent_loop_bridge.py` thin wrapper 已退役；no-active-caller / retired-surface guard 已落地。 | 真实 native PPT / review / export production proof 仍按显式 route 和 helper proof lane 后续扩展；wrapper 退役本身不是 production visual-stage soak。 | 继续用 package-module helper catalog、runtime callsite guard 和 proof lane 维护 helper 边界；不恢复 `script` / `compatibility_script` compatibility layer。 |
| New-agent template readiness | 新 OPL Agent 应能复用统一 owner-boundary、descriptor/stage/action/memory/artifact locator、sidecar/receipt schema、docs taxonomy 和 no-forbidden-write gate | RCA 已具备 canonical docs taxonomy、`agent/` anchor、runtime-program contract、product sidecar、deliverable family packages、plugin skill、native helper envelope 和 visual transition spec | RCA 物理目录高度服务 visual deliverables：`packages/redcube-runtime-family-*`、`prompts/ppt_deck|xiaohongshu`、`python/redcube_ai` 等不能直接泛化为所有新 Agent scaffold | 把可复用 scaffold 规则抽到 OPL template/checklist；RCA 保留 artifact-heavy domain implementation，不把已退役 helper wrapper 或 compat residue 当成 scaffold。 |
...
2. `opl_generic_visual_primitives_handoff`
   把 RCA-specific 的通用外围需求接到 OPL 已有或待建 read-model shell：workspace/source intake envelope、artifact gallery/handoff shell、review/repair transport、route/decision graph、native helper execution envelope、screenshot/export proof locator、attention queue、observability/SLO 和 App drilldown。RCA 保留 source readiness verdict、communication strategy、visual direction、review/export verdict、artifact mutation permission、memory body 和 owner receipt。
...
4. `scaffold_template_extraction`
   把 RCA 可复用的 agent 规则抽到 OPL template/checklist：docs taxonomy、descriptor/stage/action/memory/artifact locator、sidecar/receipt schema、visual transition spec 形状、native helper envelope、no-forbidden-write 和 runtime artifact 不进 source。RCA 保留 artifact-heavy domain implementation，不把 `packages/redcube-runtime-family-*`、`prompts/*`、`python/redcube_ai` 这类 visual-specific 目录当成通用新 Agent skeleton。
5. `memory_lifecycle_functional_receipts`
   `emit_domain_owner_receipt`、`apply_visual_memory_writeback`、`apply_visual_workspace_lifecycle` 和 `declare_visual_transition_spec` 已作为 RCA-owned callable surface / spec 落地。下一步先用真实 workspace 产出 accepted/rejected memory receipt、cleanup/restore/retention lifecycle receipt、artifact locator refs 和 visual transition typed blocker / owner receipt refs；memory body 和 canonical artifact 仍由 RCA-owned runtime/workspace store 持有，OPL 只消费 locator 和 receipt refs。
...
这些基础面仍不能升级成 production soak claim。后续缺口按顺序是：先完成功能上收、thin adapter、scaffold/template 抽取、真实 memory/lifecycle receipts 和 App projection；再跑 focused hosted controlled attempt；最后才跑 Temporal long soak。具体包括：
