...
- `当前实态`：RCA 没有发现类似 MAS local LaunchAgent scheduler 的默认运行 owner 残留；当前差距集中在 OPL generic visual primitive handoff、thin adapter cleanup、native helper wrapper cleanup、真实 memory/lifecycle receipts、scaffold 抽取和 hosted visual-stage evidence。
- `最短路径`：先做 sidecar/action/status parity cleanup 与 OPL generic visual primitives handoff，再做 native helper wrapper cleanup、scaffold/template extraction、memory/lifecycle functional receipts 和 OPL App projection。
...
2026-05-16 校准：当前 RCA 没有发现类似 MAS local LaunchAgent scheduler 的默认运行 owner 残留；旧 gateway/frontdoor/federation/Hermes-first surface 在文档和 AGENTS 中已降为历史、内部边界或受控 runtime package 语境。RCA 仍保留大量功能性代码，但这些主要是 visual-deliverable domain package 的薄程序面和 artifact-heavy 实现：CLI/MCP、service-safe domain entry、runtime families、product sidecar、review/export gate、native helper envelope、artifact locator、receipt schema、tests 和 projection builder。它们不应被全部上收到 OPL；OPL 应上收的是 provider runtime、queue、generic transition runner、workspace/source intake shell、artifact gallery/handoff shell、review/repair transport、observability/SLO 和 App/workbench shell。
...
1. `sidecar_action_status_parity_cleanup`
   先保持 direct product entry、CLI/MCP、service-safe domain entry、product sidecar、manifest、family action catalog 和 status 口径一致。新增或修改 action 必须从 canonical action/stage metadata 派生，进入同一命名体系，并同步 tests/status；旧 gateway/frontdoor/federation/Hermes-first alias 继续保持退役或 history 语境。
...
3. `native_helper_wrapper_cleanup`
   Python helper catalog 已声明 package module 为 preferred invocation；后续按 catalog 逐项迁 runtime callsite 和 helper tests，满足 no-active-caller / replacement proof 后删除或 tombstone thin script / compatibility wrapper，不保留 compatibility layer。
...
这些基础面仍不能升级成 production soak claim。后续缺口按顺序是：先完成功能上收、thin adapter、helper wrapper cleanup、scaffold/template 抽取、真实 memory/lifecycle receipts 和 App projection；再跑 focused hosted controlled attempt；最后才跑 Temporal long soak。具体包括：
