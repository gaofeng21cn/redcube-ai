...
- 本轮最新 source scan 确认：旧 managed runtime implementation 已不在 active source 中，但 `product-entry-session-snapshot-ref-adapter`、direct `runtimeWatch` read model、product sidecar guarded actions、operator evidence/stability projection、native helper envelope、MCP/CLI/product-entry wrapper 和 `@redcube/gateway` package boundary 仍是 active source。它们只能作为 refs-only adapter、domain handler target、native helper implementation、fixture、diagnostic 或 package/protocol boundary 读取；不构成 compatibility promise。generated/default caller parity、domain authority refs preserved、no-regression proof 与 provenance consumer migration 成立后，直接 rename/delete/tombstone，不新增 compatibility alias。
...
| operator evidence / stability projection | refs-only read model consumed by OPL/App。 | OPL observability/workbench shell 稳定后迁出 generic projection；RCA 只输出 owner receipt、typed blocker、artifact/memory evidence refs。 |
