...
| Native editable PPTX layout grammar | explicit route guard landed | `author_pptx_native` / `repair_pptx_native` 的 AI-authored shape plan 现在必须包含 `design_spec_lock`、`template_layout_grammar`、逐页 `template_layout_binding` 和 shape `layout_zone_id`；TS route guard 与 Python materializer / preflight 都 fail closed，防止 helper 代替 AI 选模板或补布局。剩余证明仍是 live Codex executor native sample、RCA visual director review、screenshot review、export proof 和生产级 soak，不能把 mock fixture 或 helper validation 写成 visual-ready。 |
