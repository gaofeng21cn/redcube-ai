...
- Live truth inputs: `contracts/foundry_agent_series.json`、`contracts/private_functional_surface_policy.json`、`contracts/physical_source_morphology_policy.json`、`contracts/runtime-program/current-program.index.json` 与 `contracts/runtime-program/current-program-parts/**`，以及当前 source/tests、CLI/MCP/product-entry behavior、artifact locator / review-export gate contracts 和 OPL/App read-model shape。`current-program.json` 只是 legacy read-through projection；Production acceptance refs 只用于 false-ready 边界或后置 evidence lane，不作为本文默认 worklist。
