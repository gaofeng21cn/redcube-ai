...
当前 RCA 的物理源码仍有 naming / package hygiene tail：`apps/redcube-mcp/src/server.ts` 仍是 repo-local protocol adapter 并暴露 product-entry grouped surface；`packages/redcube-runtime/src/product-entry-session-snapshot-ref-adapter.ts` 是 entry-session domain snapshot refs adapter，`product_entry_session_snapshot_refs_adapter` 仅保留为当前机器合同 semantic id；`packages/redcube-runtime-protocol/src/workspace.ts` 与 `runs.ts` 仍持有 workspace/run envelope helpers；`packages/redcube-gateway/src/actions/run-review-ref-projection.ts`、`product-sidecar-guarded-actions.ts` 和 product sidecar parts 仍承担 runtimeWatch、sidecar guarded action、operator evidence、dispatch orchestration 和 generated-surface mapping。2026-05-21 已把 `product-sidecar.ts` 的 dispatch 编排下沉到 `product-sidecar-parts/dispatch-orchestration.ts`，入口只保留 stable API、export projection 和 dispatch 委托；该变更是源码边界收薄，不声明 production evidence tail 或 generated/default shell 完成。上述路径当前是 domain handler target、refs-only adapter、native helper envelope 或 provenance，不是 RCA generic runtime owner。后续应通过 generated caller parity、contract-safe rename、tombstone 和 no-compatibility-alias 继续清理历史 runtime / gateway / session / sidecar / managed 命名。
...
2026-05-21 legacy active-reference cleanup 关闭了一项 naming/document-entry tail：旧 `managed_product_entry_hardening` support doc 已从 active reference 层删除，product-entry support 索引只保留 current session-continuity / direct / hosted entry support；active machine refs 改为 `human_doc:retired_managed_product_entry_contract_tombstone`，由 history tombstone 承担 provenance。产品/operational API 行为测试不再用旧 `get_managed_run` 或 managed supervision action 做 compatibility-only negative check，防复活只保留在 `rca-retired-surface-guard`、functional privatization audit 和 product sidecar deletion proof。

2026-05-22 naming hygiene tranche 继续把旧 surface id 从 active proof 主键降为 tombstone/provenance：`physical_deletion_guard.deleted_or_thinned_default_surfaces` 现在只列 `retired_*_tombstone` id，旧 `product_sidecar_dispatch.runtime_watch`、`product_sidecar_dispatch.product_entry_continuation`、`public_cli_mcp_gateway.get_managed_run` 和 managed supervision 字符串迁入 `retired_legacy_surface_ids` 或 `retired_no_resurrection_guards[].retired_legacy_surface_id`。product sidecar 仍 fail-closed 拒绝旧 action；测试只断言 machine contract、tombstone policy 与拒绝行为，不恢复 compatibility alias。该项压缩的是 naming/contract hygiene tail，不关闭真实 visual-stage long soak、owner receipt scaleout、memory/lifecycle receipt scaleout 或 final review/export verdict。

2026-05-22 session-continuity naming hygiene tranche 把 active product-entry manifest runtime、runtime inventory workspace binding、artifact locator contract、route-equivalence downstream truth、product-entry MVP behavior contract、current-program active baton leaf、OPL agent contract sync seed 和 focused product-entry/pack tests 的 active field 从 `session_store_root` 硬迁到 `session_continuity_root`，并把 active `session_store_contract` 字段迁为 `session_continuity_contract`。该迁移不改变 `$CODEX_HOME/projects/redcube-ai/runtime-state/product-entry-sessions/` 的真实目录，不新增 compatibility alias，不删除 `product_entry_session_store` tombstone semantic id，也不删除 forbidden `generic_session_store_owner` guard。它关闭的是 active reader-facing field naming tail；仍不关闭 OPL generated session shell、App workbench shell、Temporal visual-stage long soak、真实 owner receipt scaleout、memory/lifecycle receipt scaleout 或 final review/export verdict。

2026-05-22 domain-entry naming hygiene tranche 将 product-entry manifest 的 active `formal_entry.internal_surface` 改为 `domain_entry_protocol_boundary`，并用 `retired_internal_surface_ids=["gateway"]` 保留 provenance / tombstone 线索。Current-program active leaf 中 TypeScript owner 同步改成 `domain-entry protocol boundary`，避免 active owner 读面继续把 RCA 包边界误读为 gateway runtime owner。该项不改 `@redcube/gateway` 包名、不删除历史 refs、不恢复 compatibility alias；它只压缩 active reader-facing naming tail，仍不关闭 generated/default shell parity、Temporal visual-stage long soak、真实 owner receipt scaleout、memory/lifecycle receipt scaleout 或 final review/export verdict。

本轮证据不足、不能删除的 explicit remainder：
...
Fresh OPL stage evidence closeout 进一步读取 RCA `opl_expected_receipt_monitor_freshness_handoff` 后，6 个 RCA stage 的 expected receipt / monitor freshness 缺口均以 RCA-owned typed blocker refs 完成 OPL `record|verify`。OPL App/operator drilldown、framework readiness 与 production closeout 对 RCA stage tail 的口径已从 open 改为 `domain_owned_typed_blocker`，这关闭的是 OPL workorder accounting drift：production closeout 与 App/readiness 不再对同一 RCA blocker 给出不同 open 数。它不改变 RCA 剩余生产证据缺口：Temporal controlled visual-stage long soak、真实 memory/lifecycle receipt instances、cross-family repeated no-regression 和最终 visual/export/handoff verdict 仍必须由 RCA owner chain 或真实运行 receipt 关闭。

Fresh OPL domain-dispatch evidence closeout 进一步读取 RCA `remaining_evidence_gate_blockers` 后，`controlled_visual_stage_attempt` workorder 已通过 RCA-owned typed blocker `rca-typed-blocker:controlled-soak:temporal-long-soak-pending` 完成 OPL `domain_dispatch:redcube:sat_8623e55a341caf9672ed8ab2:record|verify`。该 OPL receipt 只记录 typed blocker、owner-chain refs、repeated no-regression refs 和 next verification command refs；它不携带 domain receipt success ref，不写 visual truth、review/export verdict body、artifact blob 或 memory body。Fresh worklist 中 RCA remaining dispatch workorder 从 7 条降到 6 条；这关闭的是 controlled-soak workorder accounting，不关闭 Temporal visual-stage long soak、真实 owner receipt instance、memory/lifecycle receipt scaleout、cross-family repeated no-regression production cadence 或最终 visual/export/handoff verdict。
