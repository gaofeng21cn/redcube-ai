...
- `当前实态`：RCA 已声明消费 OPL `family_scheduler_replacement`，并在合同和测试中显式声明不持有 generic scheduler、daemon、lifecycle、queue、attempt ledger、generic runner 或 workbench owner；sidecar/action/status parity cleanup 与 native helper wrapper retirement 的当前 repo tranche 已落地。剩余差距集中在 OPL generic visual primitive handoff、thin adapter cleanup、真实 memory/lifecycle receipts、scaffold 抽取和 hosted visual-stage evidence。
