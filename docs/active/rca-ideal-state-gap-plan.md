...
2026-05-18 fresh OPL generated interface descriptor 已 ready，`active_caller_target_proof.status=ready`；但这只表示 OPL 能生成 RCA CLI/MCP/Skill/product-entry/product-status/product-session/sidecar/OpenAI tool/AI SDK descriptor，并路由到 RCA domain handler target。它不能写成 production consumption 完成、active caller 已迁移、旧 wrapper 已退役或 RCA 已变成纯 visual knowledge pack。
...
1. `pack_compiler_generated_surface_production_consumption`
   由 OPL generated descriptor scope 生产消费 RCA visual pack，先关闭 generated surface production gap，再继续 wrapper migration。当前 descriptor ready 不等于生产默认唯一 caller 已迁走。
...
RCA 长期只保留无法声明化的 visual authority functions：source readiness verdict、communication/visual direction decision、review/export verdict、artifact mutation authorization、visual memory accept/reject、owner receipt signer、native helper implementation、typed blocker 和 safe action refs。

这些函数必须遵守 AI-first stage output 边界：故事、视觉方向、页面判断、review verdict 和 repair judgment 由 AI-authored stage artifact 承担；代码只做 validator、materializer、receipt signer 和 guard，负责 schema/locator/receipt 校验、artifact/receipt 物化、owner receipt 签发、forbidden-write 防护和 refs-only projection。OPL 可以保存 locator、metadata、provider receipt、domain receipt ref、typed blocker、repair hint 和 operator projection；不能写 RCA visual truth、memory body、review/export verdict 或 canonical artifacts。
...
- 不能把 OPL generated interface descriptor `ready` 写成 production caller migration、wrapper retirement、App/operator drilldown 或 workspace/source/lifecycle shell 已完成。
