...
| `managed` / `gateway` / `runtime` historical strings | 当前只允许 semantic-id、tombstone/provenance、negative input 或 package/protocol boundary。 | provenance consumer 可迁移时继续 rename/delete；不得恢复为 public entry、alias、facade 或 compatibility command。 |
