...
- `getProductEntrySession` session snapshot adapter、`runtimeWatch`、product sidecar guarded actions、operator evidence/stability projection：当前均有 direct / OPL generated shell / product sidecar active caller，角色是 refs-only adapter 或 domain action target；退役门仍是 generated caller parity、domain authority refs preserved 和 no-regression proof。
