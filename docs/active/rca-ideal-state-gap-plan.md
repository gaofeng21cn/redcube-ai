...
| Registry source single-source | `registry_loader_source_single_owner_landed` | `packages/redcube-runtime-family-registry/src/index.ts`、`packages/redcube-overlay-registry/src/index.ts`、registry/package-surface tests；default module lists now live beside the source loaders instead of being duplicated in package manifest metadata. |
