...
| 维度 | 当前状态 | 读法 |
| --- | --- | --- |
| 功能/结构差距 | `legacy_generic_runtime_closed_strict_purity_pending` | `functional_structure_gap_count=0` 只说明旧 generic runtime / shell owner 问题已分类并删除核心 managed runtime；stage control plane 已具备 OPL standard Progress-First delta policy 与 `family-stall-lineage.v1` typed blocker lineage policy，fresh OPL conformance / stage readiness 无 hard blocker；active repo-local product/session/domain_action_adapter/runtimeWatch/operator projection/neutral route-run record adapters 仍需 OPL default caller cutover 和删除/收薄。 |
| 生产证据 | `native_sample_review_export_closed_with_soak_gap` | `contracts/production_acceptance/rca-production-acceptance.json` 已记录 body-free artifact-producing owner receipt refs，并暴露逐 work item / 逐 stage 的 refs-only payload summary；2026-05-28 fresh `domain-handler dispatch/export` 已在 6 个用户级 runtime-state workspace 写出并聚合 36 个 refs-only receipt instances，`receipt_kind_coverage_ready=true`；2026-05-27/28/30 已有 6 条 RCA-owned no-regression refs 被 OPL ledger 记录/验证为跨 route / 跨窗口 refs-only evidence，当前 receipt 为 `opl://external-evidence/redcube_ai/rca-cross-family-repeated-no-regression-20260530-6-refs`。2026-05-29 native editable PPTX live lane 先暴露并收回 route timeout、schema、capacity、slot-count、input-hub content-depth、小 hub、横向 bus、错位 ticks 与 line 假箭头 blockers；`native-ai-first-live-20260529-v5` 在 `2026-05-29T17:42:07Z` 完成真实 `native_visual_sample=true` route：live Codex executor 输出 `native_visual_sample_compact` editable shape plan，Python native helper 物化 native PPTX/PDF/shape manifest/repair log/render screenshot，`visual_director_review` pass，`screenshot_review` pass，`export_pptx` 产出 publish bundle、published PPTX/PDF、presenter notes、交付成果和 artifact gallery，typed blockers 为空。该 run 的 input hub 为 `10.4in x 0.86in`、`22pt`，文案 `同一材料同步进入三条路线验证`，三条 `kind=connector` 竖直箭头分别对齐三张 route card center，preflight `failure_count=0`。`real-route-evolution-20260529T071105Z-74205973` 进一步证明 mock provider/test double 与 hard `expected_slide_count=1` planning contract 对齐：mock native lane 跑满 8 个 route 到 export，`detailed_outline`、`slide_blueprint` 与 native bundle 都保持 1 页，typed blockers 为空。相邻 rerun `rca-native-live-20260529-hub-arrow-v5` 的 `probe-output-current/real-route-evolution-probe.json` 当前为 `blocked`，只能作为 template-layout-grammar repair tail 读取，不能作为完成证据。`workspace_receipt_scaleout_claimed=false`、`declares_production_soak_complete=false`、`repeated_no_regression_claimed_as_soak=false` 仍保持；这关闭 native sample materialization/review/export proof lane、hub/connector regression、mock hard-count regression 和 6-ref no-regression refs-only ledger proof，不关闭 Temporal controlled visual-stage long soak、production-like repeated visual-stage no-regression、generated/default caller thinning 或全局完成。 |
| 命名/合同卫生 | `active_hygiene_tail` | `managed`、`gateway`、`runtime`、`session`、`domain_action_adapter` 等历史词只能按 semantic-id、tombstone、refs-only adapter、domain handler target 或 package/protocol boundary 读取；production evidence scaleout active surface 不再暴露 `legacy_payload_field_aliases`，`domain_receipt_refs` / `no_regression_refs` 只作为 OPL 当前 refs payload path 保留，不写成兼容映射。 |
...
1. `production_evidence_tail`
   用低风险真实 workspace 继续跑 `ppt_deck`、`xiaohongshu` 或 native sample route，从 OPL-hosted attempt 或 generated continuation shell 进入 RCA service-safe domain entry，继续累计 `typed_blocker`、`no_regression_evidence`、review/export refs 和 scaleout/long-soak 证据。artifact-producing owner receipt 槽位已由 production acceptance 的 body-free RCA-owned receipt refs 关闭；6 条真实 no-regression refs 已进入 production evidence surface 和 OPL refs-only ledger。`production_evidence_tail_workorder` 继续把 owner-chain apply、memory/lifecycle receipt scaleout、Temporal long-soak 和 cross-family no-regression 排成 refs-only typed-blocker work items，`owner_payload_item_summary` 进一步给每个 work item 暴露 success refs path、domain-owned typed blocker path 和空 payload template，供 OPL/App/operator 接续真实证据，不作为 readiness 声明。
...
4. `naming_contract_hygiene`
   将历史 `managed`、generic session store、gateway/runtime/domain_action_adapter 读者可见语义继续降到 provenance / semantic-id / tombstone。Production evidence scaleout 的 `legacy_payload_field_aliases` 已从 active source/contract/generated manifest 移除，并由 focused guard 防复发；任何后续改名必须先确认 active caller、runtime-program pointer、test contract、retired-alias no-resurrection policy 和 `source_ref_integrity_gate`；active surface 分类清单的 repo path / directory / anchor 不能悬空。

   当前 `source_ref_integrity_gate` 已覆盖 active code source 的 legacy-term allowance：focused guard 会阻断未被对应 `legacy_name_allowance` 覆盖的 active `managed`、`runtime`、`gateway`、`session` 或 `domain_action_adapter` wording。`visual_authority_functions` source refs 已收窄为实际 visual authority / source truth / executor routing files、Python native helper package 和 native-helper catalog；continuity adapter 保持在 refs-only adapter 分类中。
...
- `contracts/physical_source_morphology_policy.json` 的 active surface `source_refs` 与 `machine_boundary_refs` 必须解析到真实 repo path / directory / anchor；active code source 中的 legacy terms 必须被对应 `legacy_name_allowance` 覆盖。悬空 ref 或未分类 active legacy wording 会重新打开 naming/source hygiene gap，不得继续把分类清单读成可信 closeout。
