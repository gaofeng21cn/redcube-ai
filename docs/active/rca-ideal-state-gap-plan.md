...
- `product_sidecar_dispatch.product_entry_continuation`、`public_cli_mcp_gateway.get_managed_run` 与 retired managed supervision 字符串：只作为 no-resurrection guard、physical deletion proof 或 negative sidecar dispatch guard 存在；后续若要去掉这些字符串，需要先替换 guard 语义为更抽象的 tombstone ID，而不是恢复或兼容旧 action。
...
2026-05-21 新增的 `temporal_autonomy_readiness` 把“RCA 是否有长时间自治 / Temporal 在线管理能力”的判断从 prose 收到机器面：manifest 与 sidecar 均可读到 `can_be_opl_temporal_hosted=true`，但 `long_time_autonomy_claimed=false`、`production_visual_stage_long_soak_complete=false`。这意味着 RCA 作为 domain agent 的标准 OPL/Temporal 托管接口已具备，缺口集中在真实 provider-hosted visual-stage long soak：Temporal provider production residency、worker restart/resume/re-query、retry/dead-letter repair projection、artifact-producing owner receipt 和 cross-family no-regression 必须以真实 refs 关闭。
