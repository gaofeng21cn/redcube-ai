...
`agent/stages/manifest.json` 是 OPL compiler 所需的 canonical repo source，承载六个 stage、repo/action/transition refs 和 authority boundary；OPL 将其生成 `opl-generated:family_stage_control_plane`。RCA 已删除 root stage-control mirror，这些结构验证不授予 OPL visual truth、review/export 或 owner-receipt authority。
...
| 命名/合同卫生 | `canonical_package_identity_closed_hygiene_tail_open` | OPL canonical agent/package id 均为 `rca`；`contracts/opl_agent_package_manifest.json`、`contracts/domain_descriptor.json` 与 Stage Artifact adoption contract 使用同一 package identity，不保留 package alias。`redcube_ai` 继续是 authority owner 与 generated stage control plane target domain，`redcube-ai` 只保留 repository、语言实现包、Codex plugin/skill carrier 与路径 locator 角色。`contracts/foundry_agent_series.json` 仍只 pin OPL canonical policy/skeleton/shared release 并保留 RCA visual delta refs；generic series profile、workspace topology、public spine 与 feedback policy body 全部由 OPL owned/generated/hosted surface 持有。`managed`、`gateway`、`runtime`、`session`、`domain_action_adapter` 等历史词仍只能按 semantic-id、tombstone、refs-only adapter、domain handler target 或 implementation package/protocol boundary 读取。 |
...
| --- | --- | --- |
| Standard OPL Agent structure | done, standard OPL roots after cutover | `agent/stages/manifest.json`、`contracts/action_catalog.json`、refs-only `contracts/pack_compiler_input.json`、`contracts/runtime-program/current-program.index.json`；OPL generic stage plane 由 `opl-generated:family_stage_control_plane` 持有 |
...
| Stage artifact kernel adoption | done structurally | `contracts/stage_artifact_kernel_adoption.json` 与 `contracts/artifact_locator_contract.json` 固定 Stage Folder role manifest / receipt 接口；stage graph source 为 `agent/stages/manifest.json`，generated plane 为 `opl-generated:family_stage_control_plane`。 |
| Cognitive stage pack adoption | done structurally, standard root | `contracts/cognitive_kernel_adoption.json`、`contracts/golden_path_profile.json`、`agent/tools/domain_affordances.md` 与 `agent/stages/manifest.json` 保留 RCA visual refs；完整 generic policy/body 由 `opl-generated:family_stage_control_plane` 持有。 |
...
| Operator evidence shell retirement | `done_structurally` | `get-product-entry-manifest-parts/**` 已删除；`getProductEntryManifest` 只返回 domain evidence、typed blocker、receipt/artifact locator refs | Agent Lab、monitor、ledger、scaleout、tail workorder 与 workbench shell 归 OPL generated/hosted surface。 |
...
| Product-entry retained static export thinning | `retired` | repo-local manifest-parts policy/shell builders 已物理删除；仅保留 thin domain authority adapter。 |
...
| Product-entry manifest mapper reuse | `retired_with_generic_shell` | generic mapper/shell catalog 已删除，不再作为 RCA-local owner。 |
...
| Codex executor / author identity thinning | `codex_executor_config_tail_thinned` | `packages/redcube-runtime/src/executors/index-parts/batch-session-pool.ts`、`packages/redcube-runtime/src/executors/codex-caller.ts`、`packages/redcube-runtime/src/xiaohongshu-author-profile.ts`、focused executor/source-intake tests | Codex batch execution no longer accepts a fake `sessionPool` descriptor / reuse strategy knob while the real exec surface is `--ephemeral` per stage. The former multi-layer `redcube-config` package is retired; Xiaohongshu author identity now reads one workspace `.redcube/identity.json` plus explicit environment overrides. |
...
| Standard stage pack v2 launch gate | done structurally, OPL-owned generic runtime | `agent/stages/manifest.json` 提供 6 个 visual stage 的 RCA source refs，`opl-generated:family_stage_control_plane` 持有 generic runtime projection；RCA 无 root plane mirror。 |
...
| Large machine contract snapshots | done structurally | `contracts/runtime-program/opl-family-contract-adoption.json` 只保留 canonical/generated refs；stage graph source 为 `agent/stages/manifest.json`，generic plane 为 `opl-generated:family_stage_control_plane`，RCA root 不复制 generic body。 |
...
- `repo_local_adapter_delete_after_cutover`：RCA active source 中的 product-entry/session/internal domain_action_adapter implementation refs、operator evidence/stability projection、neutral route-run append-only event refs / route-run refs adapter 和 old compatibility path 在 no-active-caller、owner receipt/typed blocker roundtrip、direct/hosted parity 与 no-forbidden-write proof 成立后删除或收薄到 domain handler target。`runtimeWatch` 已收薄为 visual review/artifact/blocker/owner evidence refs projection，并拒绝 generic run input；`loadRouteRun` / runtime `loadRun` lookup public tail 与 `readRouteRunEvents` / runtime `readEvents` local event-log read tail均已退役。
...
- `generic_surface_thinned_only_after_gate`：session、operator evidence/stability projection、domain_action_adapter descriptor、neutral route-run record、product/status/workbench wrapper 只有在 OPL default caller parity、no-active-caller、RCA owner receipt/typed blocker roundtrip、no-forbidden-write proof 和 tombstone/provenance pointer 同时成立后，才能删除或收薄。`runtimeWatch` 已达到本项目标：通用 body 删除，保留的 visual refs projection 是 RCA domain handler surface，不是 generic wrapper。
...
| --- | --- | --- |
| generated/default caller thinning | RCA repo-local product/session/domain handler/operator projection/route-run append-only event refs / route-run refs adapters 只能作为 refs-only adapter、domain handler target、native helper implementation 或 migration input；`loadRouteRun` / runtime `loadRun` lookup public tail、`readRouteRunEvents` / runtime `readEvents` local event-log read tail，以及 `getRun` / `redcube runs get` 均已物理退役。`runtimeWatch` 已只保留 visual review/artifact/blocker/owner evidence refs，并把 generic status/attempt/telemetry/lifecycle/resumable 路由给 OPL Console / Runway / Ledger。OPL generated/default caller parity 成立后继续收薄或删除其他 generic wrapper。 | 私有实现迁移台账、`contracts/private_functional_surface_policy.json`、source/tests |
...
| --- | --- | --- | --- |
| `product-entry` / `session` / `status` / operator projection / `domain_action_adapter` wrapper tail | `packages/redcube-domain-entry/src/actions/get-product-entry-manifest*`、session / manifest / runtime-domain-action tests 和 CLI/product-entry help 仍消费这些 refs-only projection、domain handler target 或 migration inputs。`runtimeWatch` 不再属于该 generic tail。 | OPL generated/default CLI、MCP、product-entry、status、session、domain_action_adapter、workbench shell。 | OPL generated/default caller parity、no-active repo-local caller、RCA owner receipt / typed blocker roundtrip、no-forbidden-write proof、retired-alias no-resurrection 和 tombstone/provenance pointer 同时成立。 |
...
| Executor policy refs | `opl-generated:family_stage_control_plane#/stages/*/selected_executor` 是 OPL generated policy ref；repo source 为 `agent/stages/manifest.json`。 | OPL Agent Executor Adapter / attempt ledger / runtime record owner。 | 保持 refs-only consumption、no-active RCA adapter caller、no-silent-fallback 和 RCA visual authority boundary。 |
