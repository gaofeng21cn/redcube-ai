...
| Visual memory lifecycle | runtime receipt refs visible | 3 个用户级 runtime-state workspace 已通过 `emit_workspace_receipt_proof` 写出 6 条 accepted/rejected visual memory writeback receipt refs、9 条 cleanup/restore/retention lifecycle receipt refs 和 3 条 domain owner receipt refs；这些仍是 body-free refs-only receipt instances，不写 memory body、artifact blob 或 review/export verdict。 |
| Workspace receipt scaleout | refs visible, claim still false | `domain-handler export --workspace-receipt-scaleout-root ...` 读到 `observed_workspace_count=3`、`observed_receipt_count=18`、`receipt_kind_coverage_ready=true`、`missing_receipt_kinds=[]`；机器面仍保持 `workspace_receipt_scaleout_claimed=false` 与 `declares_production_soak_complete=false`，所以不能写成 scaleout complete、visual ready 或 production soak。 |
