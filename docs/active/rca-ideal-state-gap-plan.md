...
最小 `/goal` workflow 的当前完成口径是 `agent_lab_external_suite_passed_with_mock_artifact_export_smoke`：OPL AgentLab 可以直接运行 RCA suite，OMA 可以读取 `contracts/agent_lab_handoff.json` 生成 no-patch / owner-gated closeout 记录，product-entry 可以从 `opl_hosted` + `run_opl_stage_execution_plan` + `auto_to_terminal` 一次性启动 xiaohongshu workflow；focused test 还用 mock image provider 跑到 `export_bundle`，确认 PNG、publish bundle、caption、publication-state 和 series reports 已落盘。这个口径关闭“重构后是否还能自动从目标入口推进到可检查 artifact/export 证据”的 smoke gap，不关闭 live image provider、production evidence tail，也不授权 OPL/OMA 写 RCA visual truth、artifact body、memory body、review/export verdict 或 owner receipt。
