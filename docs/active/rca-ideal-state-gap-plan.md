...
| Cognitive stage pack adoption | done structurally | `contracts/cognitive_kernel_adoption.json`、`contracts/golden_path_profile.json`、`agent/tools/domain_affordances.md` 与 compact `contracts/stage_control_plane.json` 的 6 个 visual stage 已声明 `skill_refs`、`tool_refs`、`strategy_refs` 和 per-stage policy/body readback refs；完整 `tool_affordance_boundary`、`candidate_pool_policy`、`independent_gate_policy`、`handoff_policy` body 由 product-entry manifest generated descriptor 提供；`contracts/pack_compiler_input.json#/cognitive_stage_pack_contract` 固定 refs-only compiler input，明确不改变 PPT 用户流程、不作为 launch hard gate、不授权 OPL 写 visual truth、artifact body、review/export verdict 或 owner receipt。 |
...
| Line-budget source split | `strict_line_budget_structural_split_landed` | `packages/redcube-domain-entry/src/actions/family-stage-control-plane-parts/user-stage-log-contract.ts`、`packages/redcube-domain-entry/src/actions/domain-action-adapter-parts/physical-source-morphology-policy-parts/source-ref-integrity-gate.ts`、`node --experimental-strip-types scripts/line-budget.ts --strict` | Two over-budget domain-entry files are split by moving existing pure contract / gate helpers into local parts modules. This is a structural line-budget closeout only; it does not claim visual ready, exportable, handoffable, domain ready, production ready, default-caller cutover or physical delete authority. |
| Codex plugin legacy alias cutover | `root_manifest_canonical_path_landed` | `.codex-plugin/plugin.json`、`plugins/redcube-ai/.codex-plugin/plugin.json`、`plugins/redcube-ai/skills/redcube-ai/SKILL.md`、`scripts/install-codex-plugin.ts`、`tests/codex-plugin.test.ts` | Root developer manifest now reads canonical `plugins/redcube-ai` skill/assets paths directly; `plugins/rca` and `plugins/redcube-ai/skills/rca` symlink aliases are removed from the repo-local plugin scaffold. This is a Codex plugin source-channel cleanup only; `rca-local` marketplace ownership remains OPL-owned and this does not claim visual ready, exportable, handoffable, domain ready or production ready. |
...
| Type-only pack package collapse | `pack_type_boundary_collapsed` | `packages/redcube-runtime-family-{ppt,xiaohongshu,poster-onepager}/src/types.ts`、`tests/pack-first-completion.test.ts`、`tests/typescript-service-boundary-cases/package-dependency-boundary.test.ts`；the three `packages/redcube-pack-*` package directories have been removed. This closes the package-boundary overdesign only; pack ids remain strings and this does not change visual route truth or owner authority. |
...
| Runtime helper owner-local reuse | `runtime_helper_owner_local_reuse_landed` | `packages/redcube-runtime/src/runtime-utils.ts` owns repeated source-augmentation / source-readiness / route helper `safeText` / `ensureDir` / JSON read helpers; `packages/redcube-runtime-protocol/src/protocol-utils.ts` owns Stage Folder / screenshot / source-truth / Python helper helpers; PPT family reuses existing `packages/redcube-runtime-family-ppt/src/ppt-deck-runtime-family-parts/core-helpers.ts`. Remaining raw helper names are helper owners, nullable/readback variants such as `readJsonIfPresent` / `readJsonIfExists`, response parsers, or package singletons where a new util file would add coupling without net simplification. This is owner-local source hygiene only, not a new cross-repo utility package or a grep-zero mandate. |
...
| Standard stage pack v2 launch gate | done structurally | `contracts/stage_control_plane.json` 中 6 个 visual stage 均声明 `user_stage_log_contract_ref`、`progress_delta_policy_ref` 和 `typed_blocker_lineage_policy_ref`；完整 policy body 由 product-entry manifest generated descriptor readback 提供。OPL installed runtime 对 RCA conformance 读为 passed，stage readiness 读为 `launch_warning` 且 hard blocker 为 0。 |
...
| Large machine contract snapshots | done structurally | `contracts/runtime-program/opl-family-contract-adoption.json` 已收为 compact aggregate，只保留 canonical contract refs / generated product-entry readback refs；`contracts/stage_control_plane.json` 已收为 compact static contract，只保留 stage ids、core refs、authority boundary 与 generated descriptor refs。完整 machine body 回到 root contracts 或 product-entry manifest readback，不在 adoption / static stage root 重复复制。 |
