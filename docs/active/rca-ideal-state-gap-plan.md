...
后续推进只按 owner-delta 读取，不保存可直接复制的 Agent prompt。每条工作都必须从当前 RCA `agent/` pack、contracts、runtime-program current-program、source/tests、product-entry manifest、domain handler target、production acceptance、workspace/runtime receipts、artifact locator、review/export gate、OPL/App fresh read-model、Temporal/OPL provider refs、owner receipts 和 typed blockers 取 live truth；dated native proof、workspace path、attempt id、screenshot path、no-regression ref 流水和 proof 命令摘要只读 `docs/history/**` 或 runtime/evidence ledger。
