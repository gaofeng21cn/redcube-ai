...
- `contracts/physical_source_morphology_policy.json` 的当前保护口径是 role whitelist / forbidden owner flags：active callable path 只能落在 domain handler target、refs-only read model、native helper、repo-native verification wrapper、package protocol boundary 等当前角色；compatibility alias、generic gateway / session / runtime / generated-wrapper owner 继续 fail closed。
- `contracts/physical_source_morphology_policy.json` 的 payload guard 只允许 forbidden payload / receipt role 字段表达禁止项，并声明 `retired_field_ids_as_json_keys_allowed=false`；guard tests 会阻断 compatibility alias 回到 active payload template、success payload、JSON key 或 readiness claim。
