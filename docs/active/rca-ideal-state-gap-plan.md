...
| Native editable PPTX layout grammar | structural feedback improved, live sample still blocked | `author_pptx_native` / `repair_pptx_native` 的 AI-authored shape plan 现在必须包含 `design_spec_lock`、`deck_layout_rhythm_plan`、`template_layout_grammar`、逐页 `template_layout_binding` 和 shape `layout_zone_id`；TS route guard 与 Python materializer / preflight 都 fail closed，阻断三页连续复用同一 archetype / primary grid、低 composition 多样性、archetype catalog 只有 ID、selected archetype 未被 required role groups / required-zone coverage 兑现、shape 坐标逃出 declared zone、以及 panel 内安全边距问题，并把 `ai_first_text_panel_safe_area_violation` 回传为带 `panel_shape_id` / `required_inset_in` 的 exact retry contract，防止 helper 代替 AI 选模板、补布局或静默忽略 panel 内边距。2026-05-29 live probes 已确认缺失 `template_layout_binding`、非法 zone safe inset 和最终 Python preflight 失败都会保留 candidate / validation attempt refs；最新 live route 仍在 `ai_first_shape_outside_template_layout_zone`、`ai_first_text_panel_safe_area_violation`、`ai_first_content_depth_too_low`、`ai_first_page_number_missing` 与 text capacity/readability gate 处阻断。剩余证明仍是 live Codex executor native sample、RCA visual director review、screenshot review、export proof 和生产级 soak，不能把 mock fixture、helper validation 或 failure refs 写成 visual-ready。 |
