...
当前 fresh OPL generated interface descriptor 已 ready，但只表示 OPL 能生成 RCA CLI/MCP/Skill/product-entry/product-status/product-session/sidecar/OpenAI tool/AI SDK descriptor，并路由到 RCA domain handler target。它不能写成 production consumption 完成、active caller 已迁移、旧 wrapper 已退役或 RCA 已变成纯 visual knowledge pack。
