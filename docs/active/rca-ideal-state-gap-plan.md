...
- `contracts/physical_source_morphology_policy.json#/default_caller_tail_readback/compact_retirement_summary` 与 `scripts/check-private-platform-retirement.ts --format json#/default_caller_tail_compact_retirement_summary` 是同一 current worklist 的 compact 读面：`cleanup_candidate_count=0`、`owner_delta_required=true`、`can_apply_cleanup=false`。它只让 OPL/App/operator 不必扫描全部 tail 明细即可 fail closed；不减少 physical delete / further thinning 所需 owner evidence，也不授权 ready claim。
- `contracts/physical_source_morphology_policy.json#/default_caller_tail_readback/compact_retirement_summary/owner_delta_work_order_pack` 与 `scripts/check-private-platform-retirement.ts --scope default-caller-tail --format json#/owner_delta_work_order_pack` 现在把 8 个 retained tail 的 required evidence、owner-delta route 和 typed blocker ref shape 物化为 body-free work-order pack，方便 OPL/App/operator/OMA 一次读取下一步缺口。该 pack 只表达 owner-delta worklist，不执行 work order、不创建 typed blocker 实例、不签 owner receipt、不写 visual truth、不授权 physical delete、default-caller cutover、visual/export/handoff/domain/production ready。
