...
- `当前审计面`：RCA 已新增 `privatized_functional_module_audit`，把 managed-dag-scheduler、native helper envelope/wrapper、workspace receipt inventory、artifact gallery/handoff shell、review/repair transport、observability/stability read model 与 sidecar/status/action parity 收成 OPL 可读的机器矩阵。当前结论是 RCA 保留 visual-domain thin surface，没有新增 retire/tombstone candidate；OPL 后续上收的是 generic primitive 代码面和 App/workbench shell。
