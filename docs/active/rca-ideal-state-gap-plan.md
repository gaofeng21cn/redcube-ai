...
2026-05-19 后，`contracts/runtime-program/current-program.index.json` 与 `contracts/runtime-program/current-program-parts/**` 是 current-program 的 leaf-level canonical source；`current-program.json` 只保留为既有 consumer 的 generated read-through snapshot，并由 `runtime-program-provenance` 测试锁定每个 leaf ref 与聚合快照的 JSON pointer value 一致性。当前 index 声明 `canonical_truth_model=leaf_refs_are_canonical_current_program_sources`，大对象和数组按 `split_policy.max_leaf_json_line_count` 递归拆分。后续不再把巨型 `current-program.json` 或巨型 section 文件当成唯一编辑入口。
