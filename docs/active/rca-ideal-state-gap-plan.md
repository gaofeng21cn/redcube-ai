...
| Native editable PPTX layout grammar | explicit route guard landed | `author_pptx_native` / `repair_pptx_native` 的 AI-authored shape plan 现在必须包含 `design_spec_lock`、`deck_layout_rhythm_plan`、`template_layout_grammar`、逐页 `template_layout_binding` 和 shape `layout_zone_id`；TS route guard 与 Python materializer / preflight 都 fail closed，阻断三页连续复用同一 archetype / primary grid、低 composition 多样性、archetype catalog 只有 ID、selected archetype 未被 required role groups / required-zone coverage 兑现、shape 坐标逃出 declared zone、以及 panel 内安全边距问题，并把 `ai_first_text_panel_safe_area_violation` 回传为带 `panel_shape_id` / `required_inset_in` 的 exact retry contract，防止 helper 代替 AI 选模板、补布局或静默忽略 panel 内边距。剩余证明仍是 live Codex executor native sample、RCA visual director review、screenshot review、export proof 和生产级 soak，不能把 mock fixture 或 helper validation 写成 visual-ready。 |
