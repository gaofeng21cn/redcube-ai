...
| 命名/合同卫生 | `identity_split_guarded_hygiene_tail_open` | `contracts/foundry_agent_series.json` 已降级为 RCA domain-specific profile / refs anchor，不再持有 OPL Foundry generic identity hygiene body、series profile 或 public spine；`redcube_ai` 仍是 authority owner，`redcube-ai` 仍是 package / skill identity，generic Foundry series/profile 由 OPL owned/generated/hosted surface 持有。`managed`、`gateway`、`runtime`、`session`、`domain_action_adapter` 等历史词仍只能按 semantic-id、tombstone、refs-only adapter、domain handler target 或 package/protocol boundary 读取。 |
...
| --- | --- | --- |
| Standard OPL Agent structure | done, refs-only after cutover | `agent/`、refs-only `contracts/stage_control_plane.json`、refs-only `contracts/pack_compiler_input.json`、`contracts/runtime-program/current-program.index.json`；OPL generic stage control / pack compiler / Foundry series profile 不再由 RCA root contracts 维护 |
...
| Direct / hosted boundary | done structurally | product-entry manifest、minimal action target refs、RCA `domain-handler export|dispatch` target、OPL-generated `domain_action_adapter` descriptor refs、stage control projection；repo-local Foundry generic CLI spine 已退役 |
...
| Cognitive stage pack adoption | done structurally, refs-only root | `contracts/cognitive_kernel_adoption.json`、`contracts/golden_path_profile.json`、`agent/tools/domain_affordances.md` 与 refs-only `contracts/stage_control_plane.json` 只保留 RCA visual stage ids / refs / authority boundary；完整 generic policy/body 由 OPL generated product-entry manifest descriptor 或 OPL substrate 持有；`contracts/pack_compiler_input.json` 只固定 `agent/` pack refs 和 minimal authority refs，不授权 OPL 写 visual truth、artifact body、review/export verdict 或 owner receipt。 |
| Purpose-first adapter thinning | done structurally, evidence tail open | `contracts/foundry_agent_series.json#/purpose_first_adapter_thinning_policy` 与 focused pack contract tests 固定 retained surface 只能作为 declarative visual pack refs、domain handler target、minimal authority function 或 tombstone/provenance 读取；`managed/gateway/session/domain_action_adapter` 历史词不能成为 authority source、active public surface 或 generic owner surface。 |
...
| Line-budget source split | `strict_line_budget_structural_split_landed` | `packages/redcube-domain-entry/src/actions/domain-action-adapter-parts/physical-source-morphology-policy-parts/source-ref-integrity-gate.ts`、`node --experimental-strip-types scripts/line-budget.ts --strict` | The old stage-control local parts helper was retired during the OPL standard-surface cutover; the remaining split keeps the source-ref integrity gate out of the over-budget adapter file. This is structural source hygiene only; it does not claim visual ready, exportable, handoffable, domain ready, production ready, default-caller cutover or live evidence. |
| Codex plugin legacy alias cutover | `canonical_plugin_scaffold_only` | `plugins/redcube-ai/.codex-plugin/plugin.json`、`plugins/redcube-ai/skills/redcube-ai/SKILL.md`、`tests/codex-plugin.test.ts` | Root `.codex-plugin/plugin.json`、repo-local `scripts/install-codex-plugin.ts`、`plugins/rca` 和 `plugins/redcube-ai/skills/rca` alias 已退役；RCA repo 只维护 `plugins/redcube-ai/` canonical scaffold/source locator。Codex marketplace registration / install 由 OPL/plugin Flow owner 管，不声明 visual ready、exportable、handoffable、domain ready 或 production ready。 |
...
| CLI option parsing stdlib adoption | `node_util_parse_args_landed` | `apps/redcube-cli/src/cli-parts/options.ts` now uses `node:util.parseArgs` tokenization with the existing long-option map semantics preserved for unknown `--key value` options; `tests/cli-v2-smoke-cases/review-and-root-resolution.test.ts` locks `--workspace-root`, boolean, inline value and ignored short-option behavior. |
| Domain-entry local helper reuse | `domain_entry_action_utils_landed` | `packages/redcube-domain-entry/src/actions/action-utils.ts` now owns repeated `safeText` / `requireField` / JSON read helpers for the product-entry, session, fanout and domain-action action cluster. This is package-local reuse only; tests, scripts and other packages keep local helpers unless they share an existing package owner. |
...
| Retired/legacy guard density thinning | `retired_guard_density_thinned` | `tests/helpers/rca-retired-surface-guard.ts` keeps the shared active-source resurrection scan; `tests/rca-legacy-name-allowance.test.ts` and `tests/rca-retired-surface-active-guard.test.ts` now retain only machine-policy, no-resurrection and retired-substrate absence guards instead of per-surface wording/alias assertions. This is test-portfolio thinning only; it does not authorize retired surface resurrection or physical delete. |
...
| Current-program aggregate retirement | `current_program_aggregate_retired` | `contracts/runtime-program/current-program.index.json`、`contracts/runtime-program/current-program-parts/**`、`scripts/current-program/**`、`tests/helpers/current-program-contract.ts`、`tests/runtime-program-provenance.test.ts` | The former `contracts/runtime-program/current-program.json` aggregate is physically deleted. Current-program readers assemble from source parts/index, and `contracts:current-program:check` now fails if the aggregate reappears. This closes the second-truth machine snapshot, not visual/export/domain readiness. |
| Codex executor adapter surface | `codex_executor_adapter_test_surface_landed` | `tests/codex-executor-adapter.test.ts`、`scripts/test-registry.ts`、`scripts/run-test-group-lib.ts`、`scripts/check-private-platform-retirement.ts`、CLI help | The old `codex-cli-client` test/public wording surface is retired in favor of Codex executor adapter semantics. RCA still uses Codex CLI as the concrete executor, but the generic executor adapter/attempt ledger owner remains OPL. |
| Runtime OPL owner boundary | `runtime_opl_owner_boundary_landed` | `packages/redcube-runtime-protocol/src/executor-runtime.ts`、`executor-runtime-parts/route-run-records.ts`、`runtime-topology.ts`、`packages/redcube-runtime/src/default-registries.ts`、focused runtime tests | Runtime-family catalog, route-run records and executor models now carry explicit OPL generic owner fields and RCA refs-only route policy roles. This does not delete RCA visual route implementations or transfer visual truth/review/export authority. |
| Overlay surface helper consolidation | `overlay_surface_helper_consolidation_landed` | `packages/redcube-overlay-core/src/surface.ts`、overlay `surface.ts` files、`tests/typescript-service-boundaries.test.ts` | Repeated overlay bundle/path/validator shell logic is centralized in overlay-core while family-specific validators stay local. This removes duplicate helper code without creating a new generic runtime owner. |
| Standard stage pack v2 launch gate | done structurally, OPL-owned generic body | `contracts/stage_control_plane.json` 中 6 个 visual stage 已降级为 RCA stage refs、stage ids、artifact authority refs 与 OPL generated-stage-control owner boundary；`user_stage_log`、progress delta、typed blocker lineage 和 generic stage policy body 不再由 RCA root contract 维护。OPL installed runtime / generated substrate 的 live readiness 仍属于 OPL owner surface，不由本 refs-only root contract声明。 |
...
| Current-program source shape | done | `contracts/runtime-program/current-program.index.json`、`contracts/runtime-program/current-program-parts/**` 与收薄后的单一 aggregate bundle manifest metadata；`functional_privatization_audit`、OPL adoption 和 visual pack compiler handoff 等大型 surface 在 current-program 中只保留 `canonical_ref_only_no_body_copy` projection refs，不再复制完整 machine snapshot body。 |
| Large machine contract snapshots | done structurally | `contracts/runtime-program/opl-family-contract-adoption.json` 已收为 compact aggregate，只保留 canonical contract refs / generated product-entry readback refs；`contracts/stage_control_plane.json`、`contracts/pack_compiler_input.json` 和 `contracts/action_catalog.json` 已收为 RCA refs anchors。完整 generic machine body 回到 OPL generated/hosted substrate 或 product-entry manifest readback，不在 RCA root contracts 重复复制。 |
...
| Hermes-Agent proof backend | executor routing schema / runtime protocol fail-closed boundary | 仅为显式 opt-in / historical backend key。`owner=opl_runtime_manager` 与 `selected_executor_backend=hermes_agent` 分开表达；retired Hermes API / loop bridge implementation、Python helper catalog entry 和 `redcube_ai.hermes` package 已删除，本仓只保留显式 `hermes_agent` selection 的 fail-closed boundary。 |
...
1. `generated_default_caller_thinning`
   ...
   Retired Hermes API / loop bridge implementation 已满足 no-active-caller 删除门并物理退役：runtime-family 路由和 runtime descriptor 不再 import `generateStructuredArtifactViaHermesAgent*` / `readHermesAgentLoopContract`，`pyproject.toml` console script、Python helper catalog entry 和 `redcube_ai.hermes` package 已删除。剩余删除门只针对更广义的 `hermes_agent` policy / descriptor key：OPL Agent Executor Adapter、attempt ledger、runtime record/event log 和 stale attempt audit read model 成为 default caller，RCA route policy / executor receipt refs 保留，focused proof tests 迁移到 OPL-owned surface，no-active-caller scan 通过，并形成 RCA owner receipt 或 typed blocker。
