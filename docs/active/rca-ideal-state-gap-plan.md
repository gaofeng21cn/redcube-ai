...
| 物理源码形态 | `strict_source_purity_tail_gate_landed` | repo-local session / domain_handler / watch / operator projection / neutral route-run record adapter 仍可见，只能作为 refs-only adapter、domain handler target、native helper implementation 或 default-caller thinning tail；`contracts/physical_source_morphology_policy.json#/default_caller_tail_thinning_gate` 已把这些 retained tails 的 allowed roles、required delete/thin gates、no-resurrection guard 和 false-ready guard 机器化。该 gate 不授权 physical delete、OPL default caller parity、visual ready、exportable、handoffable、domain ready 或 production ready。 |
