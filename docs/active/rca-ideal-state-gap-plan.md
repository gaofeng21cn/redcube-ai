...
| Hermes-Agent proof backend | executor routing schema / runtime protocol client / helper catalog | 仅为显式 opt-in proof backend。`owner=opl_runtime_manager` 与 `selected_executor_backend=hermes_agent` 分开表达；它不是默认 runtime owner，也不是 RCA-owned generic executor runtime。 |
