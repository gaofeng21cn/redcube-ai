...
export function buildMockXhsStoryline(meta) {
  ...
  const mode = safeText(context?.framing?.mode) || 'single';
  const series = mode === 'series';
  return {
    mode,
    mode_decision: {
      result: mode,
      rationale: ['资料包含多个可独立解释的事实单元', '主题跨越认知、判断与行动', '读者需要分步完成多个任务'],
      thematic_units: series ? ['建立认知框架', '形成诊疗判断', '落实长期行动'] : ['完成一个读者任务'],
    },
    ...
    series_architecture: series ? {
      status: 'required',
      series_thesis: '从建立认知到形成判断，再落到长期行动',
      recommended_note_range: '3-6篇，按读者任务和证据单元拆分',
      chapters: [
        { chapter_id: 'C01', chapter_role: '建立认知', reader_question: '先理解什么', topic_units: ['定义与风险'], transition: '从认知进入判断' },
        { chapter_id: 'C02', chapter_role: '形成判断', reader_question: '如何判断和行动', topic_units: ['诊断、治疗与随访'], transition: '从判断进入长期行动' },
      ],
      note_briefs: [
        { note_id: 'N01', chapter_id: 'C01', working_title: '先把关键框架看懂', reader_question: '这件事到底是什么', content_scope: '定义、风险与常见误区', evidence_anchors: ['完整资料中的定义和风险事实'], estimated_pages: '8-10', transition: '下一篇进入判断路径', no_repeat_scope: ['不展开治疗与长期随访'] },
        { note_id: 'N02', chapter_id: 'C02', working_title: '判断路径怎么走', reader_question: '下一步应该如何判断', content_scope: '诊断、治疗决策与行动边界', evidence_anchors: ['完整资料中的路径和行动事实'], estimated_pages: '10-12', transition: '下一篇进入长期管理', no_repeat_scope: ['不重复基础定义'] },
      ],
      publication_arc: ['先建立共同认知', '再给判断路径和下一步行动'],
    } : {
      status: 'not_applicable',
      series_thesis: '单篇完成当前读者任务',
...
export function buildMockXhsPlan(meta) {
  ...
  return {
    series_context: {
      mode: safeText(meta?.context?.storyline?.mode) || 'single',
      note_id: safeText(meta?.context?.storyline?.series_architecture?.note_briefs?.[0]?.note_id) || 'single',
      series_role: safeText(meta?.context?.storyline?.series_architecture?.note_briefs?.[0]?.content_scope) || '完成当前单篇读者任务',
      previous_note: 'none',
      next_note: safeText(meta?.context?.storyline?.series_architecture?.note_briefs?.[1]?.note_id) || 'none',
      no_repeat_scope: safeArray(meta?.context?.storyline?.series_architecture?.note_briefs?.[0]?.no_repeat_scope).length
        ? safeArray(meta?.context?.storyline?.series_architecture?.note_briefs?.[0]?.no_repeat_scope)
        : ['none'],
    },
    title_options: [
      `${title}为什么很多人总抓不住重点`,
