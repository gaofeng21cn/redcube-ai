...
function assertGoalWorkflowSuiteShape(suite) {
  ...
  assert.equal(
    ...
    'tests/codex-executor-adapter.test.js#generateImageViaCodexNativeImagegen',
  );
  assert.equal(
    suite.artifact_sample_refs.includes(
  ...
  assert.equal(task.promotion_gate.regression_suite_refs.includes('tests/rca-goal-workflow-agent-lab-suite.test.js'), true);
