...
test('direct-delivery longrun target doc stays tracked alongside current program truth', () => {
  const currentProgram = JSON.parse(read('contracts/runtime-program/current-program.json'));

  assert.equal(existsSync(path.resolve('docs/direct_delivery_longrun_target_state.md')), true);
