...
import { createXiaohongshuDeliveryParts } from '../packages/redcube-runtime-family-xiaohongshu/src/xiaohongshu-runtime-family-parts/delivery.js';
...
function makePptExportParts() {
  ...
  const exportScript = path.join(workspaceRoot, 'export-helper.mjs');
  ...
  writeFileSync(exportScript, `import { existsSync, readFileSync, writeFileSync } from 'node:fs';\nconst countFile = ${JSON.stringify(callCountFile)};\nconst count = existsSync(countFile) ? Number(readFileSync(countFile, 'utf-8')) : 0;\nwriteFileSync(countFile, String(count + 1));\nconst pptx = process.argv[process.argv.indexOf('--output-pptx') + 1];\nconst pdf = process.argv[process.argv.indexOf('--output-pdf') + 1];\nwriteFileSync(pptx, 'pptx');\nwriteFileSync(pdf, 'pdf');\nconsole.log(JSON.stringify({ status: 'completed', page_count: 1, pptx_file: pptx, pdf_file: pdf, metrics: { page_count: 1, preview_pages: 1 } }));\n`);
