...
import { runtimeStateDisplayPath } from '../packages/redcube-runtime/dist/runtime-state.js';
...
test('phase-2 activation package freeze keeps gate surface, operator flow, minimum test surface, and closeout evidence intact', () => {
  ...
  assert.equal(contract.minimal_test_surface.existing_capability_tests_to_hold.includes('tests/source-intake.test.ts'), true);
  assert.equal(
