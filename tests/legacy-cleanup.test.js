...
test('legacy redcube-agent/workbench runtime path is removed from active code and tests', () => {
  ...
  assert.equal(existsSync(path.join(repoRoot, 'tests', 'llm-runtime-config.test.js')), false);
  ...
  const scopedFiles = [
    ...
    (file) =>
      existsSync(file) &&
      !file.endsWith(path.join('tests', 'legacy-cleanup.test.js')) &&
      !file.endsWith(path.join('tests', 'typescript-service-boundaries.test.js')) &&
      !file.endsWith(path.join('tests', 'public-docs-surface.test.js')) &&
