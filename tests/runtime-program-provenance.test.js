...
test('active machine-readable surfaces use neutral action naming instead of retired gateway fields', () => {
  ...
  for (const file of ACTIVE_MACHINE_READABLE_ROOTS.flatMap((root) => {
    ...
    if (file === 'tests/runtime-program-provenance.test.js') continue;
...
test('historical lane is a compact explicit provenance guard outside the active full suite', () => {
  ...
  const historicalEntry = TEST_REGISTRY.find((entry) => entry.file === 'tests/runtime-program-provenance.test.js');

  assert.deepEqual(groups.historical, ['tests/runtime-program-provenance.test.js']);
  assert.equal(groups.full.includes('tests/runtime-program-provenance.test.js'), false);
  assert.equal(groups['full:with-historical'].includes('tests/runtime-program-provenance.test.js'), true);
