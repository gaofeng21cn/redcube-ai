...
test('xiaohongshu series storyline materializes an actionable series architecture', async () => {
  await withMockCodexRuntime(async () => {
    const workspaceRoot = await createXhsWorkspace({
      workspacePrefix: 'redcube-xhs-series-strategy-',
      title: '垂体瘤诊疗系列科普',
      goal: '策划覆盖认知、诊断、治疗与长期随访的系列小红书笔记',
    });
    const results = await runXhsRoutes(workspaceRoot, ['research', 'storyline']);
    const storyline = results.at(-1)?.artifact?.storyline;

    assert.equal(storyline?.mode, 'series');
    assert.equal(Array.isArray(storyline?.mode_decision?.rationale), true);
    assert.equal(storyline.mode_decision.rationale.length >= 3, true);
    assert.equal(Array.isArray(storyline?.series_architecture?.chapters), true);
    assert.equal(storyline.series_architecture.chapters.length >= 2, true);
    assert.equal(Array.isArray(storyline?.series_architecture?.note_briefs), true);
    assert.equal(storyline.series_architecture.note_briefs.length >= 2, true);
    assert.equal(
      storyline.series_architecture.note_briefs.every((brief) => (
        brief.note_id
        && brief.reader_question
        && brief.content_scope
        && Array.isArray(brief.evidence_anchors)
        && brief.estimated_pages
      )),
      true,
    );
  });
