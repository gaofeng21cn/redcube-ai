...
test('CI workflow pins reproducible toolchain and keeps hosted CI on the honest quality lane', () => {
  ...
  assert.match(workflow, /quality:\n[\s\S]*?uses:\s*actions\/setup-python@v7\b[\s\S]*?python-version:\s*['"]3\.12['"][\s\S]*?uses:\s*astral-sh\/setup-uv@v9\.0\.0\b[\s\S]*?enable-cache:\s*true[\s\S]*?cache-dependency-glob:\s*['"]uv\.lock['"][\s\S]*?uv sync --locked --no-dev --extra native --no-install-project --python 3\.12[\s\S]*?REDCUBE_PYTHON_COMMAND=\$UV_PROJECT_ENVIRONMENT\/bin\/python[\s\S]*?scripts\/verify\.sh ci/);
  assert.doesNotMatch(workflow, /quality:\n[\s\S]*?node scripts\/run-test-group\.ts/);
