...
const DEFAULT_AGENT_LAB_SUITE = new URL('../contracts/production_acceptance/rca-goal-workflow-agent-lab-suite.json', import.meta.url);
...
function runOplAgentLabLongline(oplBin) {
  const result = spawnSync(oplBin, [
    ...
    'run',
    '--suite',
    fileURLToPath(DEFAULT_AGENT_LAB_SUITE),
    '--json',
  ], {
    encoding: 'utf8',
  ...
  assert.equal(
    ...
    0,
    [
      `OPL Agent Lab suite command failed: ${oplBin} agent-lab run --suite ${fileURLToPath(DEFAULT_AGENT_LAB_SUITE)} --json`,
      result.stderr,
      result.stdout,
    ].filter(Boolean).join('\n'),
  );

  return JSON.parse(result.stdout).agent_lab_run?.suite_result;
}
...
function loadOplAgentLabLongline() {
  ...
  return readOplAgentLabLongline().agent_lab_longline?.suite_result;
}
...
test('RCA Agent Lab projection stays refs-only while visual authority stays in RCA', () => {
  const suite = loadOplAgentLabLongline();

  assert.equal(suite?.status, 'passed');
  assert.ok(suite.longline_summary.domain_ids.includes(RCA_DOMAIN_ID));
  assert.equal(suite.longline_summary.ready_to_reduce_domain_longline_tests, false);
  assert.deepEqual(suite.longline_summary.recommended_repo_test_disposition, []);
