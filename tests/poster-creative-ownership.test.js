...
test('poster_onepager route artifacts record Hermes-backed ownership for story, visual, render, and director review surfaces', async () => {
  await withMockHermesUpstream(async () => {
    ...
    await createDeliverable({
      ...
      title: 'P19 单页海报创作权收口',
      goal: '验证 poster 主创作权已从 runtime seed / compiler 收回到 Hermes-backed mainline',
    });
    ...
    assert.equal(storyline.storyline.creative_sources?.headline?.owner, 'hermes');
    assert.equal(storyline.storyline.creative_sources?.headline?.materialized_from, 'upstream_run_json_output');
    ...
    assert.equal(
      blueprint.poster_blueprint.slides.every((slide) => slide.creative_authorship?.major_blueprint_text?.owner === 'hermes'),
      true,
    );
    assert.equal(
      blueprint.poster_blueprint.slides.every((slide) => slide.creative_authorship?.major_blueprint_text?.materialized_from === 'upstream_run_json_output'),
      true,
    );
    ...
    assert.equal(visual.visual_direction.creative_sources?.visual_manifest?.owner, 'hermes');
    assert.equal(visual.visual_direction.creative_sources?.visual_manifest?.materialized_from, 'upstream_run_json_output');
