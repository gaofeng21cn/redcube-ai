...
export function assertManifestActionAndStageControlPlane({
  ...
  const manifestStageIds = new Set(stageManifest.stages.map((stage) => stage.stage_id));
  for (const stage of stageManifest.stages) {
    ...
    assert.equal(stage.next_stage_refs.every((stageId) => manifestStageIds.has(stageId)), true);
  }
