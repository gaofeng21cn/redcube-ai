...
test('getProductEntryManifest projects the current direct-entry shell and shared OPL handoff truth', SERIAL_ENV_TEST, async () => {
  await withMockCodexRuntimeState(async ({ runtimeStateRoot }) => {
    ...
    assert.deepEqual(manifest.product_entry_shell.sidecar.allowed_actions, ['runtime_watch', 'supervise_managed_run', 'product_entry_continuation', 'emit_no_regression_evidence', 'notification_receipt']);
