...
test('getProductEntryManifest projects the current direct-entry shell and shared OPL handoff truth', SERIAL_ENV_TEST, async () => {
  await withMockCodexRuntimeState(async ({ runtimeStateRoot }) => {
    ...
    assertPathValues(manifest, {
      ...
      'product_entry_start.recommended_mode_id': 'start_direct_session',
      ...
      'current_truth.session_continuity_provenance_contract': 'contracts/runtime-program/product-entry-session-continuity.json',
      ...
      'current_truth.product_entry_contract': 'contracts/runtime-program/redcube-product-entry-mvp.json',
      'native_ppt_operator_ux.route_selection.default_visual_route': 'author_image_pages',
      'session_continuity.status': 'repo_tracked',
      'progress_projection.status': 'repo_tracked',
      'artifact_inventory.status': 'repo_tracked',
    });
    ...
    assertIds(manifest.product_entry_quickstart.steps, 'step_id', list('continue_current_loop inspect_current_progress default_image_ppt_proof optional_native_ppt_proof'));
    assertIds(manifest.product_entry_start.modes, 'mode_id', list('start_direct_session opl_hosted_handoff resume_session'));
    ...
    assertPathIncludes(manifest, {
      ...
      'artifact_inventory.inspect_paths': '/opl_ledger_artifact_registration',
    });
