...
test('phase-2 minimum baseline contract freezes canonical quartet, formal entry surfaces, and family consumption expectations', () => {
  const contract = readJson(BASELINE_CONTRACT);
  const gatewayIntake = readImplementation('packages/redcube-gateway/src/actions/intake-source.ts');
  ...
  const deliverableRouteLocal = readImplementation('packages/redcube-runtime/src/deliverable-route-local.ts');
...
test('phase-2 minimum baseline contract records gate surface, tests, and the absorbed next tranche honestly', () => {
  ...
  assert.equal(contract.minimal_test_surface.truth_freeze_tests.includes('tests/phase-2-source-intake-shared-source-truth-baseline.test.ts'), true);
  assert.equal(contract.minimal_test_surface.baseline_capability_tests.includes('tests/source-intake.test.ts'), true);
  assert.equal(contract.minimal_test_surface.baseline_capability_tests.includes('tests/mcp-gateway.test.ts'), true);
