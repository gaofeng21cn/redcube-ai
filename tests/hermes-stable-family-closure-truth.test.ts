...
function assertCurrentExecutionModelTruth(result, route, topicId, deliverableId) {
  ...
  assert.equal(result.execution_model.deployment_host_status, 'active_primary');
  assert.equal(result.execution_model.freeze_origin_milestone, 'P19.A');
}

test('stable family runtime routes expose one current Codex host-agent execution-model truth before gateway wrapping', async () => {
  await withMockCodexHostAgent(async () => {
    ...
    await completeSourceReadiness({
      ...
      brief: '验证 stable family runtime route output 直接暴露当前 Codex host-agent execution truth。',
    ...
    await createDeliverable({
      ...
      goal: '验证 ppt family runtime 直接暴露当前 Codex host-agent execution truth',
    });
    await createDeliverable({
      workspaceRoot,
      overlay: 'xiaohongshu',
      ...
      title: 'Hermes 小红书笔记',
      goal: '验证 xiaohongshu family runtime 直接暴露当前 Codex host-agent execution truth',
    });
    await createDeliverable({
      workspaceRoot,
      ...
      title: 'Hermes 知识海报',
      goal: '验证 poster family runtime 直接暴露当前 Codex host-agent execution truth',
    });
    ...
    assertCurrentExecutionModelTruth(ppt, 'storyline', 'topic-a', 'deck-a');
    assert.equal(ppt.contract.profile_id, 'lecture_student');
    assertCurrentExecutionModelTruth(xhs, 'research', 'topic-a', 'note-a');
    assert.equal(xhs.contract.profile_id, 'standard_note');
    assertCurrentExecutionModelTruth(poster, 'storyline', 'topic-a', 'poster-a');
    assert.equal(poster.contract.profile_id, 'knowledge_poster');
  });
});

test('xiaohongshu human-publication closure keeps the current Codex host-agent route truth without drifting durable surfaces', async () => {
  await withMockCodexHostAgent(async () => {
